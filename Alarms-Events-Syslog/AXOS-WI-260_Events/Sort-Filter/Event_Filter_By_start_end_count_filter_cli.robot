...
Suite Setup       event total count    n1
Library           String
Library           Collections
Resource          base.robot
...
Event_Filter_By_start_end_count_filter
    [Documentation]    Test case verifies events are shown as per count and between ranges via CLI
    ...
    [Tags]            @tcid=AXOS_E72_PARENT-TC-2874    @functional    @priority=P2       @user_interface=CLI

    Log         *** Verify Events are displayed as per count and for count 0 ***
    Run Keyword And Continue On Failure     Verify Events are displayed as per count    n1    ${total_count}

    Log         *** Verify events filter by range ***
    Wait Until Keyword Succeeds    30 seconds    5 seconds    Verify events filter by range    n1    ${total_count}
...
event total count
    ...
    ${total_count}    Getting Active events total count    ${device1}
