*** Settings ***
Documentation     This test suite is going to verify whether the alarms can be filtered using key and value.
Suite Setup       alarm_setup    n1_sh     n1 
Suite Teardown    alarm_teardown    n1_sh    n1   
Resource          base.robot
...
Filtering_alarm_instances_by_address
    ...
    [Tags]           @tcid=AXOS_E72_PARENT-TC-2840    @functional    @priority=P2    @user_interface=CLI

    Log         *** Filtering the alarms by address using key and value ***
    Wait Until Keyword Succeeds    30 seconds    5 seconds    Verify alarms filtered by address    n1    ${port_id}
...
alarm_setup
    ...
    ${port_id}     Wait Until Keyword Succeeds      2 min     10 sec     Triggering Loss of Signal MAJOR alarm      device=${device1}      linux=${device1_linux_mode}     user_interface=cli
...
alarm_teardown
    ...
    Wait Until Keyword Succeeds      2 min     10 sec     Clearing Loss of Signal MAJOR alarm       device=${device1}      linux=${device1_linux_mode}    user_interface=cli
