*** Settings ***
Documentation     This test case is to confirm various traffic works with match rule match untagged. and all the other types of traffic shall be dropped.
Resource          base.robot
...
*** Test Cases ***
tc_Inject_untagged_traffic_with_match_untagged_RLT_SR_2484
    [Documentation]
    ...    #	Action	Expected Result	Notes
    ...    1	Provision match rule match untagged, confirm provision complete.
    ...    2	Inject untagged traffic, confirm the traffic passes through.
    ...    3	Inject not-untagged traffic, confirm the traffic is dropped.
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-949    @globalid=2316407    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-949 setup
    [Teardown]   AXOS_E72_PARENT-TC-949 teardown
    log    STEP:
    ...
    &{result1}    subscriber_point_add_svc    subscriber_point1    untagged    ${p_data_vlan1}    set-stag-pcp=${stag_pcp}    cfg_prefix=match_untag
...
AXOS_E72_PARENT-TC-949 teardown
    ...
    subscriber_point_remove_svc    subscriber_point1    untagged    ${p_data_vlan1}    cfg_prefix=match_untag
