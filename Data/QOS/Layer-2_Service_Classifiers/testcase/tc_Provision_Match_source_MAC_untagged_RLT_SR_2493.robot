...
tc_Provision_Match_source_MAC_untagged_RLT_SR_2493
    ...
    prov_class_map    eutA    ${class_map_name_check}    ethernet    flow     1    1    src-mac=${subscriber_mac1}
    log    check class-map etherent configured
    ${class_map_result}    check_classmap_ethternet    eutA    ${class_map_name_check}    1    1    src-mac=${subscriber_mac1}
    log    ${class_map_result}
    log    duplicate src-mac would be rejected
    ${result}    prov_class_map_without_error_check    eutA    ${class_map_name_check}    ethernet    flow     1    2    src-mac=${subscriber_mac1}
