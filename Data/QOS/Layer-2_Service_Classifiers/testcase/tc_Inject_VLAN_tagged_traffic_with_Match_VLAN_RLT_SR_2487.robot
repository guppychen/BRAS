...
Documentation     This test case is to confirm VLAN tagged traffic works with match rule match VLAN, and all the other traffic than specified VLAN shall be dropped.
*** Variables ***

*** Test Cases ***
tc_Inject_VLAN_tagged_traffic_with_Match_VLAN_RLT_SR_2487
    [Documentation]
    ...    #	Action	Expected Result	Notes
    ...    1	Provision match rule match VLAN 100, confirm provision complete.
    ...    2	Inject VLAN tagged 100 traffic, confirm the traffic passes through.
    ...    3	Inject VLAN tagged 110 traffic, confirm the traffic is dropped.
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-954    @globalid=2316412    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-954 setup
    [Teardown]   AXOS_E72_PARENT-TC-954 teardown
    log    STEP:
    log    STEP:1 Provision match rule match VLAN 100, confirm provision complete.

    log    STEP:2 Inject VLAN tagged 100 traffic, confirm the traffic passes through.

    log    STEP:3 Inject VLAN tagged 110 traffic, confirm the traffic is dropped.
    ...
    prov_class_map    eutA    ${class_map_name_priority}    ethernet    flow     1    1    vlan=${p_data_cvlan1}
...
AXOS_E72_PARENT-TC-954 teardown
