*** Settings ***
Documentation     This test case is to confirm match VLAN tagged and source MAC traffic works with match rule source MAC, and all the other traffic than specified source MAC shall be dropped.
Resource          base.robot
...
*** Test Cases ***
tc_Inject_untagged_traffic_with_Match_source_MAC_RLT_SR_2493
    [Documentation]
    ...    #	Action	Expected Result	Notes
    ...    1	Provision match rule match VLAN plus source MAC (MAC-A), confirm provision complete.
    ...    2	Inject VLAN tagged traffic with source MAC address (MAC-A), confirm the traffic passes through.
    ...    3	Inject VLAN tagged traffic with different source MAC address (MAC-B), confirm the traffic is dropped.
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-968    @globalid=2316426    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-968 setup
    [Teardown]   AXOS_E72_PARENT-TC-968 teardown
    log    STEP:

    log    STEP:1 Provision match rule match VLAN plus source MAC (MAC-A), confirm provision complete.

    log    STEP:2 Inject VLAN tagged traffic with source MAC address (MAC-A), confirm the traffic passes through.

    log    STEP:3 Inject VLAN tagged traffic with different source MAC address (MAC-B), confirm the traffic is dropped

    log    EXA-19958. match mac rule would block tag+mac traffic


    log    STEP:1. match rules: src-mac;
    log    serivce 1
    log    configure class-map match src-mac success
    prov_class_map    eutA    ${class_map_name_ethertype}    ethernet    flow     1    1    src-mac=${subscriber_mac1}
    ...
    log    traffic with tag + match mac
    create_raw_traffic_udp    tg1    up_tag_unmatch    service_p1    subscriber_p1    ovlan=${p_data_vlan1}
    ...    frame_size=512    length_mode=fixed    mac_dst=${service_mac}    mac_src=${subscriber_mac1}    ip_dst=${sip}    ip_src=${cip}
    ...    rate_mbps=${rate_pps1}
    ...
    verify_traffic_all_loss_for_stream    tg1    up_tag_unmatch
...
AXOS_E72_PARENT-TC-968 teardown
