*** Settings ***
Documentation     This test case is to confirm match VLAN tagged and source OUI traffic works with match rule source OUI, and all the other traffic than specified source OUI shall be dropped.
Resource          base.robot
...
*** Test Cases ***
tc_Inject_VLAN_tagged_traffic_with_Match_source_OUI_RLT_SR_2494
    [Documentation]
    ...    #	Action	Expected Result	Notes
    ...    1	Provision match rule match VLAN plus source OUI (OUI-A), confirm provision complete.
    ...    2	Inject VLAN tagged traffic with source OUI address (OUI-A), confirm the traffic passes through.
    ...    3	Inject VLAN tagged traffic with different source OUI address (OUI-B), confirm the traffic is dropped.
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-970    @globalid=2316428    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-970 setup
    [Teardown]   AXOS_E72_PARENT-TC-970 teardown
    log    STEP:
    log    STEP:1 Provision match rule match VLAN plus source OUI (OUI-A), confirm provision complete.

    log    STEP:2 Inject VLAN tagged traffic with source OUI address (OUI-A), confirm the traffic passes through.

    log    STEP:3 Inject VLAN tagged traffic with different source OUI address (OUI-B), confirm the traffic is dropped.
...
AXOS_E72_PARENT-TC-970 teardown
