*** Settings ***
Documentation     This test case is to confirm match untagged and source MAC traffic works with match rule source MAC, and all the other traffic than specified source MAC shall be dropped.
Resource          base.robot
...
tc_Inject_untagged_traffic_with_Match_source_MAC_RLT_SR_2493
    ...
    ...    1	Provision match rule match untagged plus source MAC (MAC-A), confirm provision complete.
    ...    2	Inject untagged traffic with source MAC address (MAC-A), confirm the traffic passes through.
    ...    3	Inject untagged traffic with different source MAC address (MAC-B), confirm the traffic is dropped.
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-964    @globalid=2316422    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-964 setup
    [Teardown]   AXOS_E72_PARENT-TC-964 teardown
    log    STEP:
...
AXOS_E72_PARENT-TC-964 teardown
