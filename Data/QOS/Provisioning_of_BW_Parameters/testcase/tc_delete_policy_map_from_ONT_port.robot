...
Documentation     This test case is to confirm to delete policy map from ONT port.
*** Variables ***

*** Test Cases ***
tc_delete_policy_map_from_ONT_port
    [Documentation]    #	Action	Expected Result	Notes
    ...    1	Assign policy map on ONT port, then delete the policy map, confirm the deletion complete.
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-1011    @globalid=2316473    @priority=P2    @eut=NGPON2-4
    [Setup]      AXOS_E72_PARENT-TC-1011 setup
    [Teardown]   AXOS_E72_PARENT-TC-1011 teardown
    log    STEP:1 Provision match rule match priority-tagged plus PCP (say 5), confirm provision complete.

    log    STEP:1 Assign policy map on ONT port, then delete the policy map, confirm the deletion complete.
...
AXOS_E72_PARENT-TC-1011 teardown
