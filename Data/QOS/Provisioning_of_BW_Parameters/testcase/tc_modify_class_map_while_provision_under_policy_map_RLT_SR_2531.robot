...
Documentation     This test case is to confirm modification of class-map under policy-map works.
*** Variables ***

*** Test Cases ***
tc_modify_class_map_while_provision_under_policy_map_RLT_SR_2531
    [Documentation]    #	Action	Expected Result	Notes
    ...    1	Provision policy-map, then create class-map, confirm some modification on class-map works fine.		
    ...    2	Provision multiple class-map, and modify some of them.			
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-990    @globalid=2316452    @priority=P2    @eut=NGPON2-4
    [Setup]      AXOS_E72_PARENT-TC-990 setup
    [Teardown]   AXOS_E72_PARENT-TC-990 teardown
    log    STEP:
    log    STEP:# Action Expected Result Notes

    log    STEP:1 Provision policy-map, then create class-map, confirm some modification on class-map works fine.
    ...
    log    modify class-map when it is used by policy map
...
AXOS_E72_PARENT-TC-990 teardown
