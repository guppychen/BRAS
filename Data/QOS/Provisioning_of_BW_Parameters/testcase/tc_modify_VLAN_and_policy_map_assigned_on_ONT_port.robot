...
Documentation     This test case is to confirm to assign policy map to ONT port.
*** Variables ***

*** Test Cases ***
tc_assign_policy_map_to_ONT_port
    [Documentation]    #	Action	Expected Result	Notes
    ...    1	Provision policy-map, assign the policy map to ONT port, confirm the provision complete.		
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-1009    @globalid=2316471    @priority=P2    @eut=NGPON2-4
    [Setup]      AXOS_E72_PARENT-TC-1009 setup
    [Teardown]   AXOS_E72_PARENT-TC-1009 teardown
    log    STEP:1 Provision match rule match priority-tagged plus PCP (say 5), confirm provision complete.

    log    STEP:1 Provision policy-map, assign the policy map to ONT port, confirm the provision complete.
    ...
    log    modify class map and policy map
    prov_class_map    eutA    ${class_map_name_priority}    ethernet    flow     1    1    any=${EMPTY}
    log    create policy-map and add svc on ont-ethernet port
    prov_policy_map    eutA    ${policy_map_name}    class-map-ethernet    ${class_map_name_priority}    flow     1    set-stag-pcp=${stag_pcp}
    subscriber_point_add_svc_user_defined    subscriber_point1    ${p_data_vlan2}    ${policy_map_name}


    log    show interface ont port
    ${result}    cli    eutA    show running-config interface ont-ethernet ${service_model.subscriber_point1.name}
    should contain    ${result}    ${policy_map_name}
    should match regexp    ${result}    vlan\\s+${p_data_vlan2}


*** Keyword ***
AXOS_E72_PARENT-TC-1009 setup
...
AXOS_E72_PARENT-TC-1009 teardown
    ...
    run keyword and ignore error    subscriber_point_remove_svc_user_defined    subscriber_point1    ${p_data_vlan2}    ${policy_map_name}
