...
Documentation     This test case is to confirm provision flow under policy map/class-map.
*** Variables ***

*** Test Cases ***
tc_Provision_flow_under_policy_map_class_map_RLT_SR_2531
    [Documentation]    #	Action	Expected Result	Notes
    ...    1	Provision policy-map, class map, then flow. Confirm flow provision works fine.		
    ...    2	Provision multiple flows, confirm multiple flow can be created.		
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-993    @globalid=2316455    @priority=P2    @eut=NGPON2-4
    [Setup]      AXOS_E72_PARENT-TC-993 setup
    [Teardown]   AXOS_E72_PARENT-TC-993 teardown
    log    STEP:
    log    STEP:# Action Expected Result Notes

    log    STEP:1 Provision policy-map, class map, then flow. Confirm flow provision works fine.

    log    STEP:2 Provision multiple flows, confirm multiple flow can be created.
...
AXOS_E72_PARENT-TC-993 teardown
