...
Documentation     This test case is to confirm provision more than max number of flows will be rejected properly.
*** Variables ***

*** Test Cases ***
tc_provision_more_than_8_flow_under_policy_map_is_rejected
    [Documentation]    #	Action	Expected Result	Notes
    ...    1	Provision more than 8 flows under policy map/class-map, confirm provision is rejected with proper error message.			
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-1016    @globalid=2316478    @priority=P2    @eut=NGPON2-4
    [Setup]      AXOS_E72_PARENT-TC-1016 setup
    [Teardown]   AXOS_E72_PARENT-TC-1016 teardown
    log    STEP:
    log    STEP:# Action Expected Result Notes



    log    STEP:1 Provision more than 8 flows under policy map/class-map, confirm provision is rejected with proper error message.
    ...
    log    provision flow 9 with translate-cevlan-tag and set c tag
    log    check translate-cevlan-tag
    ${result}    prov_policy_map_without_error_check    eutA    ${policy_map_name}    class-map-ethernet    ${class_map_name_check}    flow     9    translate-cevlan-tag=${p_data_vlan1}    set-stag-pcp=${ctag_pcp}
...
AXOS_E72_PARENT-TC-1016 teardown
