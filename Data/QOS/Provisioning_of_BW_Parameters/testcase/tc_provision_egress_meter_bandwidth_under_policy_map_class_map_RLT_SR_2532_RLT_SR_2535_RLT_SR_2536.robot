...
Documentation     This test case is to confirm provision egress meter under class map.
*** Variables ***

*** Test Cases ***
tc_provision_egress_meter_bandwidth_under_policy_map_class_map_RLT_SR_2532_RLT_SR_2535_RLT_SR_2536
    [Documentation]    #	Action	Expected Result	Notes
    ...    1	Provision policy-map, class map. Provision egress --> meter, confirm provision works.		
    ...    2	Provision multiple flows under class-map, provision egress meter bandwidth on each flows, confirm provision works.		
    [Tags]       @author=ywu     @TCID=AXOS_E72_PARENT-TC-998    @globalid=2316460    @priority=P2    @eut=NGPON2-4
    [Setup]      AXOS_E72_PARENT-TC-998 setup
    [Teardown]   AXOS_E72_PARENT-TC-998 teardown
    log    STEP:
    log    STEP:# Action Expected Result Notes

    log    STEP:1 Provision policy-map, class map. Provision egress --> meter, confirm provision works.

    log    STEP:2 Provision multiple flows under class-map, provision egress meter bandwidth on each flows, confirm provision works.
...
AXOS_E72_PARENT-TC-998 teardown
