...
&{tg_ds_param}    ovlan=${service_vlan_1}    ivlan=${match_vlan}    mac_dst=${tg_client.mac}    mac_src=${tg_server.mac}    ip_dst=${tg_client.ip}    ip_src=${tg_server.ip}    rate_mbps=${pkt_rate}
${us_traffic_filter}    (vlan.id==${service_vlan_1}) && (vlan.id==${match_vlan}) && (eth.src==${tg_client.mac}) && (eth.dst==${tg_server.mac}) && (ip.src == ${tg_client.ip}) && (ip.dst == ${tg_server.ip})
...
case setup
    ...
    subscriber_point_add_svc    subscriber_point1      ${match_vlan}     ${service_vlan_1}
