...
tc_Match_untagged_add_s_tag
    ...
    ...    1	create a class-map to match untagged in flow 1	successfully		
    ...
    ...    4	apply the s-tag and policy-map to the port of uni	successfully		
    ...    5	send untagged upstream traffic to uni port	eth-port1 can pass the upstream traffic		
    ...    6	send s-tag downstream traffic to eth-port1	client1 can receive the downstream traffic;		

    
    [Tags]     @tcid=AXOS_E72_PARENT-TC-4343      @subFeature=10GE-12: Ethernet class map support      @globalid=2531530      @priority=P1      @eut=10GE-12          @user_interface=CLI    
    [Setup]     case setup
...
case setup
   [Documentation]    setup
    log    STEP:1 create a class-map to match untagged in flow 1
    ...
    log    STEP:4 apply the s-tag and policy-map to the port of ont1
    subscriber_point_add_svc    subscriber_point1    untagged    ${service_vlan_1}

case teardown
    [Documentation]    teardown
    log    svc teardown
    subscriber_point_remove_svc    subscriber_point1    untagged    ${service_vlan_1}
