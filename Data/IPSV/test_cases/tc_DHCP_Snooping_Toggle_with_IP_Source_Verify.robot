*** Settings ***
Documentation     DHCP Snooping Toggle with IP Source Verify: Provision IP Source Verify and DHCP Snooping enabled. Force a client to obtain a DHCP address. Generate UDP (500 byte frames) traffic in each direction. Toggle DHCP Snoop to disabled. Re-generate traffic. Toggle IP Source Verify to enabled. Wait for lease to renew. Re-generate traffic. -> Upstream traffic is only forwarded US when host entry is present.  Downstream direction is forwarded throughout the test.  Downstream traffic when DHCP snoop disabled would not be allowed after the ARP entry has timed out but that is not the focus of this test.
...
...    SR Security Feature Interaction Config #12: DHCP, DHCP Snoop Enabled, MACFF Disabled, IPSV Enabled
...
...    SR Security Feature Interaction Config #4: DHCP, DHCP Snoop Disabled, MACFF Disabled, IPSV Enabled
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Snooping_Toggle_with_IP_Source_Verify
    [Documentation]    DHCP Snooping Toggle with IP Source Verify: Provision IP Source Verify and DHCP Snooping enabled. Force a client to obtain a DHCP address. Generate UDP (500 byte frames) traffic in each direction. Toggle DHCP Snoop to disabled. Re-generate traffic. Toggle IP Source Verify to enabled. Wait for lease to renew. Re-generate traffic. -> Upstream traffic is only forwarded US when host entry is present.  Downstream direction is forwarded throughout the test.  Downstream traffic when DHCP snoop disabled would not be allowed after the ARP entry has timed out but that is not the focus of this test.
    ...
    ...    SR Security Feature Interaction Config #12: DHCP, DHCP Snoop Enabled, MACFF Disabled, IPSV Enabled
    ...
    ...    SR Security Feature Interaction Config #4: DHCP, DHCP Snoop Disabled, MACFF Disabled, IPSV Enabled
    [Tags]       @TCID=AXOS_E72_PARENT-TC-564    @GlobalID=2286111    @EUT=NGPON2-4
    [Setup]      setup
    ...
    log    dhcp disabled
    dprov_vlan    eutA    ${service_vlan}    l2-dhcp-profile
    Tg Clear Traffic Stats    tg1
    Tg Start All Traffic    tg1
    sleep    ${traffic_run_time}
    Tg Stop All Traffic    tg1
    ...
    log    enable dhcp again, all streams perform as before
