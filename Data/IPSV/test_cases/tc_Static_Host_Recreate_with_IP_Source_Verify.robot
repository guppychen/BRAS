*** Settings ***
Documentation     Static Host Recreate with IP Source Verify: Provision IP Source Verify enabled with a static host entry. Generate UDP (500 byte frames) traffic in each direction. Delete the static entry. Re-generate traffic. Recreate static entry. Re-generate traffic. -> Traffic is only forwarded US when static host entry is present.
...
...
*** Test Cases ***
tc_Static_Host_Recreate_with_IP_Source_Verify
    [Documentation]    Static Host Recreate with IP Source Verify: Provision IP Source Verify enabled with a static host entry. Generate UDP (500 byte frames) traffic in each direction. Delete the static entry. Re-generate traffic. Recreate static entry. Re-generate traffic. -> Traffic is only forwarded US when static host entry is present.
    ...
    ...    Note: DSL Downstream traffic will continue to flow when static entry is removed but ONT will stop traffic.  Assumption is that because MACFF is enabled and entry no longer exists the GPON will no longer forward traffic.

    ...    SR Security Feature Interaction Config #7: Static, DHCP Snoop Disabled, MACFF Enabled, IPSV Enabled
    [Tags]       @TCID=AXOS_E72_PARENT-TC-560    @GlobalID=2286107    @EUT=NGPON2-4
    [Setup]      setup
    ...
    prov_ipv4_l2host_on_sub_port    eutA    subscriber_point1    ${service_vlan}    ${client_ip2}    mac ${client_mac2} gateway1 ${gateway2}
    ...
    stop_capture    tg1    subscriber_p1
    ${res4}    get_packet_counter_on_port_with_filter    tg1    service_p1    arp and eth.src==${service_mac2}       ${pcapfile}
...
setup
    ...
    log    provision static host with mac
