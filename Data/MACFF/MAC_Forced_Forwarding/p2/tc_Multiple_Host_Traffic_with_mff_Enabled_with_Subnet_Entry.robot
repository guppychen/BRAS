*** Settings ***
Documentation     Provision static subnet entry with mff enabled.  Generate upstream UDP traffic for multiple hosts in the subnet. -> All traffic is forwarded
Resource          ./base.robot
...
*** Test Cases ***
tc_Multiple_Host_Traffic_with_mff_Enabled_with_Subnet_Entry
    [Documentation]    Provision static subnet entry with mff enabled.  Generate upstream UDP traffic for multiple hosts in the subnet. -> All traffic is forwarded
    [Tags]    @tcid=AXOS_E72_PARENT-TC-1412    @subFeature=MAC_Forced_Forwarding    @globalid=2286181    @priority=P2   @eut=NGPON2-4    @user_interface=CLI
    [Setup]      AXOS_E72_PARENT-TC-1412 setup
    [Teardown]   AXOS_E72_PARENT-TC-1412 teardown
    ...
    log    create traffic
    Tg Create Bound Single Tagged Stream On Port    tg1    us1    subscriber_p1    host2    host1    vlan_id=${subscriber_vlan1}    l2_encap=ethernet_ii_vlan
    ...      vlan_user_priority=0    rate_mbps=1    l4_protocol=udp    udp_src_port=1000    udp_dst_port=1000   length_mode=fixed   frame_size=512     
    Tg Create Bound Single Tagged Stream On Port    tg1    us3    subscriber_p1    host2    host3    vlan_id=${subscriber_vlan1}    l2_encap=ethernet_ii_vlan
    ...      vlan_user_priority=0    rate_mbps=1    l4_protocol=udp    udp_src_port=1000    udp_dst_port=1000   length_mode=fixed   frame_size=512 
...
AXOS_E72_PARENT-TC-1412 setup
    ...
    ...                         enable_ping_response=1     mac_addr=${subscriber_mac3}    encapsulation=ethernet_ii_vlan    vlan_id=${subscriber_vlan1}

    
AXOS_E72_PARENT-TC-1412 teardown
