*** Settings ***
Documentation     Provision mff enabled. Provision a static host and subnet entry on the same port of the ONT. Generate upstream UDP traffic. -> All traffic is forwarded
Resource          ./base.robot
...
*** Test Cases ***
tc_mff_Static_Host_Subnet_on_Same_ONT_Port
    [Documentation]    Provision mff enabled. Provision a static host and subnet entry on the same port of the ONT. Generate upstream UDP traffic. -> All traffic is forwarded
    [Tags]    @tcid=AXOS_E72_PARENT-TC-1421    @subFeature=MAC_Forced_Forwarding    @globalid=2286190    @priority=P2   @eut=NGPON2-4    @user_interface=CLI
    [Setup]      AXOS_E72_PARENT-TC-1421 setup
    [Teardown]   AXOS_E72_PARENT-TC-1421 teardown
    log    issue arp requests  
    Tg Stc Device Transmit Arp    tg1    host1
    Tg Stc Device Transmit Arp    tg1    host2
    Tg Stc Device Transmit Arp    tg1    host3
    log    create traffic
    Tg Create Bound Single Tagged Stream On Port    tg1    us1    subscriber_p1    host3    host1    vlan_id=${subscriber_vlan1}    l2_encap=ethernet_ii_vlan
    ...      vlan_user_priority=0    rate_mbps=1    l4_protocol=udp    udp_src_port=1000    udp_dst_port=1000   length_mode=fixed   frame_size=512     
    Tg Create Bound Single Tagged Stream On Port    tg1    us3    subscriber_p1    host3    host2    vlan_id=${subscriber_vlan1}    l2_encap=ethernet_ii_vlan
    ...      vlan_user_priority=0    rate_mbps=1    l4_protocol=udp    udp_src_port=1000    udp_dst_port=1000   length_mode=fixed   frame_size=512     
...
AXOS_E72_PARENT-TC-1421 setup
    ...
    prov_ipv4_l2host_on_sub_port     eutA     subscriber_point1    ${service_vlan1}    ${network_ip1}    gateway1 ${gateway_ip1} mask ${mask_ip2}
    prov_ipv4_l2host_on_sub_port     eutA     subscriber_point1    ${service_vlan1}    ${subscriber_ip2}    gateway1 ${gateway_ip3}
    prov_ipv4_l2host_on_sub_port     eutA     subscriber_point2    ${service_vlan1}    ${subscriber_ip3}    gateway1 ${gateway_ip1}
    log    create devices
    ...
    Tg Stc Create Device On Port     tg1    host3    subscriber_p1    intf_ip_addr=${subscriber_ip3}    gateway_ip_addr=${gateway_ip1}    resolve_gateway_mac=true
    ...                         enable_ping_response=1     mac_addr=${subscriber_mac3}    encapsulation=ethernet_ii_vlan    vlan_id=${subscriber_vlan2}
    Tg Stc Create Device On Port     tg1    host2    subscriber_p1    intf_ip_addr=${subscriber_ip2}    gateway_ip_addr=${gateway_ip3}    resolve_gateway_mac=true
    ...                         enable_ping_response=1     mac_addr=${subscriber_mac2}    encapsulation=ethernet_ii_vlan    vlan_id=${subscriber_vlan1}
    
AXOS_E72_PARENT-TC-1421 teardown
    ...
    Tg Stc Delete Device On Port    tg1    host3    subscriber_p1   mac_addr=${subscriber_mac3}
