*** Settings ***
Documentation     Provision mff service and dhcp snoop enabled. Force a client to obtain an IP address. From the dhcp client generate broadcast traffic. -> No traffic is forwarded.
Resource          ./base.robot
Force Tags       @author=wchen    @feature=MACFF
*** Variables ***
*** Test Cases ***
tc_Broadcast_Upstream_Drop_dhcp
    [Documentation]     Provision mff service and dhcp snoop enabled. Force a client to obtain an IP address. From the dhcp client generate broadcast traffic. -> No traffic is forwarded.
    [Tags]    @subFeature=MAC_Forced_Forwarding    @tcid=AXOS_E72_PARENT-TC-1420    @globalid=2286189    @priority=P1   @eut=NGPON2-4    @user_interface=CLI
    [Setup]      AXOS_E72_PARENT-TC-1420 setup
    [Teardown]   AXOS_E72_PARENT-TC-1420 teardown
    ...
    log    create traffic
    Tg Create Single Tagged Stream On Port    tg1    us    subscriber_p1    service_p1    vlan_id=${subscriber_vlan1}    vlan_user_priority=0
    ...    mac_src=${subscriber_mac1}    mac_dst=FF:FF:FF:FF:FF:FF    rate_mbps=1    frame_size=512    length_mode=fixed    l3_protocol=ipv4
    ...    ip_src_addr=${pool_start_ip}    ip_dst_addr=255.255.255.255    
    log    start traffic
...
AXOS_E72_PARENT-TC-1420 teardown
