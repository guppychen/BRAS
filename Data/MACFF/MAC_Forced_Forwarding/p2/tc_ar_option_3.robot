*** Settings ***
Documentation     Enable mff and provision DHCP server with option 3. Force a client to obtain a DHCP address. Generate continuous upstream UDP traffic. -> Traffic is forwarded and proxied by AR learned throught option 3.
...
tc_ar_option_3
    [Documentation]    Enable mff and provision DHCP server with option 3. Force a client to obtain a DHCP address. Generate continuous upstream UDP traffic. -> Traffic is forwarded and proxied by AR learned throught option 3.
    [Tags]    @tcid=AXOS_E72_PARENT-TC-1430        @subFeature=MAC_Forced_Forwarding        @globalid=2286199    @priority=P2   @eut=NGPON2-4    @user_interface=CLI
    [Setup]      AXOS_E72_PARENT-TC-1430 setup
    [Teardown]   AXOS_E72_PARENT-TC-1430 teardown
...
AXOS_E72_PARENT-TC-1430 setup
    ...
    ...    ip_version=4    ip_address=${service_ip1}    encapsulation=ETHERNET_II_VLAN    vlan_id=${service_vlan1}    dhcp_offer_options=1
...
#    create_dhcp_client    tg1    dclient1    subscriber_p1    dgroup1    ${subscriber_mac1}    ${subscriber_vlan1}   opt_list=0x01060f79
...
AXOS_E72_PARENT-TC-1430 teardown
