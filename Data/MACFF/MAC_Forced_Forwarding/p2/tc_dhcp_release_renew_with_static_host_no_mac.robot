*** Settings ***
Documentation     Provision ONT service with DHCP Snooping and mff enabled and a static host without mac. Force a client to obtain a DHCP address. Generate continuous upstream traffic for static host. Release DHCP client address. Renew DHCP client address. -> Traffic flows for static host at all times through DHCP Release & Renew.
Resource          ./base.robot
...
*** Test Cases ***
tc_dhcp_release_renew_with_static_host_no_mac
    [Documentation]    Provision ONT service with DHCP Snooping and mff enabled and a static host without mac. Force a client to obtain a DHCP address. Generate continuous upstream traffic for static host. Release DHCP client address. Renew DHCP client address. -> Traffic flows for static host at all times through DHCP Release & Renew.
    [Tags]    @tcid=AXOS_E72_PARENT-TC-2948    @subFeature=MAC_Forced_Forwarding    @globalid=2392101    @priority=P2   @eut=NGPON2-4    @user_interface=CLI
    [Setup]      AXOS_E72_PARENT-TC-2948 setup
    [Teardown]   AXOS_E72_PARENT-TC-2948 teardown
    
    log     issue arp request
    Tg Stc Device Transmit Arp    tg1    host1
    Tg Stc Device Transmit Arp    tg1    host2
    log    create traffic
    Tg Create Bound Single Tagged Stream On Port    tg1    us    subscriber_p1    host1    host2    vlan_id=${subscriber_vlan2}    l2_encap=ethernet_ii_vlan
...
AXOS_E72_PARENT-TC-2948 teardown
