*** Settings ***
Documentation     Provision service with DHCP Snooping and mff enabled. Force a client to obtain a DHCP address using a rebind time longer than the time it takes to execute test case. Generate continuous upstream UDP traffic. Disable mff. Re-enable mff. -> Traffic flows for all mff state toggles.
Resource          ./base.robot
...
tc_dhcp_mff_toggle
    [Documentation]    Provision service with DHCP Snooping and mff enabled. Force a client to obtain a DHCP address using a rebind time longer than the time it takes to execute test case. Generate continuous upstream UDP traffic. Disable mff. Re-enable mff. -> Traffic flows for all mff state toggles.
    [Tags]    @tcid=AXOS_E72_PARENT-TC-1383    @subFeature=MAC_Forced_Forwarding    @globalid=2286152    @priority=P1   @eut=NGPON2-4
    [Setup]      AXOS_E72_PARENT-TC-1383 setup
    [Teardown]   AXOS_E72_PARENT-TC-1383 teardown
    log    STEP:Provision service with DHCP Snooping and mff enabled. Force a client to obtain a DHCP address using a rebind time longer than the time it takes to execute test case. Generate continuous upstream UDP traffic. Disable mff. Re-enable mff. -> Traffic flows for all mff state toggles.
...
AXOS_E72_PARENT-TC-1383 teardown
