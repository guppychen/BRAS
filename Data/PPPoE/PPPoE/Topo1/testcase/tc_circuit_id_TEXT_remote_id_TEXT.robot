*** Settings ***
Documentation     circuit-id=TEXT ; remote-id=TEXT
Resource          ./base.robot
Force Tags        @feature=pppoe    @subfeature=pppoe    @author=joli

*** Variables ***
${circuit-id}   1111111111111111111111111111111111111111111111111111111111111111111111
${remote-id}    2222222222222222222222222222222222222222222222222222222222222222222222
${circuit-id1}  111111111111111111111111111111111111111111111111111111111111111
${remote-id1}   222222222222222222222222222222222222222222222222222222222222222

*** Test Cases ***
tc_circuit_id_TEXT_remote_id_TEXT
    [Documentation]    circuit-id=TEXT ; remote-id=TEXT
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2516    @globalid=2360112    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    ...
    check_circuit_id    tg1    service_p1    ${circuit-id1}
    check_remote_id    tg1    service_p1    ${remote-id1}

*** Keywords ***
setup
    [Documentation]  setup
    log    id-profile provision
    prov_id_profile    eutA    ${id_prf1}    circuit-id=${circuit-id}    remote-id=${remote-id}
    log    create a vlan
