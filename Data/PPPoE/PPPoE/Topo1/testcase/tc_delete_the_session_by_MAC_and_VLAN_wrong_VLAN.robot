*** Settings ***
Documentation     delete the session by MAC and VLAN (wrong VLAN)
Resource          ./base.robot
...
*** Test Cases ***
tc_delete_the_session_by_MAC_and_VLAN_wrong_VLAN
    [Documentation]    delete the session by MAC and VLAN (wrong VLAN)
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2392    @globalid=2356954    @eut=NGPON2-4    @priority=P2
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:delete the session by MAC and VLAN (wrong VLAN)
    ...
    log    delete the session MAC and VLAN (wrong VLAN)
    delete_pppoe_session    eutA     ${p_data_vlan2}    ${client_mac}
    check_pppoe_sessions_summary    eutA    ${p_data_vlan}    ${client_mac}
