*** Settings ***
Documentation     circuit-id=%ShortFormat ; remote-id=%PortNumber
Resource          ./base.robot
...
*** Test Cases ***
tc_circuit_id_ShortFormat_remote_id_PortNumber
    [Documentation]    circuit-id=%ShortFormat ; remote-id=%PortNumber
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2508    @globalid=2360104    @eut=NGPON2-4    @priority=P2
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:circuit-id=%ShortFormat ; remote-id=%PortNumber
    ...
    check_remote_id_portnumber    tg1    service_p1

*** Keywords ***
case setup
    [Documentation]  setup
    log    id-profile provision
    prov_id_profile    eutA    ${id_prf1}    remote-id=%PortNumber
    log    create a vlan
