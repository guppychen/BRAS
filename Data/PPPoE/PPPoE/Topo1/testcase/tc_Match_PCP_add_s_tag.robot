*** Settings ***
Documentation     Match PCP, add s-tag
Resource          ./base.robot
...
tc_Match_PCP_add_s_tag
    [Documentation]    Match PCP, add s-tag
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2360    @globalid=2356922    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Match PCP, add s-tag
    log    create PPPoE server and client on STC
    # wait the engine to start
    sleep    10
    TG Create Pppoe v4 Server On Port   tg1    ${server_name}    service_p1    encap=ethernet_ii_qinq    vlan_id=${p_match_vlan3}    vlan_user_priority=0
    ...    vlan_id_outer=${p_data_vlan}    vlan_outer_user_priority=0    vlan_id_count=1    num_sessions=1    mac_addr=${server_mac}
    TG Create PPPoE v4 Client On Port   tg1    ${client_name}    subscriber_p1    encap=ethernet_ii_vlan    vlan_id=${p_match_vlan3}    vlan_user_priority=${match_pcp_1}
...
setup
    ...
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%Shelf    remote-id=%Shelf
    log    create a vlan
