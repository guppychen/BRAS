*** Settings ***
Documentation     Multi S-VLANs under an ont-port, IA=%Stag
Resource          ./base.robot
...
*** Test Cases ***
tc_Multi_S_VLANs_under_an_ont_port_IA_Stag
    [Documentation]    Multi S-VLANs under an ont-port, IA=%Stag
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2368    @globalid=2356930    @eut=NGPON2-4    @priority=P2
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:Multi S-VLANs under an ont-port, IA=%Stag
    ...
    # wait to stop
    sleep    ${wait_time}
    TG Verify Traffic Loss For Stream Is Within    tg1    upstream    0.01
    TG Verify Traffic Loss For Stream Is Within    tg1    downstream    ${loss_rate}
    TG Verify Traffic Loss For Stream Is Within    tg1    upstream2    0.01
...
case setup
    [Documentation]  setup
    [Arguments]
    log    create id-profile with circute-ID and remote-ID as %Stag
