*** Settings ***
Documentation     (1:1 mode)Match untagged
...
*** Test Cases ***
tc_1_1_mode_Match_untagged
    [Documentation]    (1:1 mode)Match untagged
    [Tags]    @author=joli    @tcid=AXOS_E72_PARENT-TC-2517    @globalid=2360113    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    create PPPoE server and client on STC
    # wait the engine to start
    sleep    10
    TG Create Pppoe v4 Server On Port    tg1    ${server_name}    service_p1    encap=ethernet_ii_qinq    vlan_id=${cvlan}
    ...    vlan_user_priority=0    vlan_id_outer=${p_data_vlan3}    vlan_outer_user_priority=0    vlan_id_count=1    num_sessions=1    mac_addr=${server_mac}
    ...
    Tg Create Bound Untagged Stream On Port    tg1    upstream    subscriber_p1    ${server_name}    ${client_name}
    ...    frame_size=500    rate_kbps=${rate_kbps}    length_mode=fixed
    Tg Create Bound Untagged Stream On Port    tg1    downstream    service_p1    ${client_name}    ${server_name}
    ...    frame_size=500     rate_kbps=${rate_kbps}    length_mode=fixed
...
setup
    ...
    prov_class_map    eutA    ${class_map_name}    ${class_map_type}    flow    ${flow_index}    ${rule_index}    untagged=${EMPTY}
