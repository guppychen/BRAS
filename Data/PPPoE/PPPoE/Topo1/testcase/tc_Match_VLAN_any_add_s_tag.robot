*** Settings ***
Documentation     Match VLAN any, change c-tag
Resource          ./base.robot
...
*** Test Cases ***
tc_Match_VLAN_any_add_s_tag
    [Documentation]    Match VLAN, change c-tag
    [Tags]       @author=joli     @TCID=AXOS_E72_PARENT-TC-2367    @globalid=2356929    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Match VLAN any, change c-tag
    log    create PPPoE server and client on STC
    # wait the engine to start
    sleep    10
    TG Create Pppoe v4 Server On Port   tg1    ${server_name}    service_p1    encap=ethernet_ii_qinq    vlan_id=${p_match_vlan2}    vlan_user_priority=0
    ...    vlan_id_outer=${p_data_vlan}    vlan_outer_user_priority=0    vlan_id_count=1    num_sessions=1    mac_addr=${server_mac}
    TG Create PPPoE v4 Client On Port   tg1    ${client_name}    subscriber_p1    encap=ethernet_ii_vlan    vlan_id=${p_match_vlan2}    vlan_user_priority=0
...
setup
    ...
    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan2}    ${p_data_vlan}
...
teardown
    ...
    subscriber_point_remove_svc    subscriber_point1    ${p_match_vlan2}    ${p_data_vlan}
