*** Settings ***
Documentation     circuit-id=%Desc; remote-id=%Serial
...
*** Test Cases ***
tc_circuit_id_Desc_remote_id_Serial
    [Documentation]    circuit-id=%Desc; remote-id=%Serial
    [Tags]    @author=joli    @tcid=AXOS_E72_PARENT-TC-2512    @globalid=2360108    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    ...
    ${sub_id}    get_subscriber_id    subscriber_point1
    check_circuit_id    tg1    service_p1    ${sub_id}
    ${serial}    get_ont_serial    subscriber_point1
    check_remote_id_substring    tg1    service_p1    ${sub_num}    ${serial}
...
setup
    ...
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%Desc    remote-id=%Serial
    log    create a vlan
    ...
    prov_subscriber_id    subscriber_point1    ${desc}
