*** Settings ***
Documentation     circuit-id=%Port ; remote-id=%QTag
Resource          ./base.robot
...
*** Test Cases ***
tc_circuit_id_Port_remote_id_QTag
    [Documentation]    circuit-id=%Port ; remote-id=%QTag
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2509    @globalid=2360105    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    ...
    ${pon_port}    get_pon_port    subscriber_point1
    check_circuit_id    tg1    service_p1    ${pon_port}
    check_remote_id    tg1    service_p1    ${p_match_vlan}

*** Keywords ***
setup
    [Documentation]  setup
    log    id-profile provision
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%Port    remote-id=%QTag
    log    create a vlan
