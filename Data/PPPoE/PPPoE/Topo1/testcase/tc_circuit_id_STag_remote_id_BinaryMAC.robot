*** Settings ***
Documentation     circuit-id=%STag ; remote-id=%BinaryMAC
Resource          ./base.robot
...
*** Test Cases ***
tc_circuit_id_STag_remote_id_BinaryMAC
    [Documentation]    circuit-id=%STag ; remote-id=%BinaryMAC
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2511    @globalid=2360107    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    ...
    check_remote_id_bmac    tg1    service_p1
...
setup
    ...
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%STag    remote-id=%BinaryMAC
    log    create a vlan
