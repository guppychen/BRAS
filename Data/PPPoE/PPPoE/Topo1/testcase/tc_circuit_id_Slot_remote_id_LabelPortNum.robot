*** Settings ***
Documentation     circuit-id=%Slot ; remote-id=%LabelPortNum
Resource          ./base.robot
...
*** Test Cases ***
tc_circuit_id_Slot_remote_id_LabelPortNum
    [Documentation]    circuit-id=%Slot ; remote-id=%LabelPortNum
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2507    @globalid=2360103    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    ...
    check_circuit_id    tg1    service_p1    1
    check_remote_id_labelportnum    tg1    service_p1

*** Keywords ***
setup
    [Documentation]  setup
    log    id-profile provision
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%Slot    remote-id=%LabelPortNum
    log    create a vlan
