*** Settings ***
Documentation     circuit-id=%OntID ; remote-id=%OntPort
Resource          ./base.robot
...
*** Test Cases ***
tc_circuit_id_OntID_remote_id_OntPort
    [Documentation]    circuit-id=%OntID ; remote-id=%OntPort
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2514    @globalid=2360110    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    ...
    check_circuit_id    tg1    service_p1    ${service_model.subscriber_point1.attribute.ont_id}
    ${ont_port}    get_ont_port    subscriber_point1
    check_remote_id    tg1    service_p1    ${ont_port}

*** Keywords ***
setup
    [Documentation]  setup
    log    id-profile provision
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%OntID    remote-id=%OntPort
    log    create a vlan
