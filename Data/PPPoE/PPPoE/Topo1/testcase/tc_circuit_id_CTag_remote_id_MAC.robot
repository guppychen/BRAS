*** Settings ***
Documentation     circuit-id=%CTag ; remote-id=%MAC
...
*** Test Cases ***
tc_circuit_id_CTag_remote_id_MAC
    [Documentation]    circuit-id=%CTag ; remote-id=%MAC
    [Tags]    @author=joli    @tcid=AXOS_E72_PARENT-TC-2510    @globalid=2360106    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    ...
    Tg Save Config Into File    tg1     /tmp/nego.xml
    ...
    check_circuit_id    tg1    service_p1    ${cvlan}
    ${res}    get_onu_mac    subscriber_point1
    check_remote_id    tg1    service_p1    ${res}
...
setup
    ...
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%CTag    remote-id=%MAC
    log    create a vlan
