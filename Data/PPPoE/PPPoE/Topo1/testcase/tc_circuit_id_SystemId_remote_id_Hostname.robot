*** Settings ***
Documentation     circuit-id=%SystemId ; remote-id=%Hostname
...
*** Test Cases ***
tc_circuit_id_SystemId_remote_id_Hostname
    [Documentation]    circuit-id=%SystemId ; remote-id=%Hostname
    [Tags]    @author=joli    @tcid=AXOS_E72_PARENT-TC-2505    @globalid=2360101    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    ...
    ${host}    get_host_name    subscriber_point1
    check_circuit_id    tg1    service_p1    ${host}
    check_remote_id    tg1    service_p1    ${host}
...
setup
    ...
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%SystemId    remote-id=%Hostname
    log    create a vlan
