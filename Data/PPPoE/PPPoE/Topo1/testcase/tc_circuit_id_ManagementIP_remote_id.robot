*** Settings ***
Documentation     circuit-id=%ManagementIP ; remote-id=%%
Resource          ./base.robot
...
*** Test Cases ***
tc_circuit_id_ManagementIP_remote_id
    [Documentation]    circuit-id=%ManagementIP ; remote-id=%%
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-2515    @globalid=2360111    @eut=NGPON2-4    @priority=P2
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:circuit-id=%ManagementIP ; remote-id=%%
    ...
    # ${mgmt_ip}    get_mgmt_ip    subscriber_point1
    ${mgmt_ip}    set variable    ${DEVICES.eutA.ip}
    check_circuit_id    tg1    service_p1    ${mgmt_ip}
    check_remote_id    tg1    service_p1    %
...
case setup
    ...
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%ManagementIP    remote-id=%%
    log    create a vlan
