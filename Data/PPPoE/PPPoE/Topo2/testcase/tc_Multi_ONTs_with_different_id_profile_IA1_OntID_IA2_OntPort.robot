*** Settings ***
Documentation     Multi ONTs with different id-profile, IA1=%OntID IA2=%OntPort
...
*** Test Cases ***
tc_Multi_ONTs_with_different_id_profile_IA1_OntID_IA2_OntPort
    [Documentation]    Multi ONTs with different id-profile, IA1=%OntID IA2=%OntPort
    [Tags]    @author=joli    @tcid=AXOS_E72_PARENT-TC-2377    @globalid=2356939    @eut=NGPON2-4    @priority=P2
    [Setup]    case setup
    log    STEP:Multi ONTs with different id-profile, IA1=%OntID IA2=%OntPort
    ...
    ...    mac_addr=${client_mac}
    TG Create Pppoe v4 Server On Port    tg1    ${server_name2}    service_p1    encap=ethernet_ii_vlan    vlan_id=${p_data_vlan2}    vlan_user_priority=0
    ...    vlan_id_count=1    num_sessions=1    mac_addr=${server_mac2}
    ...
    ...    rate_kbps=${rate_kbps}    length_mode=fixed
    Tg Save Config Into File    tg1     /tmp/liyu_stream.xml
...
case setup
    ...
    prov_id_profile    eutA    ${id_prf1}    circuit-id=%OntPort    remote-id=%OntPort
    prov_id_profile    eutA    ${id_prf2}    circuit-id=%OntID    remote-id=%OntID
    log    use the id-profile in VLAN 100 and 200
    prov_vlan    eutA    ${p_data_vlan}    pppoe-ia-id-profile=${id_prf1}    mac-learning=enable
    prov_vlan    eutA    ${p_data_vlan2}    pppoe-ia-id-profile=${id_prf2}    mac-learning=enable
    ...
    subscriber_point_add_svc    subscriber_point2    untagged    ${p_data_vlan2}    cfg_prefix=auto2
...
case teardown
    ...
    subscriber_point_remove_svc    subscriber_point2    untagged    ${p_data_vlan2}    cfg_prefix=auto2
    ...
    delete_config_object    eutA    id-profile    ${id_prf2}
