*** Settings ***
Documentation     DHCP Relay Enabled insert Policy remote-id %hostname
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Relay_Enabled_insert_Policy_remote_id_hostname
    [Documentation]    1	Force an access subscriber(ethernet interface) to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server. Uplink trunk is LAG interface. -> Opt82 is inserted as remote-id hostname.	All Step action expected Results must be correct.The name of the host. This is generally 16 characters or less. Its length is defined on a per-platform basis.
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2183    @globalid=2343999    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1
    [Setup]      case setup
...
case setup
    ...
    prov_id_profile    eutA    ${id_profile_name}    remote-id=%Hostname
