*** Settings ***
Documentation     DHCP Relay Enabled insert Policy circuit-id %systemid
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Relay_Enabled_insert_Policy_circuit_id_systemid
    [Documentation]    1	Force an access subscriber to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server.uplink is ethernet interface. Opt82 is inserted as circuit-id systemid	All Step action expected Results must be correct.This field is a string of maximum 20 characters defining the globally unique ID of the system. It is provisioned by the administrator of the platform. The system default is an empty string "".
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2132    @globalid=2343948    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1    @user_interface=CLI
    [Setup]      case setup
...
case setup
    ...
    prov_id_profile    eutA    ${id_profile_name}    circuit-id=%SystemId
