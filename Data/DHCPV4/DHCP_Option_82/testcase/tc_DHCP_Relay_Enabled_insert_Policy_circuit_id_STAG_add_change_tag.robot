*** Settings ***
Documentation     DHCP Relay Enabled insert Policy circuit-id %STAG. add-change-tag
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Relay_Enabled_insert_Policy_circuit_id_STAG_add_change_tag
    [Documentation]    1	create svc: add-change-tag(translate-cevlan-tag)	success
    ...    2	Force an access subscriber to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server. Uplink trunk is ethernet interface. -> Opt82 is inserted as circuit-id stag	All Step action expected Results must be correct.The service VLAN
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2148    @globalid=2343964    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 create svc: add-change-tag(translate-cevlan-tag) success
    log    STEP:2 Force an access subscriber to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server. Uplink trunk is ethernet interface. -> Opt82 is inserted as circuit-id stag All Step action expected Results must be correct.The service VLAN
