*** Settings ***
Documentation     DHCP Relay Enabled insert Policy remote-id %PortNumber
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Relay_Enabled_insert_Policy_remote_id_PortNumber
    [Documentation]    1	Force an access subscriber(ethernet interface) to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server. Uplink trunk is ethernet interface. -> Opt82 is inserted as remote-id portnumber	All Step action expected Results must be correct.Port number. Format: +. Port Type: gfast=000. g=100. e=200. x=300. gbond=400. Ex: gfast1 - 001. g2 - 102
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2189    @globalid=2344005    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1    @user_interface=CLI
    [Setup]      case setup
...
case setup
    ...
    prov_id_profile    eutA    ${id_profile_name}    remote-id=%PortNumber
