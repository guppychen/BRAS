*** Settings ***
Documentation     DHCP Relay Enabled insert Policy circuit-id %desc
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Relay_Enabled_insert_Policy_circuit_id_desc
    [Documentation]    1	Force an access subscriber(ethernet interface) to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server. Uplink trunk is ethernet interface. -> Opt82 is inserted as circuit-id %desc	All Step action expected Results must be correct.The receiving port's description attribute
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2156    @globalid=2343972    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1
    [Setup]      case setup
