*** Settings ***
Documentation     DHCP Relay Enabled insert Policy circuit-id %MAC
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Relay_Enabled_insert_Policy_circuit_id_MAC
    [Documentation]    1	Force an access subscriber(ethernet interface) to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server. Uplink trunk is ethernet interface. -> Opt82 is inserted as circuit-id mac-addr.	All Step action expected Results must be correct.The receiving port's MAC	DHCP Relay Enabled insert Policy circuit-id %MAC. uplink is LAG
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2154    @globalid=2343970    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1    @user_interface=CLI
    [Setup]      case setup
