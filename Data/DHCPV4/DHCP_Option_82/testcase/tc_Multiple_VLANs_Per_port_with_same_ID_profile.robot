*** Settings ***
Documentation     Multiple VLANs Per port with same ID-profile
Resource          ./base.robot
...
*** Test Cases ***
tc_Multiple_VLANs_Per_port_with_same_ID_profile
    [Documentation]    1	Provision 2 services (ont-port/ethernet interface)	Succeed
    ...    2	different vlan use same ID-profile	Succeed
    ...
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2209    @globalid=2344025    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1    @user_interface=CLI
    [Setup]      case setup
...
case setup
    ...
    log    STEP:2 different vlan use same ID-profile Succeed
