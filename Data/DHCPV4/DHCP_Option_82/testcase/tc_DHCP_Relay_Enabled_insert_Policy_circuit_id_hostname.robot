*** Settings ***
Documentation     DHCP Relay Enabled insert Policy circuit-id %hostname
Resource          ./base.robot
...
*** Test Cases ***
tc_DHCP_Relay_Enabled_insert_Policy_circuit_id_hostname
    [Documentation]    1	Force an access subscriber(ethernet interface) to obtain IP address via DHCP and capture conversation forwarded from the SUT to the server. Uplink trunk is LAG interface. -> Opt82 is inserted as circuit-id hostname.	All Step action expected Results must be correct.The name of the host. This is generally 16 characters or less. Its length is defined on a per-platform basis.
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2133    @globalid=2343949    @subfeature=DHCP_Option_82    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1    @user_interface=CLI
    [Setup]      case setup
