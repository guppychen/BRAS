*** Settings ***
Documentation     match src-mac(only support untag packets) add single tag copy p-bit
Resource          ./base.robot
...
*** Test Cases ***
tc_match_src_mac_add_single_tag_copy_p_bit
    [Documentation]    1	configure svc with match src-mac add single tag copy p-bit	success
    ...
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-2055    @globalid=2334102    @subfeature=DHCP_Snoop_IPv4    @feature=DHCPV4    @eut=NGPON2-4    @priority=P1
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 configure svc with match src-mac add single tag copy p-bit success
