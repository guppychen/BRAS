*** Settings ***
Documentation     Subscriber Side Add-Tag Match All with Single Tagged Subscriber Traffic
Resource          ./base.robot
...
*** Test Cases ***
tc_Subscriber_Side_Add_Tag_Match_All_with_Single_Tagged_Subscriber_Traffic
    [Documentation]    1	Subscriber Side Add-Tag Match All with Single Tagged Subscriber Traffic: Create service with target tag-action on an access interface. The outermost VLAN is enabled for DHCP Relay. Force a Subscriber to obtain a dynamic address. Display DHCP Snoop table. -> The Subscriber obtains an address. The Subscriber address is listed as a DHCP snoop entry. Note: When testing ENET the traffic match criteria is the "match-all" by not inserting a VLAN or p-bit as a matching criteria.  When testing GPON/DSL the matching criteria includes an untagged-rule to match all untagged traffic and a tagged rule to match all tagged traffic.	All Step action expected Results must be correct
    [Tags]       @author=Ronnie_Yi     @TCID=AXOS_E72_PARENT-TC-729    @feature=DHCPV4    @globalid=2307073    @subfeature=DHCP_Snoop_IPv4    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-729 setup
    [Teardown]   AXOS_E72_PARENT-TC-729 teardown
    log    STEP:1 Subscriber Side Add-Tag Match All with Single Tagged Subscriber Traffic: Create service with target tag-action on an access interface. The outermost VLAN is enabled for DHCP Relay. Force a Subscriber to obtain a dynamic address. Display DHCP Snoop table. -> The Subscriber obtains an address. The Subscriber address is listed as a DHCP snoop entry. Note: When testing ENET the traffic match criteria is the "match-all" by not inserting a VLAN or p-bit as a matching criteria. When testing GPON/DSL the matching criteria includes an untagged-rule to match all untagged traffic and a tagged rule to match all tagged traffic. All Step action expected Results must be correct
...
*** Keywords ***
AXOS_E72_PARENT-TC-729 setup
    [Documentation]    case setup
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-729 setup
...
AXOS_E72_PARENT-TC-729 teardown
    [Documentation]    case teardown
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-729 teardown
    log    stop STC
