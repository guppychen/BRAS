...
tc_modify_the_class_map
    ...
    ...    7	modify the class map to match VLAN 11 instead of VLAN 10	successfully
    ...    8	repeat steps 5 to 6	the traffic can work with right s-tag and c-tag


    [Tags]     @tcid=AXOS_E72_PARENT-TC-4398      @subFeature=VLAN mapping: Change an S-Tag to another value      @globalid=2532660      @priority=P1      @eut=NGPON2-4    @eut=GPON-8r2    @eut=10GE-12          @user_interface=CLI
    [Setup]     case setup
    ...
    log    STEP:7 modify the class map to match VLAN 11 instead of VLAN 10 successfully
    ...
    create_dhcp_client    tg1    dclient    subscriber_p1    dcg    ${p_dhcp_client.mac}    ovlan=${new_match_vlan}
