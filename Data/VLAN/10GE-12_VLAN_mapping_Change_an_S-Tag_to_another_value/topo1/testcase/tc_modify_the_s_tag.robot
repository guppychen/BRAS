...
tc_modify_the_s_tag
    ...
    ...    7	modify the s-tag under uni ethernet port with a new VLAN	successfully
    ...    8	repeat steps 5 to 6	the traffic can work with right s-tag and c-tag


    [Tags]     @tcid=AXOS_E72_PARENT-TC-4400      @subFeature=VLAN mapping: Change an S-Tag to another value      @globalid=2532662      @priority=P1      @eut=NGPON2-4    @eut=GPON-8r2    @eut=10GE-12          @user_interface=CLI
    [Setup]     case setup
    ...
    log    STEP:7 modify the s-tag under uni ethernet port with a new VLAN successfully
    subscriber_point_remove_svc    subscriber_point1    ${p_match_vlan}    ${p_service_vlan}    cevlan=${p_translate_vlan}
    log    add new service
    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan}    ${new_s_tag_to_verify}    cevlan_action=translate-cevlan-tag
    ...    cevlan=${p_translate_vlan}
    ...
    ...    ovlan=${new_s_tag_to_verify}    ivlan=${p_translate_vlan}
    log   verify the service
    run_dhcp_and_check_traffic    dserver    dcg    service_p1    subscriber_p1    traffic_loss_rate=${p_traffic_loss_rate}
...
case setup
    ...
    &{dict_prov_service}    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan}    ${p_service_vlan}    cevlan_action=translate-cevlan-tag
    ...    cevlan=${p_translate_vlan}
...
case teardown
    ...
    subscriber_point_remove_svc    subscriber_point1    ${p_match_vlan}    ${new_s_tag_to_verify}    cevlan=${p_translate_vlan}
    run keyword and ignore error    subscriber_point_remove_svc    subscriber_point1    ${p_match_vlan}    ${p_service_vlan}    cevlan=${p_translate_vlan}
