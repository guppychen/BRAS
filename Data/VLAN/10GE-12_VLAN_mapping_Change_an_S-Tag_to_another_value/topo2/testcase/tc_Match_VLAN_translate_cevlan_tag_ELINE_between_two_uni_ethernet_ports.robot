...
*** Test Cases ***
tc_Match_VLAN_translate_cevlan_tag_ELINE_between_two_uni_ethernet_ports
    [Documentation]
    ...
    ...    3	apply the s-tag in ELINE mode and policy-map to the uni ethernet port1 and port2	successfully
    ...    4	send VLAN 10 traffic to uni ethernet port1 and port2	the two uni ports can receive the traffic from each other.


    [Tags]     @tcid=AXOS_E72_PARENT-TC-4402      @subFeature=VLAN mapping: Change an S-Tag to another value      @globalid=2532664      @priority=P1      @eut=NGPON2-4    @eut=GPON-8r2    @eut=10GE-12          @user_interface=CLI
    [Setup]     case setup
    ...
    log    STEP:3 apply the s-tag in ELINE mode and policy-map to the uni ethernet port1 and port2 successfully
