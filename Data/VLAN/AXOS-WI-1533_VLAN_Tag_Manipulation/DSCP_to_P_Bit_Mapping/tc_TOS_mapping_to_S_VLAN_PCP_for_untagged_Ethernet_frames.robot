*** Settings ***
Documentation     The untagged traffic can pass form Access interface to uplink interface. Only S-VLAN-tag is attached.
Resource          ./base.robot
...
*** Test Cases ***
tc_TOS_mapping_to_S_VLAN_PCP_for_untagged_Ethernet_frames
    [Documentation]    The untagged traffic can pass form Access interface to uplink interface. Only S-VLAN-tag is attached.
    [Tags]       @author=Wanlin Sun     @tcid=AXOS_E72_PARENT-TC-1252    @feature=VLAN    @subfeature=VLAN_Tag_Manipulation
    ...     @globalid=2318914    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-1252 setup
    [Teardown]   AXOS_E72_PARENT-TC-1252 teardown
    log    STEP:The untagged traffic can pass form Access interface to uplink interface. Only S-VLAN-tag is attached.
    ...
    ...    mac_src=${subscriber_mac1}    mac_dst=${service_mac}    rate_pps=${rate_pps1}    l3_protocol=ipv4    ip_precedence=4  #TOS_PARAM
    ...
    ${p1_cap}    generate_pcap_name    s_vlan_pcp_for_untagged_eth
...
*** Keywords ***
AXOS_E72_PARENT-TC-1252 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1252 setup
    log    create vlan
...
AXOS_E72_PARENT-TC-1252 teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1252 teardown
