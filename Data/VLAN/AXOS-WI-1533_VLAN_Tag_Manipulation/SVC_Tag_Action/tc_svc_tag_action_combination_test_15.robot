...
tc_svc_tag_action_combination_test_15
    ...
    ...    3. match rules: tagged: match vlan + p-bit
    ...    action: change-tag + set p-bit
    [Tags]    @author=Wanlin Sun    @tcid=AXOS_E72_PARENT-TC-1228    @feature=VLAN    @subfeature=VLAN_Tag_Manipulation    @globalid=2318881    @eut=NGPON2-4
    ...    @priority=P1
    [Setup]    AXOS_E72_PARENT-TC-1228 setup
    ...
    log    STEP:action: add2tags+set p-bit(use-p-bit)
    &{result1}    subscriber_point_add_svc    subscriber_point1    untagged    ${p_data_vlan1}    add-cevlan-tag    ${p_data_cvlan1}
    ...    set-stag-pcp=${stag_pcp}    cfg_prefix=v1
    prov_class_map    eutA    &{result1}[classmap]    ethernet    flow    1    1
    ...    src-mac=${src_mac}
    prov_class_map    eutA    &{result1}[classmap]    ethernet    flow    1    2
    ...    src-oui=${src_oui}
    prov_class_map    eutA    &{result1}[classmap]    ethernet    flow    1    3
    ...    ethertype=${ethertype_hex}
    log    STEP:2. match rules: tagged: only match vlan, p-bit any
    log    STEP:action: add-and-change+copy p-bit
    &{result2}    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan1}    ${p_data_vlan2}    cevlan_action=translate-cevlan-tag    cevlan=${p_ce_add_vlan1}
    ...    set-stag-pcp=promote    cfg_prefix=v2
    prov_class_map    eutA    &{result2}[classmap]    ethernet    flow    1    1
    ...    vlan=${p_match_vlan1}
    log    STEP:3. match rules: tagged: match vlan + p-bit
    log    STEP:action: change-tag + set p-bit
    &{result3}    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan2}    ${p_data_vlan3}    cevlan_action=remove-cevlan    set-stag-pcp=${stag_pcp}
    ...    cfg_prefix=v3
    prov_class_map    eutA    &{result3}[classmap]    ethernet    flow    1    1
    ...    vlan=${p_match_vlan2}    pcp=${cetag_pcp}
    ...
    Tg Create Double Tagged Stream On Port    tg1    ds_match_mac    subscriber_p1    service_p1    ${p_data_cvlan1}    0
    ...    ${p_data_vlan1}    0    ${service_mac}    ${subscriber_mac1}    frame_size=512    length_mode=fixed
    ...    rate_pps=${rate_pps1}    ether_type=${ether_type_ipv6}    l3_protocol=ipv6    ipv6_dst_addr=${subscriber_ip_v6}    ipv6_src_addr=${service_ip_v6}
    Tg Create Double Tagged Stream On Port    tg1    ds_match_oui    subscriber_p1    service_p1    ${p_data_cvlan1}    0
    ...    ${p_data_vlan1}    0    ${service_mac}    ${subscriber_mac2}    frame_size=512    length_mode=fixed
    ...    rate_pps=${rate_pps1}    ether_type=${ether_type_ipv6}    l3_protocol=ipv6    ipv6_dst_addr=${subscriber_ip_v6}    ipv6_src_addr=${service_ip_v6}
    Tg Create Double Tagged Stream On Port    tg1    ds_match_etype    subscriber_p1    service_p1    ${p_data_cvlan1}    0
    ...    ${p_data_vlan1}    0    ${service_mac}    ${subscriber_mac3}    frame_size=512    length_mode=fixed
    ...    rate_pps=${rate_pps1}    ether_type=${ether_type}    l3_protocol=ipv4    ip_dst_addr=${subscriber_ip}    ip_src_addr=${service_ip}
    Tg Create Single Tagged Stream On Port    tg1    us_match2    service_p1    subscriber_p1    ${p_match_vlan1}    ${cetag_pcp}
    ...    frame_size=512    length_mode=fixed    mac_src=${subscriber_mac4}    mac_dst=${service_mac2}    rate_pps=${rate_pps1}    l3_protocol=ipv4
    ...    ip_dst_addr=${service_ip}    ip_src_addr=${subscriber_ip}
    Tg Create Double Tagged Stream On Port    tg1    ds_match2    subscriber_p1    service_p1    ${p_ce_add_vlan1}    0
    ...    ${p_data_vlan2}    0    ${service_mac2}    ${subscriber_mac4}    frame_size=512    length_mode=fixed
    ...    rate_pps=${rate_pps1}    ether_type=${ether_type}    l3_protocol=ipv4    ip_dst_addr=${subscriber_ip}    ip_src_addr=${service_ip}
    Tg Create Single Tagged Stream On Port    tg1    us_match3    service_p1    subscriber_p1    ${p_match_vlan2}    ${cetag_pcp}
    ...    frame_size=512    length_mode=fixed    mac_src=${subscriber_mac5}    mac_dst=${service_mac3}    rate_pps=${rate_pps1}    l3_protocol=ipv4
    ...    ip_dst_addr=${service_ip}    ip_src_addr=${subscriber_ip}
    Tg Create Single Tagged Stream On Port    tg1    ds_match3    subscriber_p1    service_p1    ${p_data_vlan3}    0
    ...
    ${p1_cap}    generate_pcap_name    combination_test_15
    ...
    [Teardown]    AXOS_E72_PARENT-TC-1228 teardown

*** Keywords ***
AXOS_E72_PARENT-TC-1228 setup
    log    Enter AXOS_E72_PARENT-TC-1228 setup
...
AXOS_E72_PARENT-TC-1228 teardown
    log    Enter AXOS_E72_PARENT-TC-1228 teardown
    ...
    subscriber_point_remove_svc    subscriber_point1    ${p_match_vlan2}    ${p_data_vlan3}    cfg_prefix=v3
