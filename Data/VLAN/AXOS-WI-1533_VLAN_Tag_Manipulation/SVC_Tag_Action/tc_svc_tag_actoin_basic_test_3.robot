...
...    Action:
...    add2tag+set p-bit(use-p-bit + use-inner-p-bit)
Resource          ./base.robot
...
*** Test Cases ***
tc_svc_tag_actoin_basic_test_3
    [Documentation]    match rule:
    ...    match rule:
    ...    untagged: src-mac any, src-mac-mask any,ethertype any.
    ...    Action:
    ...    add2tag+set p-bit(use-p-bit+ use-inner-p-bit)
    [Tags]       @author=Wanlin Sun     @tcid=AXOS_E72_PARENT-TC-1188    @feature=VLAN    @subfeature=VLAN_Tag_Manipulation
    ...     @globalid=2318841    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-1188 setup
    [Teardown]   AXOS_E72_PARENT-TC-1188 teardown
    log    STEP:Action:add2tag+set p-bit(use-p-bit+ use-inner-p-bit)
...
#    prov_class_map     eutA    &{result}[classmap]    ethernet    flow    1    3    ethertype=${ethertype}
    ...
    ${p1_cap}    generate_pcap_name    basic_test_3
...
*** Keywords ***
AXOS_E72_PARENT-TC-1188 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1188 setup
    log    create vlan
...
AXOS_E72_PARENT-TC-1188 teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1188 teardown
