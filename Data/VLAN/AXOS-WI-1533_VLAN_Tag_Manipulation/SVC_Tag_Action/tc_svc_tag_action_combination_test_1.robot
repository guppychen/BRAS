...
...    1. match rules: untagged src-mac any+src-mac-mask+ethertype any;
...    action: add-tag + set pbit
...    2. match rules: tagged only match vlan + pbit any
...    action: add-tag + set pbit
...    3. match rules: tagged only match vlan + pbit any
...    action: add-tag +pbit copy
Resource          ./base.robot
...
*** Test Cases ***
tc_svc_tag_action_combination_test_1
    [Documentation]    combination of the 3 actions. The 3 services use different s-tag:
    ...    1. match rules: untagged src-mac any+src-mac-mask+ethertype any;
    ...    action: add-tag + set pbit
    ...    2. match rules: tagged only match vlan + pbit any
    ...
    ...    3. match rules: tagged only match vlan + pbit any
    ...    action: add-tag +pbit copy
    [Tags]       @author=Wanlin Sun     @tcid=AXOS_E72_PARENT-TC-1214    @feature=VLAN    @subfeature=VLAN_Tag_Manipulation
    ...     @globalid=2318867    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-1214 setup
    [Teardown]   AXOS_E72_PARENT-TC-1214 teardown
    log    STEP:combination of the 3 actions. The 3 services use different s-tag:

    log    STEP:1. match rules: untagged src-mac any+src-mac-mask+ethertype any;
    log    STEP:action: add-tag + set pbit
    &{result1}    subscriber_point_add_svc    subscriber_point1    untagged    ${p_data_vlan1}    set-stag-pcp=${stag_pcp}    cfg_prefix=v1
    prov_class_map     eutA    &{result1}[classmap]    ethernet    flow    1    1    untagged=${EMPTY}

    log    STEP:2. match rules: tagged only match vlan + pbit any
    log    STEP:action: add-tag + set pbit
    &{result2}    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan1}    ${p_data_vlan2}    set-stag-pcp=${stag_pcp}    cfg_prefix=v2
    prov_class_map     eutA    &{result2}[classmap]    ethernet    flow    1    1    vlan=${p_match_vlan1}

    log    STEP:3. match rules: tagged only match vlan + pbit any
    log    STEP:action: add-tag +pbit copy
    &{result3}    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan2}    ${p_data_vlan3}    set-stag-pcp=promote    cfg_prefix=v3
    prov_class_map     eutA    &{result3}[classmap]    ethernet    flow    1    1    vlan=${p_match_vlan2}
    ...
    ...    l3_protocol=ipv6    ipv6_dst_addr=${service_ip_v6_2}    ipv6_src_addr=${subscriber_ip_v6_2}
    ...
    Tg Create Single Tagged Stream On Port    tg1    ds_match_mac    subscriber_p1    service_p1    ${p_data_vlan1}    0
    ...
    Tg Create Single Tagged Stream On Port    tg1    ds_match_oui    subscriber_p1    service_p1    ${p_data_vlan1}    0
    ...    ${service_mac}    ${subscriber_mac2}    frame_size=512    length_mode=fixed    rate_pps=${rate_pps1}    ether_type=${ether_type_ipv6}
    ...    l3_protocol=ipv6    ipv6_dst_addr=${subscriber_ip_v6_2}    ipv6_src_addr=${service_ip_v6_2}
    Tg Create Single Tagged Stream On Port    tg1    ds_match_etype    subscriber_p1    service_p1    ${p_data_vlan1}    0
    ...
    ...    l3_protocol=ipv4    ip_dst_addr=${service_ip_2}    ip_src_addr=${subscriber_ip_2}
    ...
    ...    l3_protocol=ipv4    ip_dst_addr=${subscriber_ip_2}    ip_src_addr=${service_ip_2}
    ...
    ...    l3_protocol=ipv4    ip_dst_addr=${service_ip_3}    ip_src_addr=${subscriber_ip_3}

    Tg Create Double Tagged Stream On Port    tg1    ds_match3    subscriber_p1    service_p1    ${p_match_vlan2}    0    ${p_data_vlan3}    0
    ...    ${service_mac3}    ${subscriber_mac5}    frame_size=512    length_mode=fixed    rate_pps=${rate_pps1}    ether_type=${ether_type}
    ...    l3_protocol=ipv4    ip_dst_addr=${subscriber_ip_3}    ip_src_addr=${service_ip_3}
    ...
    Wsk Verify Vlan By Index    ${p_match_vlan2}  2
    Tg Clear Traffic Stats    tg1


*** Keywords ***
AXOS_E72_PARENT-TC-1214 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1214 setup
...
AXOS_E72_PARENT-TC-1214 teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1214 teardown
#    Tg Save Config Into File      tg1    /tmp/philar_1.xml
#    sleep      33
    Run Keyword And Ignore Error    Tg Stop All Traffic    tg1
    Run Keyword And Ignore Error    Tg Delete All Traffic    tg1

    subscriber_point_remove_svc    subscriber_point1    untagged    ${p_data_vlan1}    cfg_prefix=v1
