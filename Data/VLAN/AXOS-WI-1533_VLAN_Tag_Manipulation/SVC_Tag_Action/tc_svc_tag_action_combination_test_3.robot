...
...    action: add2tag + set bpit (use -p-bit)
...    2. match rules: tagged only match vlan + pbit any
...    action: add-tag + set pbit
...    3. match rules: tagged only match vlan+p-bit any
...    action: change-tag +p-bit copy
Resource          ./base.robot
...
*** Test Cases ***
tc_svc_tag_action_combination_test_3
    [Documentation]    combination of the 3 actions. The 3 services use different s-tag:
    ...    1. match rules: untagged src-mac any+src-mac-mask+ethertype any;
    ...    action: add2tag + set bpit (use -p-bit)
    ...    2. match rules: tagged only match vlan + pbit any
    ...
    ...    3. match rules: tagged only match vlan+p-bit any
    ...    action: change-tag +p-bit copy
    [Tags]       @author=Wanlin Sun     @tcid=AXOS_E72_PARENT-TC-1216    @feature=VLAN    @subfeature=VLAN_Tag_Manipulation
    ...     @globalid=2318869    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-1216 setup
    [Teardown]   AXOS_E72_PARENT-TC-1216 teardown
    ...
    log    STEP:action: add2tag + set bpit (use -p-bit)
    ...
    log    STEP:3. match rules: tagged only match vlan+p-bit any
    log    STEP:action: change-tag +p-bit copy
    &{result3}    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan2}    ${p_data_vlan3}    cevlan_action=remove-cevlan
    ...    set-stag-pcp=promote    cfg_prefix=v3
    ...
    ${p1_cap}    generate_pcap_name    combination_test_3
...
*** Keywords ***
AXOS_E72_PARENT-TC-1216 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1216 setup
...
AXOS_E72_PARENT-TC-1216 teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1216 teardown
    Tg Save Config Into File       tg1       /tmp/tc_svc_tag_action_combination_test_3.xml
    sleep    33
