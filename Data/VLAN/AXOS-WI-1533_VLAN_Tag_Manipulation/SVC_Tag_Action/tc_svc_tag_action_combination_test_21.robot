*** Settings ***
Documentation     combination of the 2 actions. The 2 services use different s-tag:
...    1. match rules:src-mac any,src-mac-mask any,ethertype any;
...
*** Test Cases ***
tc_svc_tag_action_combination_test_21
    [Documentation]    combination of the 2 actions. The 2 services use different s-tag:
    ...    1. match rules:src-mac any,src-mac-mask any,ethertype any;
    ...    action: add2tags+set p-bit(use-p-bit);
    ...    2. match rules: tagged: match vlan + p-bit;
    ...    action: add-and-change+copy p-bit;
    [Tags]       @author=Wanlin Sun     @tcid=AXOS_E72_PARENT-TC-1234    @feature=VLAN    @subfeature=VLAN_Tag_Manipulation
    ...     @globalid=2318887    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-1234 setup
    [Teardown]   AXOS_E72_PARENT-TC-1234 teardown
    log    STEP:combination of the 2 actions. The 2 services use different s-tag:

    log    STEP:1. match rules:src-mac any,src-mac-mask any,ethertype any;
    ...
    ${p1_cap}    generate_pcap_name    combination_test_21
...
*** Keywords ***
AXOS_E72_PARENT-TC-1234 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1234 setup
    prov_vlan    eutA    ${p_data_vlan1}
    prov_vlan    eutA    ${p_data_vlan2}

    log    service_point_add_vlan for uplink service
    service_point_add_vlan    service_point_list1    ${p_data_vlan1},${p_data_vlan2}

AXOS_E72_PARENT-TC-1234 teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1234 teardown
    ...
    service_point_remove_vlan    service_point_list1    ${p_data_vlan1},${p_data_vlan2}
