...
...    3. match rules: tagged: only match p-bit, ignore vlan;
...    action: add-tag + set p-bit;
Resource          ./base.robot
...
*** Test Cases ***
tc_svc_tag_action_combination_test_19
    [Documentation]    combination of the 3 actions. The 3 services use different s-tag:
    ...    1. match rules: untagged: src-mac any, src-mac-mask any,ethertype any;
    ...    action: add-tag + set p-bit;
    ...    2. match rules: tagged: match vlan + p-bit
    ...
    ...    3. match rules: tagged: only match p-bit, ignore vlan;
    ...    action: add-tag + set p-bit;
    [Tags]       @author=Wanlin Sun     @tcid=AXOS_E72_PARENT-TC-1232    @feature=VLAN    @subfeature=VLAN_Tag_Manipulation
    ...     @globalid=2318885    @eut=NGPON2-4    @priority=P1
    [Setup]      AXOS_E72_PARENT-TC-1232 setup
    [Teardown]   AXOS_E72_PARENT-TC-1232 teardown
    ...
    log    STEP:3. match rules: tagged: only match p-bit, ignore vlan;
    log    STEP:action: add-tag + set p-bit;
    &{result3}    subscriber_point_add_svc    subscriber_point1    ${p_match_vlan2}    ${p_data_vlan3}     set-stag-pcp=${stag_pcp}    cfg_prefix=v3
    prov_class_map     eutA    &{result3}[classmap]    ethernet    flow    1    1    pcp=${cetag_pcp}
    ...
    ${p1_cap}    generate_pcap_name    combination_test_19
...
*** Keywords ***
AXOS_E72_PARENT-TC-1232 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1232 setup
...
AXOS_E72_PARENT-TC-1232 teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-1232 teardown
