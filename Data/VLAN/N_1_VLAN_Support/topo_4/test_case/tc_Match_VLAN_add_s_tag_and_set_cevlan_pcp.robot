...
*** Test Cases ***
tc_Match_VLAN_add_s_tag_and_set_cevlan_pcp
    [Documentation]    1	create a class-map to match VLAN 10 in flow 1
    ...    2	create a policy-map to bind the class-map and set cevlan pcp
    ...
    ...    6	send VLAN 10 upstream traffic to ont1 with SMAC 000001000001 DMAC 000002000002;
    [Tags]    @author=AnneLI    @globalid=2298733    @tcid=AXOS_E72_PARENT-TC-623     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    ...
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p3    eth.src==${mac2} and eth.dst==${mac1} and vlan.id==${match_vlan}     ${error_rate}
    log    step6: send VLAN 10 upstream upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
     Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2      vlan_id=${match_vlan}    vlan_user_priority=0    frame_size=512
    ...
    show_interface_counters    eutA    ${interface_type2}    ${service_model.subscriber_point2.name}
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${match_vlan} and vlan.priority==${cevlan_pcp}   ${error_rate}
...
setup
    ...
    log    step1:create class-map to match vlan ${match_vlan} in flow 1
    log    step2: create a policy-map to bind the class-map and set cevlan pcp ${cevlan_pcp}
    log    step4:apply the s-tag and policy-map to the port of ont1 and ont2
    subscriber_point_add_svc    subscriber_point1      ${match_vlan}    ${service_vlan}         set-cevlan-pcp= ${cevlan_pcp}   cfg_prefix=auto1
    subscriber_point_add_svc    subscriber_point2      ${match_vlan}    ${service_vlan}     set-cevlan-pcp= ${cevlan_pcp}    cfg_prefix=auto2
