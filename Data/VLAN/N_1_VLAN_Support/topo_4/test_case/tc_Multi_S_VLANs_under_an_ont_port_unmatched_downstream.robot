...
*** Test Cases ***
tc_Multi_S_VLANs_under_an_ont_port_unmatched_downstream
    [Documentation]    1 create class-map1 to match src-mac A and class-map2 to match src-mac B
    ...    2 create policy-map1 to bind class-map1 add-ctag 10 and policy-map2 to bind class-map2 add-ctag20
    ...
    ...    5 send double-tagged S=100 C=30 and S=200 C=40 downstream traffics to eth-port1
    [Tags]    @author=AnneLI    @globalid=2298757    @tcid=AXOS_E72_PARENT-TC-647     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send double-tagged-tagged s=${service_vlan} c=${vlan_2} and s=${service_vlan_2} c=${vlan_4} downstream traffic to ${interface_type1} ${service_model.service_point1.member.interface1} ;
...
setup
    ...
    log    step3:add eth-port1 to s-tag1= ${service_vlan} and s-tag2= ${service_vlan_2} with transport-service-profile
