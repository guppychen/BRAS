...
*** Test Cases ***
tc_Multi_flows_in_one_class_map_matched_downstream
    ...
    ...    5	send double-tagged S=100 C=10 and S=100 C=20 downstream traffics to eth-port1	both clients can receive the traffics with VLAN=10 and VLAN=20

    [Tags]    @author=AnneLI    @globalid=2298760    @tcid=AXOS_E72_PARENT-TC-650     @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step5: send double-tagged S=${service_vlan} C=${match_vlan_2} and S=${service_vlan} C=${match_vlan_4} downstream traffics to ${interface_type2} ${service_model.subscriber_point1.name} ;
    ...
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p3    eth.src==${mac2} and eth.dst==${mac1} and vlan.id==${match_vlan_2}    ${error_rate}
