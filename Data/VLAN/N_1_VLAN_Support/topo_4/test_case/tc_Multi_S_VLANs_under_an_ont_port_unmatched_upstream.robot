...
*** Test Cases ***
tc_Multi_S_VLANs_under_an_ont_port_unmatched_upstream
    ...
    ...    5 send single-tagged=30 and 40 upstream traffic to ont1
    [Tags]    @author=AnneLI    @globalid=2298755    @tcid=AXOS_E72_PARENT-TC-645     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send single-tagged=${vlan_2} and ${vlan_4} upstream traffic to ont1 ${interface_type2} ${service_model.subscriber_point1.name} ;
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=${vlan_2}    vlan_user_priority=0
    ...
    Tg Create single Tagged Stream On Port    tg1    raw_upstream2    p1    p2    vlan_id=${vlan_4}    vlan_user_priority=0
