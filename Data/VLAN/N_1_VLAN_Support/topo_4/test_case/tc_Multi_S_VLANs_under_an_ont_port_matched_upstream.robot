...
*** Test Cases ***
tc_Multi_S_VLANs_under_an_ont_port_matched_upstream
    ...
    ...    5 send single-tagged=10 and 20 upstream traffic to ont1
    [Tags]    @author=AnneLI    @globalid=2298754    @tcid=AXOS_E72_PARENT-TC-644     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send single-tagged vlan ${match_vlan_2} and ${match_vlan_4} uptream traffic to ${interface_type2} ${service_model.subscriber_point1.name} ;
    ...
    ...    ip_dst_addr=${ip4}    l4_protocol=udp    udp_dst_port=${udp_port3}    udp_src_port=${udp_port4}    rate_bps=${rate_bps}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${match_vlan_2} and vlan.id==${service_vlan}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream2    p1    eth.src==${mac3} and eth.dst==${mac4} and vlan.id==${match_vlan_4} and vlan.id==${service_vlan_2}    ${error_rate}
    [Teardown]    teardown
...
setup
    ...
    log    step3:add eth-port1 to s-tag1=100 and s-tag2=200 with transport-service-profile
