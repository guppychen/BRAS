...
*** Test Cases ***
tc_Match_any_add_s_tag_upstream_is_priority_tag
    ...
    ...    6 send priority-tagged upstream traffic to ont1 with SMAC 000001000001 DMAC 000002000002;
    [Tags]    @author=AnneLI    @globalid=2298737    @tcid=AXOS_E72_PARENT-TC-627    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    ...
    log    step6: send priority-tagged upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    ...
    Tg Start All Traffic    tg1
    log    send traffic,wait ${send_traffic_to_learn_mac_time}s
    sleep    ${send_traffic_time}
    Tg Stop All Traffic    tg1
    sleep    ${stop_traffic_time}
    log    clear interface counters before start traffic
