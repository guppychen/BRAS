...
*** Test Cases ***
tc_Multi_rules_in_one_flow_matched_downstream
  [Documentation]    1	create class-map with flow1 to match vlan 10 in rule1 and match vlan 20 in rule2
    ...
    ...    5	send double-tagged S=100 C=10 and S=100 C=20 downstream traffics to eth-port1
    [Tags]    @author=AnneLI    @globalid=2298764    @tcid=AXOS_E72_PARENT-TC-654     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
     log    step5: send double-tagged S=${service_vlan} C=${match_vlan_2} and S=${service_vlan} C=${match_vlan_4} downstream traffics to ${interface_type1} ${service_model.service_point1.member.interface1};
    ...
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p2    eth.src==${mac4} and eth.dst==${mac3} and vlan.id==${match_vlan_4}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream2    p3    eth.src==${mac2} and eth.dst==${mac1} and vlan.id==${match_vlan_2}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream2    p3    eth.src==${mac4} and eth.dst==${mac3} and vlan.id==${match_vlan_4}    ${error_rate}
    [Teardown]    teardown
*** Keywords ***
setup
   [Documentation]    setup
