...
tc_Match_src_mac_add_s_tag_and_set_stag_pcp
    ...
    ...    2	create a policy-map to bind the class-map and set s-tag PCP
    ...    3	add eth-port1 to s-tag with transport-service-profile
    ...    4	apply the s-tag and policy-map to the port of ont1 and ont2
    ...    5	send single-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 000001000001;
    ...    6	send untagged upstream traffic to ont1 with SMAC 000001000001 DMAC 000002000002;


    [Tags]    @author=AnneLI    @globalid=2298732    @tcid=AXOS_E72_PARENT-TC-622     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send single-tagged downstream traffic to ${interface_type1} ${service_model.service_point1.member.interface1} with SMAC ${mac2} DMAC ${mac1};
    ...
    show_interface_counters    eutA    ${interface_type2}    ${service_model.subscriber_point2.name}
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.priority==${stag_pcp}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p2    eth.src==${mac2} and eth.dst==${mac1}     ${error_rate}
...
setup
    ...
    log    step1:create class-map to match ${mac1}in flow ${flow_index} }
    prov_class_map    eutA    ${class_map_name}    ${class_map_type}    flow    ${flow_index}    ${rule_index}
    ...    src-mac=${mac1}
    log    step2:create a policy-map to bind the class-map and set s-tag PCP
