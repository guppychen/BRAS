...
*** Test Cases ***
tc_Match_any_add_s_tag_upstream_is_multi_types
    ...
    ...    5 send multi downstream traffics at the same time; traffic1: double-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 000001000001; traffic2: single-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 000001000001;
    ...    6 send multi upstream traffics at the same time; traffic1: ethertype=0x9100; traffic2: PCP=1; traffic3: priority-tagged=1; traffic4: untagged
    [Tags]    @author=AnneLI    @globalid=2298739    @tcid=AXOS_E72_PARENT-TC-629    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step:5 send multi downstream traffics at the same time; traffic1: double-tagged downstream traffic to eth-port1 with SMAC ${mac2} DMAC ${mac1};traffic2: single-tagged downstream traffic to eth-port1 with SMAC ${mac2} DMAC ${mac1};
    ...
    ...    rate_bps=${rate_bps}
    Tg Create single Tagged Stream On Port    tg1    raw_downstream2    p2    p1    vlan_id=${service_vlan}    vlan_user_priority=5
    ...    frame_size=512    length_mode=fixed    mac_src=${mac2}    mac_dst=${mac1}    l3_protocol=ipv4    ip_src_addr=${ip4}
    ...    ip_dst_addr=${ip3}    l4_protocol=udp    udp_dst_port=${udp_port4}    udp_src_port=${udp_port3}    rate_bps=${rate_bps}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p2    eth.src==${mac4} and eth.dst==${mac3} and vlan.id==${c_vlan}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p3    eth.src==${mac4} and eth.dst==${mac3} and vlan.id==${c_vlan}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream2    p2    eth.src==${mac2} and eth.dst==${mac1} and ip.src==${ip4}   ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream2    p3    eth.src==${mac2} and eth.dst==${mac1} and ip.src==${ip4}     ${error_rate}
    log    step6: send multi upstream traffics at the same time; traffic1: ethertype=0x9100; traffic2: PCP=1; traffic3: priority-tagged=1; traffic4: untagged
    Tg Create single Tagged Stream On Port    tg1    raw_upstream2    p1    p2    vlan_id=${vlan_2}    vlan_user_priority=${pcp_1}
    ...
    Tg Create single Tagged Stream On Port    tg1    raw_upstream3    p1    p2    vlan_id=0    vlan_user_priority=${pcp_1}
    ...
    TG Create Untagged Stream On Port    tg1    raw_upstream4    p1    p2    frame_size=512    length_mode=fixed
    ...    mac_src=${mac1}    mac_dst=${mac2}    l3_protocol=ipv4    ip_src_addr=${ip3}    ip_dst_addr=${ip4}    l4_protocol=udp
    ...    udp_dst_port=${udp_port3}    udp_src_port=${udp_port4}    rate_bps=${rate_bps}
    ...
    show_interface_counters    eutA    ${interface_type2}    ${service_model.subscriber_point2.name}
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream2    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${vlan_2} and vlan.id==${service_vlan}      ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream3    p1    eth.src==${mac3} and eth.dst==${mac4} and vlan.id==0 and vlan.id==${service_vlan}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream4    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and ip.src==${ip3}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p2     eth.src==${mac4} and eth.dst==${mac3} and vlan.id==${c_vlan}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream2    p2     eth.src==${mac2} and eth.dst==${mac1} and ip.src==${ip4}   ${error_rate}
...
setup
    ...
    log    step3:add eth-port1 to s-tag= ${service_vlan} with transport-service-profile
