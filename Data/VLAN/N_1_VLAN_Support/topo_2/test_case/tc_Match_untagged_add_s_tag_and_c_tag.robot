...
*** Test Cases ***
tc_Match_untagged_add_s_tag_and_c_tag
    [Documentation]    1	create a class-map to match untagged in flow 1
    ...    2	create a policy-map to bind the class-map and add c-tag
    ...
    [Tags]    @author=AnneLI     @globalid=2298720       @tcid=AXOS_E72_PARENT-TC-610     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    ...
    log    step6: send double-tagged downstream traffic to eth-port ${service_model.service_point1.member.interface1} with SMAC ${mac2} DMAC ${mac1};
...
setup
    ...
    subscriber_point_add_svc    subscriber_point1    untagged    ${service_vlan}    ctag_action=add-cevlan-tag    cvlan=${c_vlan}
...
teardown
    ...
    subscriber_point_remove_svc    subscriber_point1    untagged    ${service_vlan}    ${c_vlan}
