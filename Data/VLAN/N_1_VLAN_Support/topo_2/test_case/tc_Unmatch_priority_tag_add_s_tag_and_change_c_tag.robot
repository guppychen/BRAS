...
*** Test Cases ***
tc_Unmatch_priority_tag_add_s_tag_and_change_c_tag
    [Documentation]    1	create a class-map to match priority-tag 6 in flow 1
    ...
    ...    5	send priority-tag 1 upstream traffic to ont1
    [Tags]    @author=AnneLI    @globalid=2298750    @tcid=AXOS_E72_PARENT-TC-640     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send upstream traffic with priority-tagged ${pcp_1} to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=0    vlan_user_priority=${pcp_1}
...
setup
    ...
    log    step1:create a class-map to match priority-tagged ${match_pcp_1} in flow ${flow_index}
    prov_class_map    eutA    ${class_map_name}    ${class_map_type}    flow    ${flow_index}    ${rule_index}
    ...    priority-tagged pcp=${match_pcp_1}
