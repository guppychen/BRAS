...
*** Test Cases ***
tc_Unmatch_ethertype_add_s_tag
    ...
    ...    5	send upstream traffic with ethertype=ipv4 to ont1	no eth-port can pass the upstream traffic
    [Tags]    @author=AnneLI    @globalid=2298753    @tcid=AXOS_E72_PARENT-TC-643     @eut=NGPON2-4    @priority=P2   @jira=PREM-22090
    [Setup]    setup
    log    step6: send upstream traffic with ethertype=ipv4 to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2}
