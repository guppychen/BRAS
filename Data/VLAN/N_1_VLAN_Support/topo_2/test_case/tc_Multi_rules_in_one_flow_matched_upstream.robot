...
tc_Multi_rules_in_one_flow_matched_upstream
   [Documentation]    1	create class-map with flow1 to match vlan 10 in rule1 and match vlan 20 in rule2
    ...
    [Tags]    @author=AnneLI    @globalid=2298762    @tcid=AXOS_E72_PARENT-TC-652     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send single-tagged VLAN ${match_vlan_2} and VLAN ${match_vlan_4} upstream traffics to ont1to ${interface_type2} ${service_model.subscriber_point1.name} ;
    ...
    verify_traffic_loss_within_with_filter    tg1    raw_upstream2    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${match_vlan_4}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream2    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${match_vlan_4}    ${error_rate}
    [Teardown]    teardown
*** Keywords ***
setup
    [Documentation]    setup
