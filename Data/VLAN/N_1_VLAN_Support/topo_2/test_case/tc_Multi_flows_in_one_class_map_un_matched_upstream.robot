...
*** Test Cases ***
tc_Multi_flows_in_one_class_map_un_matched_upstream
    [Documentation]
    ...    1	create class-map with flow1 to match vlan 10 and flow2 to match vlan 20
    ...
    ...    5	send single-tagged VLAN=30 and VLAN=40 upstream traffics to ont1	no eth-port can pass the traffics
    [Tags]    @author=AnneLI    @globalid=2298759    @tcid=AXOS_E72_PARENT-TC-649     @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step5: send single-tagged VLAN ${match_vlan} and VLAN ${match_vlan_3} upstream traffics to ont1to ${interface_type2} ${service_model.subscriber_point1.name} ;
    ...
    Tg Create single Tagged Stream On Port    tg1    raw_upstream2    p1    p2    vlan_id=${match_vlan_3}    vlan_user_priority=0
