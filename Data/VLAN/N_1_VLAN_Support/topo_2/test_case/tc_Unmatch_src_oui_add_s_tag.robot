...
*** Test Cases ***
tc_Unmatch_src_oui_add_s_tag
    [Documentation]    1	create a class-map to match src-oui in flow 1
    ...
    ...    5	send upstream traffic that does not match src-oui to ont1
    [Tags]    @author=AnneLI    @globalid=2298752     @tcid=AXOS_E72_PARENT-TC-642     @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step5: send upstream traffic to ont-port ${service_model.subscriber_point1.name} with SMAC ${mac3} DMAC ${mac4};
...
setup
   [Documentation]    setup
    ...
    log    step1:create a class-map to match oui ${mac_oui} in flow ${flow_index}
    prov_class_map    eutA    ${class_map_name}    ${class_map_type}    flow    ${flow_index}    ${rule_index}    src-oui=${mac_oui}
