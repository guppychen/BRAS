...
*** Test Cases ***
tc_Match_PCP_change_c_tag
    [Documentation]    1	create a class-map to match PCP 2 in flow 1
    ...    2	create a policy-map to bind the class-map and remove-cevlan
    ...
    ...    5	send upstream trafficwith PCP 2 to ont1 with SMAC 000001000001 DMAC 000002000002;	both eth-port1 and eth-port2 can pass the upstream traffic
    ...    6	send single-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 000001000001;
    [Tags]       @author=Anne Li    @globalid=2298725    @tcid=AXOS_E72_PARENT-TC-615     @eut=NGPON2-4    @priority=P2    @jira=PXA-2130
    [Setup]       setup
    log    step5: send vlan ${match_vlan_3} PCP ${match_pcp_2} upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=${match_vlan_3}    vlan_user_priority=${match_pcp_2}
    ...
    Tg Create single Tagged Stream On Port    tg1    raw_downstream1    p1    p2    vlan_id=${service_vlan}    vlan_user_priority=0
    ...    frame_size=512    length_mode=fixed    mac_src=${mac2}    mac_dst=${mac1}    l3_protocol=ipv4    ip_src_addr=${ip2}
    ...    ip_dst_addr=${ip1}    l4_protocol=udp    udp_dst_port=${udp_port2}    udp_src_port=${udp_port1}    rate_bps=${rate_bps}
...
setup
   [Documentation]    setup
    ...
    log    step1: create a class-map to match PCP ${match_pcp_2} in flow ${flow_index}
    prov_class_map    eutA    ${class_map_name}    ${class_map_type}    flow    ${flow_index}    ${rule_index}    pcp=${match_pcp_2}
    log    step2: create a policy-map to bind the class-map
    prov_policy_map    eutA    ${policy_map_name}    class-map-ethernet    ${class_map_name}    sub_view_type=flow    sub_view_value=${flow_index}    remove-cevlan=${EMPTY}
