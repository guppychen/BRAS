...
*** Test Cases ***
tc_Match_priority_tag_add_s_tag
    ...
    ...    5	send upstream traffic with priority-tagged to ont1 with SMAC 000001000001 DMAC 000002000002;	both eth-port1 and eth-port2 can pass the upstream traffic
    ...    6	send double-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 000001000001;	client1 can receive the downstream traffic; only eth-port1 can pass the upstream traffi

    [Tags]    @author=AnneLI    @globalid=2298727    @tcid=AXOS_E72_PARENT-TC-617     @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==0    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==0    ${error_rate}
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream1    p2    p1    vlan_id=0    vlan_user_priority=0
