...
tc_Unmatch_PCP_add_s_tag
    ...
    [Tags]       @author=Anne Li    @globalid=2298746    @tcid=AXOS_E72_PARENT-TC-636     @eut=NGPON2-4    @priority=P2
    [Setup]       setup
    log    step5: send vlan ${match_vlan_3} PCP ${pcp_2} upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=${match_vlan_3}    vlan_user_priority=${pcp_2}
