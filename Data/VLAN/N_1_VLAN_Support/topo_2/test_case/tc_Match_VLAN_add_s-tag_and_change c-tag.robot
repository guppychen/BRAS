...
*** Test Cases ***
tc_Match_VLAN_add_s_tag_and_change_c_tag
     [Documentation]    1	create a class-map to match VLAN 10 in flow 1
    ...    2	create a policy-map to bind the class-map and traslate-cevlan-tag to 20
    ...
    ...    5	send VLAN 10 upstream traffic to ont1 with SMAC 000001000001 DMAC 000002000002;	both eth-port1 and eth-port2 can pass the upstream traffic
    ...    6	send double-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 00000100000
    [Tags]    @author=AnneLI    @globalid=2298726      @tcid=AXOS_E72_PARENT-TC-616     @eut=NGPON2-4    @priority=P1
    [Setup]    setup

    log    step5: send vlan ${match_vlan} upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2      vlan_id=${match_vlan}    vlan_user_priority=0
    ...    frame_size=512    length_mode=fixed    mac_src=${mac1}    mac_dst=${mac2}    l3_protocol=ipv4    ip_src_addr=${ip1}    ip_dst_addr=${ip2}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id == ${service_vlan} and vlan.id == ${c_vlan}      ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id == ${service_vlan} and vlan.id == ${c_vlan}     ${error_rate}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id == ${service_vlan} and vlan.id == ${c_vlan}     ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p2    eth.src==${mac2} and eth.dst==${mac1} and vlan.id == ${match_vlan}      ${error_rate}
...
setup
     ...
     subscriber_point_add_svc    subscriber_point1      ${match_vlan}       ${service_vlan}         cevlan_action=translate-cevlan-tag         cevlan=${c_vlan}
teardown
     [Documentation]    teardown
    log    teardown
    Run Keyword And Ignore Error    Tg Stop All Traffic    tg1
    Tg Delete All Traffic    tg1
    subscriber_point_remove_svc    subscriber_point1      ${match_vlan}        ${service_vlan}          cevlan=${cvlan}
