...
tc_Unmatch_PCP_change_c_tag
    ...
    ...    5	send PCP 0 upstream traffic to ont1	no eth-port can pass the upstream traffic
    [Tags]    @author=AnneLI    @globalid=2298747      @tcid=AXOS_E72_PARENT-TC-637     @eut=NGPON2-4    @priority=P2
    [Setup]    setup

    log    step6: send vlan ${match_vlan_2} PCP ${pcp_2} upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=${match_vlan_2}    vlan_user_priority=${pcp_2}
