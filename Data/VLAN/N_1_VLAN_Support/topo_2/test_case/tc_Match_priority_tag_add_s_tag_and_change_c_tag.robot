...
tc_Match_priority_tag_add_s_tag_and_change_c_tag
    [Documentation]    1 create a class-map to match priority-tagged in flow 1
    ...
    ...    5 send upstream traffic with priority-tagged to ont1 with SMAC 000001000001 DMAC 000002000002;
    ...    6 send double-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 000001000001;
    [Tags]    @author=AnneLI    @globalid=2298728    @tcid=AXOS_E72_PARENT-TC-618     @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send upstream traffic with priority-tagged to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${traslate_cevlan}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${traslate_cevlan}    ${error_rate}
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream1    p2    p1    vlan_id=${traslate_cevlan}    vlan_user_priority=0
...
setup
    ...
    log    step1:create a class-map to match priority-tagged in flow ${flow_index}
    prov_class_map    eutA    ${class_map_name}    ${class_map_type}    flow    ${flow_index}    ${rule_index}
    ...    priority-tagged=${EMPTY}
    log    step2:create a policy-map to bind the class-map and traslate-cevlan-tag to ${traslate_cevlan}
