...
*** Test Cases ***
tc_Unmatch_src_mac_add_s_tag
    [Documentation]    1	create a class-map to match src-mac in flow 1
    ...
    ...    5	send upstream traffic that does not match src-mac to ont1

    [Tags]    @author=AnneLI    @globalid=2298751      @tcid=AXOS_E72_PARENT-TC-641     @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step5: send upstream traffic that does not match src-mac ont-port ${service_model.subscriber_point1.name};
