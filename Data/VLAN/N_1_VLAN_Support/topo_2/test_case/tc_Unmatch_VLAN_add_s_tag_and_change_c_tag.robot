...
*** Test Cases ***
tc_Unmatch_VLAN_add_s_tag_and_change_c_tag
    [Documentation]    1	create a class-map to match VLAN 10 in flow 1
    ...    2	create a policy-map to bind the class-map and translate cevlan to VLAN 30
    ...
    ...    5	send VLAN 20 single-tag upstream traffic to ont1	no eth-port can pass the upstream traffic
    ...    6	send double tagged downstream traffic to eth-port1 with C-tag=20	no client can receive the downstream traffic
    [Tags]    @author=AnneLI    @globalid=2298748      @tcid=AXOS_E72_PARENT-TC-638     @eut=NGPON2-4    @priority=P2
    [Setup]    setup

    log    step5: send vlan ${match_vlan_2} upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2} ;
    log    step6: send double-tagged downstream traffic to ${interface_type1} ${service_model.service_point1.member.interface1} with SMAC ${mac2} DMAC ${mac1} and c-tag=${cvlan_2};
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream1    p2    p1    vlan_id=${cvlan_2}    vlan_user_priority=0
...
setup
     ...
     subscriber_point_add_svc    subscriber_point1      ${match_vlan}       ${service_vlan}         cevlan_action=translate-cevlan-tag         cevlan=${cvlan}
teardown
