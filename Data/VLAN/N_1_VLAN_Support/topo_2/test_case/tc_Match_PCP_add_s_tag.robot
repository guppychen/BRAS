...
*** Test Cases ***
tc_Match_PCP_add_s_tag
    [Documentation]    1.create a class-map to match PCP 2 in flow 1
    ...
    ...    5.send upstream traffic with PCP 2 to ont1 with SMAC 000001000001 DMAC 000002000002;
    ...    6.send double-tagged downstream traffic to eth-port1 with SMAC 000002000002 DMAC 000001000001;
    [Tags]       @author=Anne Li    @globalid=2298724    @tcid=AXOS_E72_PARENT-TC-614     @eut=NGPON2-4    @priority=P1
    [Setup]       setup
    log    step5: send vlan ${match_vlan_3} PCP ${match_pcp_1} upstream traffic to ${interface_type2} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${match_vlan_3} and vlan.priority == ${match_pcp_1} and vlan.priority == ${match_pcp_1}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${match_vlan_3} and vlan.priority == ${match_pcp_1} and vlan.priority == ${match_pcp_1}    ${error_rate}
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream1    p2    p1    vlan_id=${match_vlan_3}    vlan_user_priority=0
