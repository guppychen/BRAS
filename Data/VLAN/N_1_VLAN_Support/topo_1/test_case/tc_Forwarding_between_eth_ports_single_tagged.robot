...
*** Test Cases ***
tc_Forwarding_between_eth_ports_single_tagged
    [Documentation]    1	add eth-port1 eth-port2 and eth-port3 to the same transport-service-profile.
    ...    2	send single-tagged traffic into eth-port1 with SMAC 000001000001 DMAC 000002000002;	the traffic can be received from both eth-port2 and eth-port3
    ...    3	send single-tagged traffic into eth-port2 with SMAC 000002000002 DMAC 000001000001;	the traffic can only be received from eth-port1
    ...    4	show bridge table	SMACs learned correctly on eth-ports and VLAN
    ...    5	capture packtes on eth-port3	no packet received

    [Tags]    @author=AnneLI    @globalid=2298710      @tcid=AXOS_E72_PARENT-TC-600       @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step2: send single-tagged traffic into ${service_model.service_point1.member.interface1} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_stream1    p2    p1      vlan_id=${service_vlan}    vlan_user_priority=0    frame_size=512
