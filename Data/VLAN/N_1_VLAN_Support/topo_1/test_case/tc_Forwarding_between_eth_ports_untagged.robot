...
*** Test Cases ***
tc_Forwarding_between_eth_ports_untagged
    [Documentation]    1 add eth-port1 eth-port2 and eth-port3 to the same transport-service-profile.
    ...    2 set the eth-ports native-vlan as 1
    ...    3 send untagged traffic into eth-port1 with SMAC 000001000001 DMAC 000002000002; the traffic can be received from both eth-port2 and eth-port3
    ...    4 send untagged traffic into eth-port2 with SMAC 000002000002 DMAC 000001000001; the traffic can only be received from eth-port1
    ...    5 show bridge table SMACs learned correctly on eth-ports and VLAN
    ...    6 capture packtes on eth-port3 no packet received
    [Tags]    @author=AnneLI    @globalid=2298709    @tcid=AXOS_E72_PARENT-TC-599    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step3: send untagged traffic into ${service_model.service_point1.member.interface1} with SMAC ${mac1} DMAC ${mac2};
    TG Create Untagged Stream On Port    tg1    raw_stream1    p2    p1    frame_size=512    length_mode=fixed
    ...
    log    step4: send untagged traffic into ${service_model.service_point2.member.interface1} with SMAC ${mac2} DMAC ${mac1};
    TG Create Untagged Stream On Port    tg1    raw_stream2    p1    p2    frame_size=512
     ...    length_mode=fixed
     ...    mac_src=${mac2}    mac_dst=${mac1}    l3_protocol=ipv4    ip_src_addr=${ip2}    ip_dst_addr=${ip1}
     ...     l4_protocol=udp
     ...
     check_bridge_table    eutA    ${mac2}    ${service_model.service_point2.member.interface1}    ${native_vlan1}
     log    step6: capture packtes on ${service_model.service_point3.member.interface1}
     log    show interface counters after stop traffic
     show_interface_counters    eutA    ${interface_type1}    ${service_model.service_point1.member.interface1}
     show_interface_counters    eutA    ${interface_type1}    ${service_model.service_point2.member.interface1}
     show_interface_counters    eutA    ${interface_type1}    ${service_model.service_point3.member.interface1}
...
setup
    ...
    log    step2: set the eth-ports native-vlan as ${native_vlan1}
