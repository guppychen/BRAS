...
*** Test Cases ***
tc_Forwarding_between_eth_ports_single_tagged_match_eth_port_ethertype_0x88a8
    ...
    ...    3	send single-tagged traffic into eth-port1 with SMAC 000001000001 DMAC 000002000002; the ethertype of ther traffic is 0x88a8	the traffic can be received from both eth-port2 and eth-port3 and the received traffic should be 0x8100 ethertype
    ...    4	send single-tagged traffic into eth-port2 with SMAC 000002000002 DMAC 000001000001;	the traffic can only be received from eth-port1 and the received traffic should be 0x88a8 ethertype
    ...    5	show bridge table	SMACs learned correctly on eth-ports and VLAN
    ...    6	capture packtes on eth-port3	no packet received
    [Tags]    @author=AnneLI    @globalid=2298715      @tcid=AXOS_E72_PARENT-TC-605       @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step2: send single-tagged traffic into ${service_model.service_point1.member.interface1} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_stream1    p2    p1      vlan_id=${service_vlan}    vlan_user_priority=0    vlan_tpid=34984    frame_size=512
    ...
    log    step3: send single-tagged traffic into ${service_model.service_point2.member.interface1} with SMAC ${mac2} DMAC ${mac1};
