...
*** Test Cases ***
tc_Forwarding_between_eth_ports_double_tagged_match_eth_port_ethertype_0x9100
    [Documentation]    1	set eth-port1 ethertype as 0x9100
    ...    2	add eth-port1 eth-port2 and eth-port3 to the same transport-service-profile.
    ...    3	send double-tagged traffic into eth-port1 with SMAC 000001000001 DMAC 000002000002; the ethertype of ther traffic is 0x9100	the traffic can be received from both eth-port2 and eth-port3 and the received traffic should be 0x8100 ethertype
    ...    4	send double-tagged traffic into eth-port2 with SMAC 000002000002 DMAC 000001000001;	the traffic can only be received from eth-port1 and the received traffic should be 0x9100 ethertype
    ...    5	show bridge table	SMACs learned correctly on eth-ports and VLAN
    ...    6	capture packtes on eth-port3	no packet received

    [Tags]    @author=AnneLI    @globalid=2298716      @tcid=AXOS_E72_PARENT-TC-606       @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step2: send double-tagged traffic into ${service_model.service_point1.member.interface1} with SMAC ${mac1} DMAC ${mac2},; the ethertype of ther traffic is 0x9100;
    Tg Create Double Tagged Stream On Port    tg1    raw_stream1    p2    p1    vlan_id=${c_vlan}    vlan_user_priority=0    vlan_tpid=37120
    ...    vlan_id_outer=${service_vlan}    vlan_outer_user_priority=0    vlan_outer_tpid=37120    frame_size=512    length_mode=fixed    mac_src=${mac1}    mac_dst=${mac2}
    ...
    log    step3: send double-tagged traffic into ${service_model.service_point2.member.interface1} with SMAC ${mac2} DMAC ${mac1};
    ...
    check_bridge_table    eutA    ${mac1}     ${service_model.service_point1.member.interface1}    ${service_vlan}
