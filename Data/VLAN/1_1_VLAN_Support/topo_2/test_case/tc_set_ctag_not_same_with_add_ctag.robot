...
*** Test Cases ***
tc_set_ctag_not_same_with_add_ctag
    [Documentation]    1.set-ctag not same with add-ctag
    [Tags]    @globalid=2318829    @tcid=AXOS_E72_PARENT-TC-1184    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    ...
    ...    vlan_id=${cvlan_one2one_2}    vlan_user_priority=0    frame_size=512    length_mode=fixed    mac_src=${mac4}    mac_dst=${mac3}
    ...    l3_protocol=ipv4    ip_src_addr=${ip2}    ip_dst_addr=${ip1}    l4_protocol=udp    udp_dst_port=${udp_port2}    udp_src_port=${udp_port1}
    ...    rate_bps=${rate_bps}
...
setup
    ...
    ...    add-cevlan-tag=${cvlan_one2one_1}
