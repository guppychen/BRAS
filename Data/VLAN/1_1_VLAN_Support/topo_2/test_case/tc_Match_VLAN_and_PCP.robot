...
*** Test Cases ***
tc_Match_VLAN_and_PCP
    [Documentation]    1.set vlan 100 mode as one2one
    ...    2.add eth-port1 and eth-port2 to VLAN 100 with transport-service-profile
    ...    3.apply VLAN 100 to ONT1 and ONT2
    ...    4.set (S;C) tags for ONT1 as (100;10) and for ONT2 as (100;20)
    ...    5.send single-tagged upstream traffic to ONT1
    ...    6.send downstream traffic with (100;10) into eth-port1
    [Tags]    @globalid=2318789    @tcid=AXOS_E72_PARENT-TC-1144    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send single-tagged VLAN ${match_vlan_3} PCP ${match_pcp_1} upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=${match_vlan_3}    vlan_user_priority=${match_pcp_1}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==${match_vlan_3} and vlan.priority ==${match_pcp_1} and vlan.priority ==${match_pcp_1} and vlan.priority ==${match_pcp_1}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==${match_vlan_3} and vlan.priority ==${match_pcp_1} and vlan.priority ==${match_pcp_1} and vlan.priority ==${match_pcp_1}    ${error_rate}
    log    step6: send downstream traffic with (${service_vlan};${cvlan_one2one_1};${match_vlan_3}) into ${service_model.service_point1.attribute.interface_type} ${service_model.service_point1.member.interface1} with SMAC ${mac2} DMAC ${mac1};
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream1    p2    p1    vlan_id_other=${service_vlan}    vlan_user_priority_other=0
    ...    vlan_id_outer=${cvlan_one2one_1}    vlan_outer_user_priority=0    vlan_id=${match_vlan_3}    vlan_user_priority=0    frame_size=512    length_mode=fixed
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream2    p2    p1    vlan_id_other=${service_vlan}    vlan_user_priority_other=0
    ...    vlan_id_outer=${cvlan_one2one_2}    vlan_outer_user_priority=0    vlan_id=${match_vlan_3}    vlan_user_priority=0    frame_size=512    length_mode=fixed
    ...    mac_src=${mac3}    mac_dst=${mac4}    l3_protocol=ipv4    ip_src_addr=${ip2}    ip_dst_addr=${ip1}    l4_protocol=udp
    ...    udp_dst_port=${udp_port2}    udp_src_port=${udp_port1}    rate_bps=${rate_bps}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==${match_vlan_3} and vlan.priority ==${match_pcp_1} and vlan.priority ==${match_pcp_1} and vlan.priority ==${match_pcp_1}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p2    eth.src==${mac2} and eth.dst==${mac1} and vlan.id == ${match_vlan_3}    ${error_rate}
...
setup
    ...
    ...    vlan=${match_vlan_3}    pcp=${match_pcp_1}
