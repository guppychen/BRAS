...
*** Test Cases ***
tc_Match_untagged_and_set_stag_pcp
    ...
    [Tags]    @globalid=2318788    @tcid=AXOS_E72_PARENT-TC-1143    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.priority ==${stag_pcp} and vlan.priority ==${stag_pcp}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.priority ==${stag_pcp} and vlan.priority ==${stag_pcp}    ${error_rate}
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream2    p2    p1    vlan_id_outer=${service_vlan}    vlan_outer_user_priority=0
