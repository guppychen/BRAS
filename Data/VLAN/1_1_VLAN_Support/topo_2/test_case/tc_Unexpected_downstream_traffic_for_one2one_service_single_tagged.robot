...
*** Test Cases ***
tc_Unexpected_downstream_traffic_for_one2one_service_single_tagged
    [Documentation]    1.send single tagged downstream traffic to uplink port
    [Tags]    @globalid=2318824    @tcid=AXOS_E72_PARENT-TC-1179    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    1.send single tagged downstream traffic to uplink port
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p3    eth.src==${mac2} and eth.dst==${mac1} and vlan.id==${service_vlan} and vlan.priority ==0    ${error_rate}
