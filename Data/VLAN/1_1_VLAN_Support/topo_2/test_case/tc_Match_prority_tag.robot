...
tc_Match_priority_tag
    ...
    [Tags]    @globalid=2318783    @tcid=AXOS_E72_PARENT-TC-1138    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send priority tagged upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=0    vlan_user_priority=0
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==0 and vlan.priority ==0 and vlan.priority ==0 and vlan.priority ==0    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==0 and vlan.priority ==0 and vlan.priority ==0 and vlan.priority ==0    ${error_rate}
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream1    p2    p1    vlan_id_other=${service_vlan}    vlan_user_priority_other=0    vlan_id_outer=${cvlan_one2one_1}    vlan_outer_user_priority=0    vlan_id=0    vlan_user_priority=0
    ...    frame_size=512    length_mode=fixed    mac_src=${mac2}    mac_dst=${mac1}
    ...
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream2    p2    p1    vlan_id_other=${service_vlan}    vlan_user_priority_other=0    vlan_id_outer=${cvlan_one2one_2}    vlan_outer_user_priority=0    vlan_id=0    vlan_user_priority=0
    ...    frame_size=512    length_mode=fixed    mac_src=${mac3}    mac_dst=${mac4}
    ...    l3_protocol=ipv4    ip_src_addr=${ip2}    ip_dst_addr=${ip1}    l4_protocol=udp    udp_dst_port=${udp_port2}    udp_src_port=${udp_port1}
    ...    rate_bps=${rate_bps}
