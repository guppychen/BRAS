...
tc_Match_priority_tag_PCP_7
    ...
    [Tags]    @globalid=2318790    @tcid=AXOS_E72_PARENT-TC-1145    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send priority tagged PCP ${match_pcp_3} upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=0    vlan_user_priority=${match_pcp_3}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==0 and vlan.priority ==${match_pcp_3} and vlan.priority ==${match_pcp_3} and vlan.priority ==${match_pcp_3}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==0 and vlan.priority ==${match_pcp_3} and vlan.priority ==${match_pcp_3} and vlan.priority ==${match_pcp_3}    ${error_rate}
    log    step6: send downstream traffic with (${service_vlan};${cvlan_one2one_1};0) into ${service_model.service_point1.attribute.interface_type} ${service_model.service_point1.member.interface1} with SMAC ${mac2} DMAC ${mac1};
    Tg Create Double Tagged Stream On Port    tg1    raw_downstream1    p2    p1    vlan_id_other=${service_vlan}    vlan_user_priority_other=0
    ...    vlan_id_outer=${cvlan_one2one_1}    vlan_outer_user_priority=0    vlan_id=0    vlan_user_priority=0    frame_size=512    length_mode=fixed
    ...
    ...    vlan_id_outer=${cvlan_one2one_2}    vlan_outer_user_priority=0    vlan_id=0    vlan_user_priority=0    frame_size=512    length_mode=fixed
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==0 and vlan.priority ==${match_pcp_3} and vlan.priority ==${match_pcp_3} and vlan.priority ==${match_pcp_3}    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p2    eth.src==${mac2} and eth.dst==${mac1} and vlan.id ==0    ${error_rate}
...
setup
    ...
    ...    priority-tagged=${EMPTY}    pcp=${match_pcp_3}
