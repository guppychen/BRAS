...
tc_Unmatch_PCP
    ...
    ...    5.send unmatched PCP upstream traffic to ont1
    [Tags]    @globalid=2318813    @tcid=AXOS_E72_PARENT-TC-1168    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step5: send unmatched PCP upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name}
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=${match_vlan_2}    vlan_user_priority=${match_pcp_2}    frame_size=512    length_mode=fixed
