...
tc_Unmatch_priority_tag
    ...
    ...    5.send VLAN 10 upstream traffic to ont1
    [Tags]    @globalid=2318816    @tcid=AXOS_E72_PARENT-TC-1171    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step5: send VLAN ${match_vlan_1} upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name}
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=${match_vlan_1}    vlan_user_priority=0    frame_size=512    length_mode=fixed
