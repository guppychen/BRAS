...
*** Test Cases ***
tc_Unexpected_downstream_traffic_for_one2one_service_double_tagged
    [Documentation]    1.send single tagged downstream traffic to uplink port
    [Tags]    @globalid=2318826    @tcid=AXOS_E72_PARENT-TC-1181    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    ...
    ...    vlan_id=0    vlan_user_priority=1    frame_size=512    length_mode=fixed    mac_src=${mac2}    mac_dst=${mac1}
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_downstream1    p3    eth.src==${mac2} and eth.dst==${mac1} and vlan.id==${service_vlan} and vlan.id==0 and vlan.priority ==0 and vlan.priority ==1    ${error_rate}
