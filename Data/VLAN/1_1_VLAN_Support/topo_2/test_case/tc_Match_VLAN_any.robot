...
tc_Match_VLAN_any
    ...
    [Tags]    @globalid=2318787    @tcid=AXOS_E72_PARENT-TC-1142    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    log    step5: send single-tagged VLAN ${match_vlan_3} upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name} with SMAC ${mac1} DMAC ${mac2};
    ...
    log    verify traffic
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p1    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==${match_vlan_3} and vlan.priority ==0 and vlan.priority ==0 and vlan.priority ==0    ${error_rate}
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p3    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.id==${cvlan_one2one_1} and vlan.id==${match_vlan_3} and vlan.priority ==0 and vlan.priority ==0 and vlan.priority ==0    ${error_rate}
