...
*** Test Cases ***
tc_Unmatch_priority_tag_and_PCP
    ...
    ...    5.send VLAN 0 PCP 6 upstream traffic to ont1
    ...    6.send VLAN 10 PCP 5 upstream traffic to ont1
    [Tags]    @globalid=2318815    @tcid=AXOS_E72_PARENT-TC-1170    @eut=NGPON2-4    @priority=P2
    [Setup]    setup
    log    step5: send VLAN 0 PCP ${match_pcp_2} upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name}
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p1    p2    vlan_id=0    vlan_user_priority=${match_pcp_2}    frame_size=512    length_mode=fixed
    ...
    log    step6: send VLAN ${match_vlan_1} PCP ${match_pcp_1} upstream traffic to ${service_model.subscriber_point1.attribute.interface_type} ${service_model.subscriber_point1.name}
    Tg Create single Tagged Stream On Port    tg1    raw_upstream2    p1    p2    vlan_id=${match_vlan_1}    vlan_user_priority=${match_pcp_1}    frame_size=512    length_mode=fixed
    ...    mac_src=${mac3}    mac_dst=${mac4}    l3_protocol=ipv4    ip_src_addr=${ip1}    ip_dst_addr=${ip2}    l4_protocol=udp
    ...    udp_dst_port=${udp_port1}    udp_src_port=${udp_port2}    rate_bps=${rate_bps}
    log    clear interface counters before start traffic
    clear_interface_counters    eutA    ${service_model.service_point1.attribute.interface_type}    ${service_model.service_point1.member.interface1}
    clear_interface_counters    eutA    ${service_model.service_point1.attribute.interface_type}    ${service_model.service_point2.member.interface1}
...
setup
    ...
    ...    priority-tagged=${EMPTY}    PCP=${match_pcp_1}
