...
*** Test Cases ***
tc_TLAN_Untagged_traffic_tlan_between_different_ONTs_on_the_same_gpon_port
    [Documentation]    Summary:
    ...    The acronym TLS stands for Transparent LAN Services. TLS allows traffic from multiple customers to be transparently
    ...    forwarded through an access network using service VLANs or S-VLAN.
    ...    This is done by wrapping customer vlans (c-vids) with a service vlan (s-vid).
    ...
    ...    applied to it. So untagged traffic will enter the pon with a single tag and c-vid untagged traffic that enters the
    ...    TLS port on the ont will have a Double Q tag (DQ tag).
    [Tags]    @EUT=NGPON2-4    @TCID=AXOS_E72_PARENT-TC-144   @GlobalID=1526514
    [Setup]    case setup
    log    Send known Unicast frames from each EP frames are received from each EP
    Tg Stc Create Device On Port    tg1    uplink    service_p1    intf_ip_addr=${int_ip1}    gateway_ip_addr=${int_ip2}    mac_addr=${int_mac1}
    Tg Stc Create Device On Port    tg1    downlink    subscriber_p1    intf_ip_addr=${int_ip2}    gateway_ip_addr=${int_ip1}    mac_addr=${int_mac2}
    ...
    TG Create Untagged Stream On Port    tg1    ucast1_2    service_p1    subscriber_p1    mac_dst=${mac2}    mac_src=${mac1}    length_mode=fixed    frame_size=512    rate_pps=${rate_pps10000}
    TG Create Untagged Stream On Port    tg1    ucast2_1    subscriber_p1    service_p1    mac_dst=${mac1}    mac_src=${mac2}    length_mode=fixed    frame_size=512    rate_pps=${rate_pps10000}
    log    Send Bcast from each EP EUT frames are received from each EP
    TG Create Untagged Stream On Port    tg1    bcast1_2    service_p1    subscriber_p1    mac_dst=${bcast_mac}    mac_src=${mac3}    length_mode=fixed    frame_size=512    rate_pps=${rate_pps10000}
    TG Create Untagged Stream On Port    tg1    bcast2_1    subscriber_p1    service_p1    mac_dst=${bcast_mac}    mac_src=${mac4}    length_mode=fixed    frame_size=512    rate_pps=${rate_pps10000}
    log    Send multicast frames from each EP EUT frames are received from each EP
    TG Create Untagged Stream On Port    tg1    mcast1_2    service_p1    subscriber_p1    mac_dst=${mcast_mac1}    mac_src=${mac5}    length_mode=fixed    frame_size=512    rate_pps=${rate_pps10000}
    TG Create Untagged Stream On Port    tg1    mcast2_1    subscriber_p1    service_p1    mac_dst=${mcast_mac2}    mac_src=${mac6}    length_mode=fixed    frame_size=512    rate_pps=${rate_pps10000}
    ...
    verify_no_traffic_on_port_with_filter    tg1    service_p1    vlan.id
    verify_no_traffic_on_port_with_filter    tg1    subscriber_p1    vlan.id
