*** Settings ***
Documentation     Verify that  when traffic send from pon-1 port to pon-2 port and unicast packets can be recieved on pon-2 port which have vlan mode in ELINE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_Unicast_from_one_pon_port_to_another_port_which_have_vlan_mode_in_ELINE_mode
    [Documentation]    Verify that  when traffic send from pon-1 port to pon-2 port and unicast packets can be recieved on pon-2 port which have vlan mode in ELINE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-386    @globalid=2191703    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that when traffic send from pon-1 port to pon-2 port and unicast packets can be recieved on pon-2 port which have vlan mode in ELINE mode.
