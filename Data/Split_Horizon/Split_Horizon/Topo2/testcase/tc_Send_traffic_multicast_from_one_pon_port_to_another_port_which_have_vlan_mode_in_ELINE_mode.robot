*** Settings ***
Documentation     Verify that  when traffic send from pon-1 port to pon-2 port and multicast packets can be recieved on pon-2 port which have vlan mode in ELINE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_multicast_from_one_pon_port_to_another_port_which_have_vlan_mode_in_ELINE_mode
    [Documentation]    Verify that when traffic send from pon-1 port to pon-2 port and multicast packets can be recieved on pon-2 port which have vlan mode in ELINE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-414    @globalid=2263614    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that when traffic send from pon-1 port to pon-2 port and multicast packets can be recieved on pon-2 port which have vlan mode in ELINE mode.

    log    send traffic from ONT-1 of PON-1 to ONT-2 of PON-2.(multicast traffic)
