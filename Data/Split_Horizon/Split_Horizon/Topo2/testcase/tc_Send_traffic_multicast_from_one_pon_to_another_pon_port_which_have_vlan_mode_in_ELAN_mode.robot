*** Settings ***
Documentation     Verify that  when traffic send from pon-1 port to pon-2 port and multicast packets can be received on pon-2 port which have vlan mode in ELAN mode. 
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_multicast_from_one_pon_to_another_pon_port_which_have_vlan_mode_in_ELAN_mode
    [Documentation]    Verify that when traffic send from pon-1 port to pon-2 port and multicast packets can be received on pon-2 port which have vlan mode in ELAN mode. 
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-412    @globalid=2263612    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that when traffic send from pon-1 port to pon-2 port and multicast packets can be received on pon-2 port which have vlan mode in ELAN mode.
