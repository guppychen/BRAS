*** Settings ***
Documentation     Verify that multicast packets got dropped when traffic send from one pon port to another pon port which have vlan mode in N2ONE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_multicast_from_one_pon_port_to_another_pon_port_which_have_vlan_mode_in_N2ONE_mode
    [Documentation]    Verify that multicast packets got dropped when traffic send from one pon port to another pon port which have vlan mode in N2ONE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-410    @globalid=2263610    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that multicast packets got dropped when traffic send from one pon port to another pon port which have vlan mode in N2ONE mode.
