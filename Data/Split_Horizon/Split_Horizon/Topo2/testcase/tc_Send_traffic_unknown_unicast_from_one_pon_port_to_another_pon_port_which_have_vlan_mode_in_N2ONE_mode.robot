*** Settings ***
Documentation     Verify that unknown unicast packets got dropped when traffic send from one pon port to another pon port which have vlan mode in N2ONE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_unknown_unicast_from_one_pon_port_to_another_pon_port_which_have_vlan_mode_in_N2ONE_mode
    [Documentation]    Verify that unknown  unicast packets got dropped when traffic send from one pon port to another pon port which have vlan mode in N2ONE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-383    @globalid=2191201    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that unknown unicast packets got dropped when traffic send from one pon port to another pon port which have vlan mode in N2ONE mode.

    log    send traffic from ONT-1 of PON-1 to ONT-2 of PON-2.(Unicast traffic)
    Tg Create Single Tagged Stream On Port    tg1    stream1    subscriber_p2    subscriber_p1    vlan_id=${p_match_vlan2}    vlan_user_priority=0
