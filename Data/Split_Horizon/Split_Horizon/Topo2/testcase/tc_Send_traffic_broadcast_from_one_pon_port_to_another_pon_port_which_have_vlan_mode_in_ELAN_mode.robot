*** Settings ***
Documentation     Verify that broadcast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELAN mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_broadcast_from_one_pon_port_to_another_pon_port_which_have_vlan_mode_in_ELAN_mode
    [Documentation]    Verify that broadcast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELAN mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-413    @globalid=2263613    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that broadcast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELAN mode.

    log    send traffic from ONT-1 of PON-1 to ONT-2 of PON-2.(broadcast traffic)
    ...
    tg save config into file    tg1    /tmp/sp2.xml
