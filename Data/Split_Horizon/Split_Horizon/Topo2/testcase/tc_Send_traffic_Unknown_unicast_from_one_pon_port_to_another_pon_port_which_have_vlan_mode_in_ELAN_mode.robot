*** Settings ***
Documentation     Verify that unknown unicast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELAN mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_Unknown_unicast_from_one_pon_port_to_another_pon_port_which_have_vlan_mode_in_ELAN_mode
    [Documentation]    Verify that unknown unicast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELAN mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-385    @globalid=2191701    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    ...
    log    send traffic from ONT-1 of PON-1 to ONT-2 of PON-2.(Unknown Unicast traffic)
    Tg Create Single Tagged Stream On Port    tg1    stream1    subscriber_p1    subscriber_p2    vlan_id=${p_match_vlan2}    vlan_user_priority=0
    ...    mac_dst=${subscriber_mac1}    mac_src=${subscriber_mac2}    rate_pps=${rate_pps}    frame_size=512    length_mode=fixed    l3_protocol=ipv4
    ...    ip_dst_addr=${subscriber_ip1}    ip_src_addr=${subscriber_ip2}    l4_protocol=udp    udp_dst_port=64    udp_src_port=63
