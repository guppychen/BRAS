*** Settings ***
Documentation     Verify that unknown unicast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELINE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_Unknown_Unicast_from_one_pon_port_to_another_pon_port_which_have_vlan_mode_in_ELINE_mode
    [Documentation]    Verify that unknown unicast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELINE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-387    @globalid=2191705    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that unknown unicast packets can be received on pon-2 port when traffic send from pon-1 port to pon-2 port which have vlan mode in ELINE mode.
