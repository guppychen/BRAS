*** Settings ***
Documentation     Verify that  when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and unicast packets can be recieved on ont 2 port which have vlan mode in ELINE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_Unicast_from_pon_1_ont_1_to_pon_1_ont_2_which_have_vlan_mode_in_ELINE_mode
    [Documentation]    Verify that when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and unicast packets can be recieved on ont 2 port which have vlan mode in ELINE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-399    @globalid=2262105    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and unicast packets can be recieved on ont 2 port which have vlan mode in ELINE mode.

    log    send traffic from ONT-1 of PON-1 to ONT-2 of PON-1.(Unicast traffic)
