*** Settings ***
Documentation     Verify that  when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and unicast packets can be received on ont-2 port which have vlan mode in ELAN mode. 
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_unicast_from_pon_1_ont_1_to_pon_1_ont_2_which_have_vlan_mode_in_ELAN_mode
    [Documentation]    Verify that  when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and unicast packets can be received on ont-2 port which have vlan mode in ELAN mode. 
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-397    @globalid=2262103    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and unicast packets can be received on ont-2 port which have vlan mode in ELAN mode.
