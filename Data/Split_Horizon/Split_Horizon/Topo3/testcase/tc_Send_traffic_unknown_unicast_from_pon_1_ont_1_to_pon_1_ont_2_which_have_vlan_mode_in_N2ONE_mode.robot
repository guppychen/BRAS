*** Settings ***
Documentation     Verify that unknown unicast packets got dropped when traffic send from pon 1 ont 1 port to pon 1 ont 2 port which have vlan mode in N2ONE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_unknown_unicast_from_pon_1_ont_1_to_pon_1_ont_2_which_have_vlan_mode_in_N2ONE_mode
    [Documentation]    Verify that unknown unicast packets got dropped when traffic send from pon 1 ont 1 port to pon 1 ont 2 port which have vlan mode in N2ONE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-396     @globalid=2262102    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that unknown unicast packets got dropped when traffic send from pon 1 ont 1 port to pon 1 ont 2 port which have vlan mode in N2ONE mode.
