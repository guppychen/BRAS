*** Settings ***
Documentation     Verify that unknown unicast packets can be received on ont-2 port when traffic send from pon-1 ont 1 port to pon-1 ont 2 port which have vlan mode in ELAN mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_Unknown_unicast_from_pon_1_ont_1_port_to_pon_1_ont_2_port_which_have_vlan_mode_in_ELAN_mode
    [Documentation]    Verify that unknown unicast packets can be received on ont-2 port when traffic send from pon-1 ont 1 port to pon-1 ont 2 port which have vlan mode in ELAN mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-398    @globalid=2262104    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that unknown unicast packets can be received on ont-2 port when traffic send from pon-1 ont 1 port to pon-1 ont 2 port which have vlan mode in ELAN mode.
