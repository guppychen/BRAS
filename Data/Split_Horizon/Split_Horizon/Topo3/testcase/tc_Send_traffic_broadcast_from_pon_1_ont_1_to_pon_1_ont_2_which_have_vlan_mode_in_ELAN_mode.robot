*** Settings ***
Documentation      Verify that  when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and broadcast packets can be received on ont-2 port which have vlan mode in ELAN mode. 
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_broadcast_from_pon_1_ont_1_to_pon_1_ont_2_which_have_vlan_mode_in_ELAN_mode
    [Documentation]    Verify that  when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and broadcast packets can be received on ont-2 port which have vlan mode in ELAN mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-406    @globalid=2263606    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that when traffic send from pon-1 ont 1 port to pon-1 ont 2 port and broadcast packets can be received on ont-2 port which have vlan mode in ELAN mode.
...
teardown
    ...
    Tg Save Config Into File     tg1    /tmp/split_horizon_2263606.xml
