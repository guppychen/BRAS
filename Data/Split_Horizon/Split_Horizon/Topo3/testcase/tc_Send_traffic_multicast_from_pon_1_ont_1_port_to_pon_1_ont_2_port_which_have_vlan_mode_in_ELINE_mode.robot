*** Settings ***
Documentation     Verify that multicast packets can be received on ont-2 port when traffic send from pon-1 ont 1 port to pon-1 ont 2 port which have vlan mode in ELINE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_multicast_from_pon_1_ont_1_port_to_pon_1_ont_2_port_which_have_vlan_mode_in_ELINE_mode
    [Documentation]    Verify that multicast packets can be received on ont-2 port when traffic send from pon-1 ont 1 port to pon-1 ont 2 port which have vlan mode in ELINE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-409    @globalid=2263609    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that multicast packets can be received on ont-2 port when traffic send from pon-1 ont 1 port to pon-1 ont 2 port which have vlan mode in ELINE mode.
