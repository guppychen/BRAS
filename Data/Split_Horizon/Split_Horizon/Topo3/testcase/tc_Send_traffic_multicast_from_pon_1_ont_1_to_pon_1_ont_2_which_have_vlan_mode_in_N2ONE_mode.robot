*** Settings ***
Documentation     Verify that multicast packets got dropped when traffic send from pon 1 ont 1 port to pon 1 ont 2 port which have vlan mode in N2ONE mode.
Resource          ./base.robot
...
*** Test Cases ***
tc_Send_traffic_multicast_from_pon_1_ont_1_to_pon_1_ont_2_which_have_vlan_mode_in_N2ONE_mode
    [Documentation]    Verify that multicast packets got dropped when traffic send from pon 1 ont 1 port to pon 1 ont 2 port which have vlan mode in N2ONE mode.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-405     @globalid=2263605    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:Verify that multicast packets got dropped when traffic send from pon 1 ont 1 port to pon 1 ont 2 port which have vlan mode in N2ONE mode.

    log    send traffic from ONT-1 to ONT-2.(multicast traffic)
