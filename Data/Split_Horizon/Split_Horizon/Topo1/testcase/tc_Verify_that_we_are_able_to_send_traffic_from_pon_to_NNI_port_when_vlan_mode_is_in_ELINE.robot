*** Settings ***
Documentation     Verify that we are able to send traffic from pon to NNI port when vlan mode is in ELINE.
Resource          ./base.robot
...
*** Test Cases ***
tc_Verify_that_we_are_able_to_send_traffic_from_pon_to_NNI_port_when_vlan_mode_is_in_ELINE
    [Documentation]    Verify that we are able to send traffic from pon to NNI port when vlan mode is in ELINE.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-401    @globalid=2262107    @eut=NGPON2-4    @priority=P2
    [Setup]      setup
    [Teardown]   teardown
    Tg Start Arp Nd On All Devices    tg1
    Tg Start Arp Nd On All Stream Blocks      tg1
    log    send traffic from ONT connected STC interface to EUT connected STC interface(ethernet interface NNI).
