*** Settings ***
Documentation     Verify that we are able to send traffic from NNI to another NNI port when vlan mode is in ELAN.
Resource          ./base.robot
...
*** Test Cases ***
tc_Verify_that_we_are_able_to_send_traffic_from_NNI_to_another_NNI_port_when_vlan_mode_is_in_ELAN
    [Documentation]    Verify that we are able to send traffic from NNI to another NNI port when vlan mode is in ELAN.
    [Tags]       @author=joli     @tcid=AXOS_E72_PARENT-TC-393    @globalid=2218154    @eut=NGPON2-4    @priority=P1
    [Setup]      setup
    [Teardown]   teardown
