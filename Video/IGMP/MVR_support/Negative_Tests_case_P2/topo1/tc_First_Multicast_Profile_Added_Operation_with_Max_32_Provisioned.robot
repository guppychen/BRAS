*** Settings ***
Documentation    First Multicast Profile Added Operation with MAX(32) Provisioned
Resource     ./base.robot
...
*** Test Cases ***
tc_First_Multicast_Profile_Added_Operation_with_Max_32_Provisioned
    [Documentation]
    ...    1	Create max number of Multicast Profiles.			
    ...    2	Add video service to an access interface using the first Multicast Profile created on the system.			
    ...
    [Tags]     @tcid=AXOS_E72_PARENT-TC-1518      @subFeature=MVR support      @globalid=2321587      @priority=P2      @user_interface=CLI      @eut=NGPON2-4 
    [Setup]     case setup
    [Teardown]     case teardown
      
    log    STEP:1 Create max number of Multicast Profiles. 
    : FOR    ${index}    IN RANGE    ${first_mcprf_idx}    ${first_mcprf_idx}+${p_max_mcast_prf_num}
