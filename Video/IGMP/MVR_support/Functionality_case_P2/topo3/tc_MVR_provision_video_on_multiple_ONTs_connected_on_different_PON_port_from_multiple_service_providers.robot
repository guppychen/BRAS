*** Settings ***
Documentation     MVR: provision video on multiple ONTs connected on different PON port from multiple service providers
...
*** Test Cases ***
tc_MVR_provision_video_on_multiple_ONTs_connected_on_different_PON_port_from_multiple_service_providers
    [Documentation]    MVR: provision video on multiple ONTs connected on different PON port from multiple service providers. Verify they all can view same channels and different channels at the same time
    ...
    [Tags]       @author=CindyGao     @TCID=AXOS_E72_PARENT-TC-1460    @globalid=2321528    @priority=P2    @user_interface=CLI    @eut=NGPON2-4
    [Setup]      case setup
    ...
    subscriber_point_add_svc    subscriber_point3    ${p_match_vlan_sub3}    ${p_data_vlan}    cevlan_action=remove-cevlan    mcast_profile=mcast_prf2   cfg_prefix=sub3
    ...
    ...    ${p_match_vlan_sub3}    session=1    mc_group_name=mc_grp1    mc_group_start_ip=${mc_ip_1}  
    ...
    Wait Until Keyword Succeeds    1min    10sec    subscriber_point_check_igmp_multicast_summary    subscriber_point3    ${p_data_vlan}    ${mc_ip_1}    ${mvr_vlan_2}   contain=no
    Wait Until Keyword Succeeds    1min    10sec    subscriber_point_check_igmp_multicast_summary    subscriber_point3    ${p_data_vlan}    ${mc_ip_2}    ${mvr_vlan_2}
    Wait Until Keyword Succeeds    1min    10sec    subscriber_point_check_igmp_multicast_summary    subscriber_point3    ${p_data_vlan}    ${mc_ip_3}    ${mvr_vlan_2}
...
case teardown
    ...
    subscriber_point_remove_svc    subscriber_point3    ${p_match_vlan_sub3}    ${p_data_vlan}    mcast_profile=mcast_prf2   cfg_prefix=sub3
