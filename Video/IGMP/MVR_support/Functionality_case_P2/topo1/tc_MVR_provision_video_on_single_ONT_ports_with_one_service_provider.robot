*** Settings ***
Documentation     MVR: provision video on single ONT port with one service provider 
Resource          ./base.robot
...
*** Test Cases ***
tc_MVR_provision_video_on_single_ONT_ports_with_one_service_provider
    [Documentation]    Provisioning MVR video on ONT port from single service provider. Verify video is working and channel changes working. 
    [Tags]       @author=CindyGao     @TCID=AXOS_E72_PARENT-TC-1454    @globalid=2321522    @priority=P2    @user_interface=CLI    @eut=NGPON2-4
    [Setup]      case setup
    ...
    ...    ${p_match_vlan}    session=1    mc_group_name=mc_grp2    mc_group_start_ip=${new_grp}
    ...
    log    STEP:6 send multicast downstream traffic and verify traffics fully passed for new group, traffics fully lost for org group
    ...
    verify_traffic_stream_all_pkt_loss    tg1    ds_mc_traffic1
