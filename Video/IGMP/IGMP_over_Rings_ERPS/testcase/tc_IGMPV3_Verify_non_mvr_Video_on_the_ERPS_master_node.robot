*** Settings ***
Documentation     (IGMPV3)Verify non mvr Video on the ERPS master node
Resource          ./base.robot
...
*** Test Cases ***
tc_IGMPV3_Verify_non_mvr_Video_on_the_ERPS_master_node    
    [Documentation]    Configure ERPS ring and IGMPv3 non-mvr on Topo1
    ...    1    Verify non-mvr Video on the ERPS master node	IPTV works fine
    [Tags]       @author=CindyGao     @TCID=AXOS_E72_PARENT-TC-1835    @globalid=2324427    @priority=P2    @user_interface=CLI    @eut=NGPON2-4    @ticket=21518    @eut=GPON-8r2
