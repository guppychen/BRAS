*** Settings ***
Documentation     (IGMPV3)Switch ERPS transit node and check mvr video
Resource          ./base.robot
...
*** Test Cases ***
tc_IGMPV3_Switch_ERPS_transit_node_and_check_mvr_video 
    [Documentation]    Configure ERPS ring and IGMPv3 mvr on Topo1
    ...    1    Switch ERPS transit node and check mvr video	IPTV works fine
    [Tags]       @author=CindyGao     @TCID=AXOS_E72_PARENT-TC-1841    @globalid=2324433    @priority=P2    @user_interface=CLI    @eut=NGPON2-4    @ticket=21518    @eut=GPON-8r2
