*** Settings ***
Documentation     (IGMPV2)Switch ERPS master node and check mvr video
Resource          ./base.robot
...
*** Test Cases ***
tc_IGMPV2_Switch_ERPS_master_node_and_check_mvr_video 
    [Documentation]    Configure ERPS ring and IGMPv2 mvr on Topo1
    ...    1    Switch ERPS master node and check mvr video	IPTV works fine
    [Tags]       @author=CindyGao     @TCID=AXOS_E72_PARENT-TC-1829    @globalid=2324421    @priority=P1    @user_interface=CLI    @eut=NGPON2-4    @eut=GPON-8r2
