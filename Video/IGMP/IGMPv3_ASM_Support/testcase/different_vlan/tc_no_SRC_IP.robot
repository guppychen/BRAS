...
*** Test Cases ***
tc_no_SRC_IP
    [Documentation]    1	Do an ASM join.	show mcast lists joined address.
    ...    2	V3 report: SRC IP = none and type is TO_IN	This is an ASM leave. Show mcast shows cleared address.
    [Tags]       @author=philip_chen     @TCID=AXOS_E72_PARENT-TC-2285    @GlobalID=2346552
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 Do an ASM join. show mcast lists joined address.

    log    STEP:2 V3 report: SRC IP = none and type is TO_IN This is an ASM leave. Show mcast shows cleared address.
    ...
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2285.pcap
    ...
    log    save captured file and analyze
    stop_capture    tg1    service_p1
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2285-1.pcap
    Tg Store Captured Packets   tg1    service_p1    ${save_file}
    log    save captured packets to ${save_file}
    sleep    ${wait_to_save_file}
    log    analyze imgp packet
    analyze_packet_count_greater_than    ${save_file}    ((igmp.record_type==3) && (vlan.id == @{p_video_vlan_list}[0])) && (ip.dst == 224.0.0.22)
...
case setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2285 setup


case teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2285 teardown
