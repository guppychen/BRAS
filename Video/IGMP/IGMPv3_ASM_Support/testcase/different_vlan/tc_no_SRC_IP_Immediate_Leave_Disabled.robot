...
*** Test Cases ***
tc_no_SRC_IP_Immediate_Leave_Disabled
    [Documentation]    1	Immediate Leave is disabled. V3 report: SRC IP = none	Report is treated as an ASM leave. Confirm two group specific query messages are sent in response (assuming the last-member-query count level is at default). Once the last max response time for the last query has expired the content for the channels is removed
    [Tags]       @author=philip_chen     @TCID=AXOS_E72_PARENT-TC-2286    @GlobalID=2346553
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 Immediate Leave is disabled. V3 report: SRC IP = none Report is treated as an ASM leave. Confirm two group specific query messages are sent in response (assuming the last-member-query count level is at default). Once the last max response time for the last query has expired the content for the channels is removed
    ...
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2286.pcap
    ...
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2286-1.pcap
...
case setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2286 setup
...
case teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2286 teardown
