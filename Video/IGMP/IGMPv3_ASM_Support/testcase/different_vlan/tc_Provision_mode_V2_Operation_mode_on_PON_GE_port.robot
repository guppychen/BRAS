...
*** Test Cases ***
tc_Provision_mode_V2_Operation_mode_on_PON_GE_port
    [Documentation]    1	Connect V2 router and V3 STB	V3 report will be discarded no V3 report forward to uplink. subscriber interface operation mode is V2.
    ...    2	Connect V3 router and V3 STB	V3 report will be discarded no V3 report forward to uplink. V3 query RX on E7 uplink port would be discarded
    [Tags]       @author=philip_chen     @TCID=AXOS_E72_PARENT-TC-2262    @GlobalID=2346529
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 Connect V2 router and V3 STB V3 report will be discarded no V3 report forward to uplink. subscriber interface operation mode is V2.

    log    STEP:2 Connect V3 router and V3 STB V3 report will be discarded no V3 report forward to uplink. V3 query RX on E7 uplink port would be discarded
    ...
    check_igmp_routers_sumarry_not_contain    eutA    @{p_video_vlan_list}[1]    ${service_model.service_point1.member.interface1}    V3    @{p_proxy_2.ip}[0]    ${p_igmp_querier2.ip}
    check_igmp_host_summary    eutA    @{p_video_vlan_list}[0]    subscriber_point1    V2    @{p_proxy_1.ip}[0]
    check_igmp_host_summary    eutA    @{p_video_vlan_list}[1]    subscriber_point2    V2    @{p_proxy_2.ip}[0]
    ...
    check_igmp_multicast_group_not_contain    eutA    @{p_mvr_start_ip_list}[0]     @{p_video_vlan_list}[1]    @{service_model.subscriber_point2.attribute.pon_port}[0]
    ...
    check_igmp_multicast_summary_not_contain    eutA    ${p_data_vlan}    ${service_model.subscriber_point2.member.interface1}    @{p_mvr_start_ip_list}[0]    @{p_video_vlan_list}[1]
...
case setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2262 setup
...
case teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2262 teardown
