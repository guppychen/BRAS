...
*** Test Cases ***
tc_VLAN_IGMP_proxy_version_V2
    [Documentation]    1	connect V2 router and add EUT sub	System working as IGMP v2 mode; traffic flowing fine with channel change works; retrieve mcast table and stats; it reflect the right entities. Uplink and subscriber interface show operation version as V2
    [Tags]       @author=philip_chen     @TCID=AXOS_E72_PARENT-TC-2259    @GlobalID=2346526
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 connect V2 router and add EUT sub System working as IGMP v2 mode; traffic flowing fine with channel change works; retrieve mcast table and stats; it reflect the right entities. Uplink and subscriber interface show operation version as V2
    ...
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2259.pcap
    ...
    log    save captured file and analyze
    stop_capture    tg1    service_p1
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2259_1.pcap
    Tg Store Captured Packets   tg1    service_p1    ${save_file}
    log    save captured packets to ${save_file}
    sleep    ${wait_to_save_file}
    log    analyze imgp packet
    analyze_packet_count_greater_than    ${save_file}    ((igmp) && (vlan.id == @{p_video_vlan_list}[0])) && (ip.dst == @{p_mvr_end_ip_list}[0])
...
case setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2259 setup


case teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2259 teardown
    tg control igmp    tg1    igmp_host1    leave
