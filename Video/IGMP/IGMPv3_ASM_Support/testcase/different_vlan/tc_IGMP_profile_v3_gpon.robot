...
*** Test Cases ***
tc_IGMP_profile_v3_gpon
    [Documentation]    1	Provision IGMP version as v3 and connect V3 router and STB on gpon subscriber port. The IGMP profile is proxy and using default value on other attribute	Video traffic flowing fine. Capture traffic and it follow the IGMP profile attribute setting like query interval etc
    [Tags]       @author=philip_chen     @TCID=AXOS_E72_PARENT-TC-2254    @GlobalID=2346521
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 Provision IGMP version as v3 and connect V3 router and STB on gpon subscriber port. The IGMP profile is proxy and using default value on other attribute Video traffic flowing fine. Capture traffic and it follow the IGMP profile attribute setting like query interval etc
    ...
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2254.pcap
...
case setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2254 setup


case teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2254 teardown
