...
*** Test Cases ***
tc_IGMP_auto_on_system_and_IGMP_v3_on_ONT
    [Documentation]    1	Connect V3 router and v3 STB on the ONT	Video traffic flowing fine the EUT with channel change work as expected.
    ...    2	Connect V2 router and V2 STB on the ONT	Video traffic flowing fine the EUT with channel change work as expected.
    [Tags]       @author=philip_chen     @TCID=AXOS_E72_PARENT-TC-2271    @@GlobalID=2346538
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1 Connect V3 router and v3 STB on the ONT Video traffic flowing fine the EUT with channel change work as expected.

    log    STEP:2 Connect V2 router and V2 STB on the ONT Video traffic flowing fine the EUT with channel change work as expected.
    ...
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2271.pcap
    ...
    log    delete the igmp query and igmp host
    ...
    log    change the igmp version to V2
    ...
    log    save captured file and analyze
    stop_capture    tg1    service_p1
    ${save_file}    set variable    ${p_tg_store_file_path}/TC-2271-1.pcap
    Tg Store Captured Packets   tg1    service_p1    ${save_file}
    log    save captured packets to ${save_file}
    sleep    ${wait_to_save_file}
    log    analyze imgp packet
...
case setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2271 setup


case teardown
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-22 teardown
    tg control igmp    tg1    igmp_host1    leave
    tg control igmp querier by name    tg1    igmp_querier1    stop
    tg delete igmp querier    tg1    igmp_querier1
    tg delete igmp    tg1    igmp_host1
