...
...               2 Verify the max response time in the group specific query is set to the last member query interval (default 1 second) and that the group specific queries generated after an IGMP leave report are sent at this interval. A total of [last member query count] group specific queries should be generated.
...               3 Modify the last member query interval to .5 seconds and the last member query count to 5 and verify the queries are generated in correct quantity and timing.
...               4 Modify the last member query interval to 5 seconds and the last member query count to 10 and verify the queries are generated in correct quantity and timing.
Resource          ./base.robot

*** Variables ***

*** Test Cases ***
tc_RFC2236_section_8_8_8_9_IGMPv2_Last_Member_query
    [Documentation]    1 Configure a full video service on the DUT using attached template as example. If the full service isn’t configured IGMP joins/leaves won’t be generated by the DUT. STC should use IGMPv2 only for this testing.
    ...    2 Verify the max response time in the group specific query is set to the last member query interval (default 1 second) and that the group specific queries generated after an IGMP leave report are sent at this interval. A total of [last member query count] group specific queries should be generated.
    ...    3 Modify the last member query interval to .5 seconds and the last member query count to 5 and verify the queries are generated in correct quantity and timing.
    ...    4 Modify the last member query interval to 5 seconds and the last member query count to 10 and verify the queries are generated in correct quantity and timing.
    [Tags]    @author=AnsonZhang    @tcid=AXOS_E72_PARENT-TC-1577    @globalid=2321651    @priority=P1    @eut=NGPON2-4    @eut=GPON-8r2
    [Setup]    case setup
    ...
    log    STEP:2 Verify the max response time in the group specific query is set to the last member query interval (default 1 second) and that the group specific queries generated after an IGMP leave report are sent at this interval. A total of [last member query count] group specific queries should be generated.
    ...
    Wait Until Keyword Succeeds    1min    10sec    subscriber_point_check_igmp_multicast_group    subscriber_point1    ${p_data_vlan}    @{p_groups_list}[0]
    ...    no
    log    sleep 10s to capture the general specific query
    sleep    10s
    log    stop capture and check the
    stop_capture    tg1    subscriber_p1
    stop_capture    tg1    service_p1
    log    verify the packets
    ...
    log    STEP:3 Modify the last member query interval to .5 seconds and the last member query count to 5 and verify the queries are generated in correct quantity and timing.
    prov_igmp_profile    eutA    ${p_igmp_prf}    last-member-query-count=5    last-member-query-interval=5
    log    start to capture the igmp at client
    start_capture    tg1    subscriber_p1
    start_capture    tg1    service_p1
    tg control igmp    tg1    igmp_host    join
    ...
    log    verify the packets
    ${tg_gsq}    save_and_analyze_packet_on_port    tg1    subscriber_p1    igmp.type==0x11&&igmp.maddr==@{p_groups_list}[0]
    Should Be true    ${tg_gsq}==5
    log    STEP:4 Modify the last member query interval to 5 seconds and the last member query count to 10 and verify the queries are generated in correct quantity and timing.
    prov_igmp_profile    eutA    ${p_igmp_prf}    last-member-query-count=8    last-member-query-interval=50
    log    start to capture the igmp at client
    start_capture    tg1    subscriber_p1
    start_capture    tg1    service_p1
    tg control igmp    tg1    igmp_host    join
    ...
    Wait Until Keyword Succeeds    1min    10sec    subscriber_point_check_igmp_multicast_group    subscriber_point1    ${p_data_vlan}    @{p_groups_list}[0]
    ...    no
    log    sleep 60s to capture the general specific query
    sleep    60s
    log    stop capture and check the
    stop_capture    tg1    subscriber_p1
    stop_capture    tg1    service_p1
    log    verify the packets
    ${tg_gsq}    save_and_analyze_packet_on_port    tg1    subscriber_p1    igmp.type==0x11&&igmp.maddr==@{p_groups_list}[0]
    Should Be true    ${tg_gsq}==8
    [Teardown]    case teardown

*** Keywords ***
case setup
    log    create the igmp profile
    prov_igmp_profile    eutA    ${p_igmp_prf}    general-query-interval=300    pbit-priority=4    last-member-query-count=2    last-member-query-interval=5
