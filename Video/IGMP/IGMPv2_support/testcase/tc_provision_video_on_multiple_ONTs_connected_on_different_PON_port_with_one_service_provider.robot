*** Settings ***
Documentation     Non MVR: provision video on multiple ONTs connected on different PON port with one service provider
...
...
*** Test Cases ***
tc_provision_video_on_multiple_ONTs_connected_on_different_PON_port_with_one_service_provider
    [Documentation]    Non MVR: provision video on multiple ONTs connected on different PON port with one service provider
    ...
    ...    Verify they all can view same channels and different channels at the same time.
    [Tags]    @author=AnsonZhang    @TCID=AXOS_E72_PARENT-TC-1601    @globalid=2321676    @priority=P2    @eut=NGPON2-4    @eut=GPON-8r2
    [Setup]    case setup
    log    STEP:Non MVR: provision video on multiple ONTs connected on different PON port with one service provider
    ...
    Wait Until Keyword Succeeds    1min    10sec    subscriber_point_check_igmp_multicast_group    subscriber_point3    ${p_data_vlan}    @{p_groups_list}[0]
    log    create traffic
...
case setup
    ...
    subscriber_point_add_svc_user_defined    subscriber_point3    ${p_data_vlan}    ${sub_policy_map}    mcast_profile=${p_mcast_prf}
...
case teardown
    ...
    subscriber_point_remove_svc_user_defined    subscriber_point3    ${p_data_vlan}    ${sub_policy_map}    mcast_profile=${p_mcast_prf}
