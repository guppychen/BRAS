*** Settings ***
Documentation     Mixed MVR and Non-MVR Video Service: Provision 1 access interfaces with non-MVR video service and 1 access interfaces with MVR video service. Each service allows a unique range of streams. Create all streams. IGMP Proxy mode is used.  For each access interface, attempt to join all created streams. -> Streams are forwarded on only if the line is associated with that service. All other streams not forwarded. [Note: DSL lines should be a mixture of unbonded and bonded. Results should contain description of lines used.]
Resource          ./base.robot
...
*** Test Cases ***
tc_Mixed_MVR_and_Non_MVR_Video_Service
    [Documentation]    Mixed MVR and Non-MVR Video Service: Provision 1 access interfaces with non-MVR video service and 1 access interfaces with MVR video service. Each service allows a unique range of streams. Create all streams. IGMP Proxy mode is used.  For each access interface, attempt to join all created streams. -> Streams are forwarded on only if the line is associated with that service. All other streams not forwarded. [Note: DSL lines should be a mixture of unbonded and bonded. Results should contain description of lines used.]
    [Tags]       @author=AnsonZhang     @TCID=AXOS_E72_PARENT-TC-1673    @globalid=2321762    @priority=P1    @eut=NGPON2-4    @eut=GPON-8r2
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:Mixed MVR and Non-MVR Video Service: Provision 1 access interfaces with non-MVR video service and 1 access interfaces with MVR video service. Each service allows a unique range of streams. Create all streams. IGMP Proxy mode is used. For each access interface, attempt to join all created streams. -> Streams are forwarded on only if the line is associated with that service. All other streams not forwarded. [Note: DSL lines should be a mixture of unbonded and bonded. Results should contain description of lines used.]
...
case setup
    ...
    ...    ${p_igmp_querier1.gateway}    ${p_mvr_vlan}    query_interval=60
