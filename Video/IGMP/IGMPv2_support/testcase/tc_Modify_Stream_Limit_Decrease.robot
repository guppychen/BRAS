*** Settings ***
Documentation     Modify Stream Limit Decrease: Add basic video service to the access interfce using any stream limit value and very low query interval. Proxy IGMP mode in use. Actively join the limit +1 streams. Decrease the limit by one. Stop all active joins and wait for query interval. Restart active joins. Continue to decrease limit process several additional times. -> The streams forwarded are limited to the provisioned limit decreasing as the limit decreases. Note: As another way to verify the service bandwidth requirement implementation differences between GPON and DSL GPON scripts should provision bw-profile < total multicast bandwidth required and DSL scripts should be provisioned with => total multicast bandwidth required.
Resource          ./base.robot
...
*** Test Cases ***
tc_Modify_Stream_Limit_Decrease
    [Documentation]    Modify Stream Limit Decrease: Add basic video service to the access interfce using any stream limit value and very low query interval. Proxy IGMP mode in use. Actively join the limit +1 streams. Decrease the limit by one. Stop all active joins and wait for query interval. Restart active joins. Continue to decrease limit process several additional times. -> The streams forwarded are limited to the provisioned limit decreasing as the limit decreases. Note: As another way to verify the service bandwidth requirement implementation differences between GPON and DSL GPON scripts should provision bw-profile < total multicast bandwidth required and DSL scripts should be provisioned with => total multicast bandwidth required.
    [Tags]       @author=AnsonZhang     @TCID=AXOS_E72_PARENT-TC-1643    @globalid=2321718    @priority=P1    @eut=NGPON2-4    @eut=GPON-8r2
    [Setup]      case setup
    log    STEP:Modify Stream Limit Decrease: Add basic video service to the access interfce using any stream limit value and very low query interval. Proxy IGMP mode in use. Actively join the limit +1 streams. Decrease the limit by one. Stop all active joins and wait for query interval. Restart active joins. Continue to decrease limit process several additional times. -> The streams forwarded are limited to the provisioned limit decreasing as the limit decreases. Note: As another way to verify the service bandwidth requirement implementation differences between GPON and DSL GPON scripts should provision bw-profile < total multicast bandwidth required and DSL scripts should be provisioned with => total multicast bandwidth required.
