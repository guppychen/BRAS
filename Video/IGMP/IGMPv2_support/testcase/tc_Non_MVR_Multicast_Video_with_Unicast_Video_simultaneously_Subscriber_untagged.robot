*** Settings ***
Documentation     Non-MVR Multicast Video with Unicast Video simultaneously (Subscriber untagged): Add basic video service to ONT/xDSL line. Generate bidirectional unicast traffic at some rate lower than the bw-profile. Total DS bandwidth provisioned on a DSL interface includes both unicast and multicast video where as total DS bandwidth on GPON ONT includes only unicast video bandwidth. Join a multicast channel that makes total bw forwarded downstream slightly lower than the bw-profile. -> Both the bi-directional unicast traffic and the downstream multicast stream are forwarded with no losses.
Resource          ./base.robot

*** Variables ***

*** Test Cases ***
tc_Non_MVR_Multicast_Video_with_Unicast_Video_simultaneously_Subscriber_untagged
    [Documentation]    Non-MVR Multicast Video with Unicast Video simultaneously (Subscriber untagged): Add basic video service to ONT/xDSL line. Generate bidirectional unicast traffic at some rate lower than the bw-profile. Total DS bandwidth provisioned on a DSL interface includes both unicast and multicast video where as total DS bandwidth on GPON ONT includes only unicast video bandwidth. Join a multicast channel that makes total bw forwarded downstream slightly lower than the bw-profile. -> Both the bi-directional unicast traffic and the downstream multicast stream are forwarded with no losses.
    [Tags]    @author=AnsonZhang    @TCID=AXOS_E72_PARENT-TC-1653    @globalid=2321729    @priority=P2    @eut=NGPON2-4    @eut=GPON-8r2
    [Setup]    case setup
    ...
    create_raw_traffic_udp    tg1    us    service_p1    subscriber_p1    mac_dst=${p_igmp_querier.mac}    mac_src=${p_igmp_host.mac}
    ...    ip_dst=${p_igmp_querier.ip}    ip_src=${p_igmp_host.ip}    rate_mbps=${rate_mbps1}
...
case setup
    ...
    ...    ${p_igmp_host.gateway}    session=${p_igmp_group_session_num}    mc_group_start_ip=@{p_groups_list}[0]
    log    create query
