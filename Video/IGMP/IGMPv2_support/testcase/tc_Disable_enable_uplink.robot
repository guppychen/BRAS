*** Settings ***
Documentation     Disable and then enable uplink port and verify video service restore.
Resource          ./base.robot

*** Variables ***

*** Test Cases ***
tc_Disable_enable_uplink
    [Documentation]    Disable and then enable uplink port and verify video service restore.
    [Tags]    @author=AnsonZhang    @TCID=AXOS_E72_PARENT-TC-1609    @globalid=2321684    @priority=P1    @eut=NGPON2-4    @eut=GPON-8r2
    [Setup]    case setup
    log    STEP:Disable and then enable uplink port and verify video service restore.
    ...
    no_shutdown_port    eutA    ${service_model.service_point1.type}    ${service_model.service_point1.member.interface1}
    Wait Until Keyword Succeeds    20sec    3sec    check_interface_up    eutA    ${service_model.service_point1.type}    ${service_model.service_point1.member.interface1}
    log    check the mrouter
    Wait Until Keyword Succeeds    1min    10sec    service_point_check_igmp_routers    service_point1    ${p_data_vlan}    @{p_proxy.ip}[0]
