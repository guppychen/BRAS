...
*** Test Cases ***
tc_clear_igmp_statistics_on_vlan
    [Documentation]       Test suite verify clear igmp statistics on vlan
    ...    1	retrieve the igmp counters on vlan	successful
    ...    2    clear the igmp counters on vlan	successful
    ...    3	retrieve the igmp counters on vlan again	statistic on the vlan is 0
    [Tags]    @feature=IGMP    @subfeature=IGMP Proxy    @author=CindyGao    @globalid=2276052    @tcid=AXOS_E72_PARENT-TC-536
    ...
    log    STEP:1 retrieve the igmp counters on vlan successful
    ...
    log    STEP:2 clear the igmp counters on vlan successful
    clear_igmp_statistics    eutA    vlan    ${p_video_vlan}
    
    log    STEP:3 retrieve the igmp counters on vlan again, statistic on the vlan is 0
    check_igmp_statistics    eutA    vlan    ${p_video_vlan}
