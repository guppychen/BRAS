*** Settings ***
Documentation     Test suite retrieve igmp statistics
Resource          ./base.robot
...
*** Test Cases ***
tc_retrieve_igmp_statistics
    [Documentation]       Test suite retrieve igmp statistics
    ...    1	retrieve the igmp counters on system	successful		
    ...    2	retrieve the igmp counters on vlan	successful		
    ...    3	retrieve the igmp counters on interface	successful
    [Tags]    @feature=IGMP    @subfeature=IGMP Proxy    @author=CindyGao    @globalid=2276054    @tcid=AXOS_E72_PARENT-TC-538
    ...
    ${stat_vlan}    check_igmp_statistics    eutA    vlan    ${p_video_vlan}
    ...
    log    STEP:1 retrieve the igmp counters on system successful
    ...
    log    STEP:2 retrieve the igmp counters on vlan successful
    # check_igmp_statistics    eutA    vlan    ${p_video_vlan}
    check_cmd_result    ${stat_vlan}
    ...    rx-reports=${tg_report}    rx-leaves=${tg_leave}    rx-general-queries=${tg_gen_query}    rx-group-queries=${tg_grp_query}
    ...    tx-reports=${eut_report}    tx-leaves=${eut_leave}    tx-general-queries=${eut_gen_query}    tx-group-queries=${eut_grp_query}

    log    STEP:3 retrieve the igmp counters on interface successful (According to EXA-18133, this feature doesn't support)
