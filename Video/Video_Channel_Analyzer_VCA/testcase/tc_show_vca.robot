...
tc_show_vca
    ...
    [Tags]     @author=YUE SUN    @tcid=AXOS_E72_PARENT-TC-4578      @globalid=2533302      @priority=P2      @eut=NGPON2-4    @eut=GPON-8r2    @eut=10GE-12    @eut=GE-24r2          @user_interface=CLI    
    [Setup]     case_setup
    ...
    log    STEP 1: show vca, check vca parameter display correctly 
    ...
    log    STEP 2: Clear vca counters, successfully
    cli    eutA    clear vca counters all
    Wait Until Keyword Succeeds    2min    5s    check_vca_rx_packets    eutA    none    rx-packets=0    admin-state=Disabled
