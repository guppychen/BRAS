...
*** Test Cases ***
tc_start_vca_snoop_duration_changed
    [Documentation]    check it can works fine  
    [Tags]     @author=YUE SUN    @tcid=AXOS_E72_PARENT-TC-4573      @globalid=2533297      @priority=P2      @eut=NGPON2-4    @eut=GPON-8r2    @eut=10GE-12    @eut=GE-24r2          @user_interface=CLI      
    [Setup]     case_setup
    [Teardown]     case_teardown
    
    log    STEP 1: start vca snoop ip/vlan
    ...
    log    start vca snoop ip with flow, start traffic,    
    ...
    log    STEP 2: start vca snoop ip/vlan, duration none
    ...
    log    start vca snoop ip with flow, duration change
    cli    eutA    start vca snoop ip ${vca_ip} vlan ${service_vlan}
