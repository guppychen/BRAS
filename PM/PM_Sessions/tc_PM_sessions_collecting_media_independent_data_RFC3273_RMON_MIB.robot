*** Settings ***
Documentation     Test suite verifies ethernet interface supports collection of PM session Media Independent Table data defined in RFC 3273 RMON MIB for High Capacity Packet and Octet counters and its Utilization Statistics  
Resource          base.robot
...
TC1: Verify PM session Media Independent Table data defined in RFC 3273 RMON MIB and its Utilization Statistics of bins are collected 

     [Documentation]    Test case verifies PM sessions Media Independent Table data defined in RFC 3273 RMON MIB and Utilization Statistics of bins are collected
     ...                1.  Create 100 K Frames of traffic stream for each attributes defined in RFC 3273 RMON MIB 
     ...
     ...                3.  Verify packet counters of each attributes defined in RFC 3273 RMON MIB have 30000000 in the bins

     [Tags]    @globalid=2201642    @tcid=AXOS_E72_PARENT-TC-33    @priority=P3    @functional   @eut=NGPON2-4    @user_interface=CLI     @jira=EXA-17081
     ...
     Run Keyword And Continue On Failure    Verify Media Independent Data RFC 3273 RMON MIB     n1    ${DEVICES.n1.ports.p1.port}
