*** Settings ***
Documentation     Test suite verifies ethernet interface supports collection of PM session Ifentry data defined in the RFC 2863 Interfaces MIB with the High Capacity Counter Extensions and its Utilization Statistics  
Resource          base.robot
...
TC1: Verify PM session Ifentry data defined in the RFC 2863 Interfaces MIB and its Utilization Statistics of bins are collected 

     [Documentation]    Test case verifies PM sessions Ifentry data defined in the RFC 2863 Interfaces MIB and Utilization Statistics of bins are collected
     ...                1.  Create 100 K Frames of traffic stream for each attributes defined in RFC 2863 Interfaces MIB 
     ...
     ...                3.  Verify packet counters of each attributes defined in RFC 2863 Interfaces MIB have 30000000 in the bins

     [Tags]    @globalid=2201643    @tcid=AXOS_E72_PARENT-TC-34    @priority=P3    @functional   @eut=NGPON2-4    @user_interface=CLI   @jira=EXA-17078
     ...
     Run Keyword And Continue On Failure     Verify Ifentry Data RFC 2863 Interfaces MIB     n1    ${DEVICES.n1.ports.p1.port}
