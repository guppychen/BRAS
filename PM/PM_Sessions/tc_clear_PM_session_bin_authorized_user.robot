*** Settings ***
Documentation     Test suite verifies the users with appropriate permission to clear PM session bins and the same should be reported and recorded in syslog
Resource          base.robot
...
TC1: Verify user with appropriate permissions must be able to clear the PM Session Current and Historcal Bins

     [Documentation]    Test case verifies user with appropriate permissions clears the bins
     ...                1. Configure a Grade of Service profile (GoS) with specific attributes and verify its is successful
     ...                2. Configure the user with valid role to clear the bins
     ...
     ...                6. Clear PM session current bin and verify current bin got cleared and reported in management plane
     ...                7. Clear PM session all bin and verify all bin got cleared and reported in management plane

     [Tags]    @globalid=2201637    @tcid=AXOS_E72_PARENT-TC-28    @priority=P3    @functional   @eut=NGPON2-4    @user_interface=CLI
     ...
     Log  ****** Clear current bin and verify it got cleared and reported to management plane. Verify counters wraps to zero and starts counting again ********
...
PM Teardown
    [Documentation]    teardown
     Clear Current Bin Authorized User    n1_auth    ${DEVICES.n1.ports.p1.port}
     Clear All Bin Authorized User    n1_auth    ${DEVICES.n1.ports.p1.port}
