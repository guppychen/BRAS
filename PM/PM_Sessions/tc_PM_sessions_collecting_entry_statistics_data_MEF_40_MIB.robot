*** Settings ***
Documentation     Test suite verifies ethernet interface supports collection of PM session Entry Statistics data defined in the MEF 40 MIB for High Capacity Packet and Octet counters its Utilization Statistics  
Resource          base.robot
...
TC1: Verify PM session Entry Statistics data defined in MEF 40 MIB for High Capacity Packet counters and its Utilization Statistics of bins are collected 

     [Documentation]    Test case verifies PM sessions etherStats data defined in MEF 40 MIB for High Capacity Packet and Utilization Statistics of bins are collected
     ...
     ...                3.  Verify packet counters of each attributes defined in MEF 40 MIB have 30000000 in the bins

     [Tags]    @globalid=2201644    @tcid=AXOS_E72_PARENT-TC-35    @priority=P3    @functional   @eut=NGPON2-4    @user_interface=CLI    @jira=EXA-17078
     ...
     Tg Create Untagged Stream On Port    tg1    stream12   p1    p2    mac_src=${src_mac}    mac_dst=${vlan_mac}    l3_protocol=ipv4
     ...                                  ip_src_addr=${stc_pon_ip}    ip_dst_addr=${unknown_ip}     rate_pps=10    frame_size=1040  length_mode=fixed
     ...
     Run Keyword And Continue On Failure     Verify Entry Statistics Data MEF 40 MIB     n1    ${DEVICES.n1.ports.p1.port}
