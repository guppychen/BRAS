*** Settings ***
Documentation     Test suite verifies the users with out appropriate permission to clear PM session bins and should not be allowed to clear
Resource          base.robot
...
TC1: Verify user with out appropriate permissions must not clear the PM Session Current and Historcal Bins

     [Documentation]    Test case verifies user with out appropriate permissions is not allowed to clear the bins
     ...                1. Configure a Grade of Service profile (GoS) with specific attributes and verify its is successful
     ...                2. Configure the user with out valid role to clear the bins
     ...
     ...                6. Clear PM session bins and verify bins are not cleared and command got rejected

     [Tags]    @globalid=2201649    @tcid=AXOS_E72_PARENT-TC-40    @priority=P3    @functional   @eut=NGPON2-4    @user_interface=CLI
     ...
     Log  ****** Clear PM session bins and verify bins are not cleared and command got rejected ********
...
PM Teardown
    [Documentation]    teardown
     Clear Bin Unauthorized User    n1_unauth    ${DEVICES.n1.ports.p1.port}
