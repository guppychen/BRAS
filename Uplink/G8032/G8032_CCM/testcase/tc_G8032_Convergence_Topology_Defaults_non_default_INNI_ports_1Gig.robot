*** Settings ***
Documentation     The purpose of this test is to verify CFM autodiscovery works on 1 Gig INNI ports (other than the default discovery manager supported ports).
...               1.Config G8032 ring(1G)
...
*** Test Cases ***
tc_G8032_Convergence_Topology_Defaults_non_default_INNI_ports_1Gig.robot
    [Tags]    @tcid=AXOS_E72_PARENT-TC-2029    @globalid=2329421    @eut=NGPON2-4    @feature=G8032    @subfeature=G8032    @author=pzhang
    [Documentation]     The purpose of this test is to verify CFM autodiscovery works on 1 Gig INNI ports (other than the default discovery manager supported ports).
    ...               1.Config G8032 ring(1G)
    ...               2.config CCM auto to G8032 ring interface.
    ...               3.Check G8032 ring state(idle) and meg status
    [Setup]    setup
    log    check (1G)g8032 status
