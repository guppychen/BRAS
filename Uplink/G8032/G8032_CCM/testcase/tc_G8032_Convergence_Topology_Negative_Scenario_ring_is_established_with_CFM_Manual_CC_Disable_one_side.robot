*** Settings ***
Documentation     The purpose of this test is to verify the scenario where a ring is established with CFM and then CCM is disabled on one side.
...               1.Check g8032 ring with ccm manual
...               2.disable one side mep and check g8032 status
...
...
*** Test Cases ***
tc_G8032_Convergence_Topology_Negative_Scenario_ring_is_established_with_CFM_Manual_CC_Disable_one_side.robot
    [Tags]       @tcid=AXOS_E72_PARENT-TC-2023     @globalid=2329414    @eut=NGPON2-4    @feature=G8032    @subfeature=G8032    @author=pzhang
    [Documentation]     The purpose of this test is to verify the scenario where a ring is established with CFM and then CCM is disabled on one side.
    ...               1.Check g8032 ring with ccm manual
    ...               2.disable one side mep and check g8032 status
    [Setup]      setup
    ...
    log     check g8032 the peer side alarm setting
