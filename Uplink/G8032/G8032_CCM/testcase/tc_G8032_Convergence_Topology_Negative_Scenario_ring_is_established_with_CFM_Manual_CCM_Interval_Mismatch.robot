*** Settings ***
Documentation     The purpose of this test is to verify the scenario where a ring is established with CFM and then the CCM interval doesn't match.
...               1.Check g8032 ring with ccm manual
...               2.set incorrect  mep ccm time and check g8032 status
...
...
*** Test Cases ***
tc_G8032_Convergence_Topology_Negative_Scenario_ring_is_established_with_CFM_Manual_CCM_Interval_Mismatch.robot
    [Tags]       @tcid=AXOS_E72_PARENT-TC-2022     @globalid=2329413    @eut=NGPON2-4    @feature=G8032    @subfeature=G8032    @author=pzhang
    [Documentation]     The purpose of this test is to verify the scenario where a ring is established with CFM and then the CCM interval doesn't match.
    ...               1.Check g8032 ring with ccm manual
    ...               2.set incorrect  mep ccm time and check g8032 status
    [Setup]      setup
