...
*** Variables ***
*** Test Cases ***
tc_change_role_of_node_in_erps_domain_measure_services_recovery
    [Documentation]    1	set master node to transit node, transit node to master node,services recover   
    ...      2	show erps-domain to check admin status and operational status to confirm erps-doamin returns to nomal,Successfully
    [Tags]       @tcid=AXOS_E72_PARENT-TC-1307    @globalid=2319057    @subfeature=ERPS    @priority=P1    @eut=NGPON2-4    @eut=GPON8-R2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP 1 set master node to transit node, transit node to master node,services recover
    ...
    Tg Create single Tagged Stream On Port    tg1    raw_downstream1    subscriber_p1      service_p1    vlan_id=${service_vlan}    vlan_user_priority=0    frame_size=512    length_mode=fixed
    ...    mac_src=${dmac}    mac_dst=${smac}    rate_pps=${rate_pps}    l3_protocol=ipv4    ip_src_addr=${dip}    ip_dst_addr=${sip}
    ...    l4_protocol=udp    mac_src_count=400    mac_src_mode=increment
    ...
    prov_erps_ring    eutA    6    transit
    prov_erps_ring    eutB    6    master
    wait until keyword succeeds    3 min    5 sec    check_erps_ring_up    eutB    6
    ...
    log    step 2 show erps-domain to check admin status and operational status to confirm erps-doamin returns to nomal,Successfully
    check_erps_ring_status    eutB    6   admin-state=enable    acting-role=master
...
teardown
    ...
    prov_erps_ring    eutA    6    master
    prov_erps_ring    eutB    6    transit
    wait until keyword succeeds    3 min    5 sec    check_erps_ring_up    eutA    6
