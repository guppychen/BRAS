...
*** Variables ***
*** Test Cases ***
tc_Topo1_run_known_unicast_traffic_do_erps_switch_measure_switch_time_and_service_recover
    [Documentation]    1	show erps ring status	Correct
    ...    2	run l2 kown traffic using STC, with mac 400	Verify traffic flow between each node.
    ...    3	disable forwarding interface on master node	traffis loss <1s,and noop
    ...    4	show erps ring status	Correct
    ...    5	enable interface	alarm cleared ,and erps-domain switchback , traffic loss <1s,and no loop
    ...    6	show erps ring status	Recover
    [Tags]        @tcid=AXOS_E72_PARENT-TC-1261    @globalid=2319011    @subfeature=ERPS    @priority=P1    @eut=NGPON2-4    @eut=GPON8-R2
    [Setup]      setup
    [Teardown]   teardown
    log    STEP:1 show erps ring status Correct
    check_erps_ring_status    ${service_model.service_point1.device}    ${service_model.service_point1.name}    primary-interface-fwd-state=forwarding

    log    STEP:2 run l2 kwown traffic using STC, with mac 400 Verify traffic flow between each node.
    ...
    log    STEP:3 disable forwarding interface on master node traffis loss <1s,and loop
    ...
    log    STEP:4 show erps ring status Correct
    ...
    log    STEP:5 enable interface alarm cleared ,and erps-domain switchback , traffic loss <1s,and no loop
    Tg Clear Traffic Stats    tg1
    Tg Start All Traffic    tg1
    ...
    log    STEP:6 show erps ring status Recover
