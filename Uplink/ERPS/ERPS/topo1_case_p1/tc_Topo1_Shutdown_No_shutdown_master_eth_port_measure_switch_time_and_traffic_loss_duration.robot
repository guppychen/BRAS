...
*** Test Cases ***
tc_Topo1_Shutdown_No_shutdown_master_eth_port_measure_switch_time_and_traffic_loss_duration
    [Documentation]    1	Shutdown forwarding eth-port on master node, measure switch time	no more than 250ms, and traffic loss is reasonable
    ...    2	No shutdown eth-port	erps switch back ,and service recover
    [Tags]       @tcid=AXOS_E72_PARENT-TC-1291    @globalid=2319041    @subfeature=ERPS    @priority=P1    @eut=NGPON2-4    @eut=GPON8-R2
    [Setup]       setup
    [Teardown]    teardown
    log    STEP:1 Shutdown forwarding eth-port on master node, measure switch time no more than 250ms, and traffic loss is reasonable
