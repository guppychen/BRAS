...
*** Test Cases ***
tc_Topo1_run_DHCPv4_traffic_reboot_forwarding_card_measure_switch_time_and_service_recover
    [Documentation]    1	config data service, run data services	can get DHCP lease, dhcp bounded traffic is Ok
    ...    2	reboot forwarding card, wait until erps back to complete	check packet loss
    [Tags]       @tcid=AXOS_E72_PARENT-TC-2715    @globalid=2362701    @subfeature=ERPS    @priority=P1    @eut=NGPON2-4    @eut=GPON8-R2
    [Setup]      setup
    [Teardown]   teardown
    ...
    log    STEP:2 reboot forwarding card, wait until erps back to complete check packet loss
    Reload System    eutB
