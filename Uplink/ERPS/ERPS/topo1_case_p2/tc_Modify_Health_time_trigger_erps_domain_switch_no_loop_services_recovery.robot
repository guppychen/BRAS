...
*** Test Cases ***
tc_Modify_Health_time_trigger_erps_domain_switch_no_loop_services_recovery
    [Documentation]    1	set Health-time in the range of 1-10	Execute command successfully
    ...    2	reboot card to trigger erps-domain switch	no loop, and services recover
    [Tags]        @tcid=AXOS_E72_PARENT-TC-1305    @globalid=2319055    @subfeature=ERPS    @priority=P2    @eut=NGPON2-4    @eut=GPON8-R2
    [Setup]       setup
    [Teardown]    teardown
    log    STEP:1 set Health-time in the range of 1-10 Execute command successfully
    prov_erps_ring    eutA    6    health-time=7
    ...
    log    STEP:2 reboot card to trigger erps-domain switch no loop, and services recover    
    Reload System    eutB
