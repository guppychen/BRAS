...
*** Test Cases ***
tc_Topo2_run_unknown_unicast_traffic_do_erps_switch_measure_switch_time_and_service_recover
    [Documentation]    1	run l2 unkown traffic using Ixia with 400 mac	Verify traffic flow between each node.
    ...
    [Tags]        @tcid=AXOS_E72_PARENT-TC-1277    @globalid=2319027    @subfeature=ERPS    @priority=P2    @eut=NGPON2-4    @eut=GPON8-R2
    [Setup]       setup
    [Teardown]    teardown
   
    log    STEP:1 run l2 unkown traffic using Ixia with 400 mac Verify traffic flow between each node.
