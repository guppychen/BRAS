...
*** Test Cases ***
tc_mixture_erps_and_g8032_run_unicast_traffic_switch_g8032_measure_service_recover
    [Documentation]    1	Run unicast  traffic, Traffic run normally
    ...    2	switch erps	doesn't affect g8032 and services recover
    [Tags]       @tcid=AXOS_E72_PARENT-TC-2944    @globalid=2392001    @subfeature=ERPS    @priority=P2    @eut=NGPON2-4    @eut=GPON8-R2
    [Setup]      case setup
    ...
    Tg Start All Traffic     tg1
    sleep    ${traffic_run_time}
    shutdown_port    eutC    ethernet    ${service_model.service_point8.member.interface1}
    no_shutdown_port    eutC    ethernet    ${service_model.service_point8.member.interface1}
    wait until keyword succeeds    3 min    5 sec    check_interface_up    eutC    ethernet    ${service_model.service_point8.member.interface1}
