...
# ---------------------------------------------------------------------------------------------------------------
# Function   : TC1
# Synopsis   : Link Protection/LAG/Distribution Algorithms/Link Distribution based on Source and Destination IP
# Return     : Pass/Fail
# ---------------------------------------------------------------------------------------------------------------
...
Veopy E5 device running configuration data *****
    ...
    ...    13 Configure LAG Hash method as SRC_DST_IP for the LAG group.
    ...    14 Verify LAG Hash method as SRC_DST_IP for the LAG group
    ...
    ...    25 Verify LAG Hash method as SRC_DST_IP using traffic.
    ...    26 Starting traffic
    ...
    [Tags]       @tcid=AXOS_E72_PARENT-TC-1058    @globalid=2316547    @smoke   @priority=P1
...
#***** 14. Verify LAG Hash method as SRC_DST_IP for the LAG group*****
    ...
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Hash Method      n1     ${lag_group}     src-dst-ip
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Hash Method      n1     ${lag_group}     src-dst-ip
...
#***** 19. Verify LAG Hash method as SRC_DST_IP using traffic*****
    Tg Create Single Tagged Stream On Port          tg1     up_1        p2          p1        vlan_id=500
       ...  vlan_user_priority=7            mac_src=00:00:01:00:00:01          mac_dst=00:00:02:00:00:01
            ...  l3_protocol=ipv4       ip_src_addr=192.168.1.1
    ...    ip_src_count=100         ip_src_mode=increment
     ...  ip_dst_addr=192.168.1.111       rate_percent=${rate_percent_p1}    frame_size=512    length_mode=fixed
...
#***** 25. Verify LAG Hash method as SRC_DST_IP using traffic*****
    Tg Create Single Tagged Stream On Port          tg1     up_2        p2          p1          vlan_id=500     vlan_user_priority=7            mac_src=00:00:01:00:00:01       mac_dst=00:00:02:00:00:01            l3_protocol=ipv4       ip_src_addr=192.168.1.1          ip_dst_addr=192.168.1.11       ip_dst_count=100
    ...     ip_dst_mode=increment       rate_percent=${rate_percent_p1}    frame_size=1024         length_mode=fixed
