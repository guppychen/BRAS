...
# ---------------------------------------------------------------------------------------------------------------
# Function   : TC1
# Synopsis   : Link Protection/LAG/Distribution Algorithms/Link Distribution based on Source IP.
# Return     : Pass/Fail
# ---------------------------------------------------------------------------------------------------------------
...
Verify E5 device running configuration data *****
    ...
    ...    9 Configure LAG Hash method as SRC_IP for the LAG group*****dd Transport Service Profile To The Lag Group
    ...
    ...    13 Configure LAG Hash method as SRC_IP for the LAG group.
    ...    14 Verify LAG Hash method as SRC_IP for the LAG group
    ...
    ...    19 Verify LAG Hash method as SRC_IP using traffic.\
    ...    20 Starting traffic.
    ...
    [Tags]       @tcid=AXOS_E72_PARENT-TC-1069    @globalid=2316558    @smoke   @priority=P1
...
#***** 13. Configure LAG Hash method as SRC_IP for the LAG group *****
    ...
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Configure LAG Group Hash Method      n1     ${lag_group}     src-ip
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Configure LAG Group Hash Method      n2     ${lag_group}     src-ip

#***** 14. Verify LAG Hash method as SRC_IP for the LAG group*****

    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Hash Method      n1     ${lag_group}     src-ip
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Hash Method      n1     ${lag_group}     src-ip
...
#***** 19. Verify LAG Hash method as SRC_IP using traffic*****
    Tg Create Single Tagged Stream On Port          tg1     up_1        p2          p1        vlan_id=500     vlan_user_priority=7            mac_src=00:00:01:00:00:01          mac_dst=00:00:02:00:00:01            l3_protocol=ipv4       ip_src_addr=192.168.1.1       ip_src_count=100         ip_src_mode=increment          ip_dst_addr=192.168.1.111       rate_percent=${rate_percent_p1}     frame_size=512         length_mode=fixed
