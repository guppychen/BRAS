...
Verify LINKAGG-R-210 Calix Link Aggregation must support a command allowing for a LinkAgg group containing one member to be converted into a single Ethernet link,deleting the group but maintaining the services on it.
    ...
    [Tags]         @tcid=AXOS_E72_PARENT-TC-1114    @globalid=2316620    @functional    @priority=P2
    ...
    :For     ${num}     IN RANGE       1       3
         \    Add Device Interface To LAG      n${num}     ${DEVICES.n${num}.ports.p1.port}      ${lag_group}    ${slot}


     #Log *** Verify ethernet interface of device added to the LAG group ***
    :For     ${num}     IN RANGE       1       3
         \    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Interface Added To LAG      n${num}     ${DEVICES.n${num}.ports.p1.port}      ${lag_group}
   ...
   :For     ${num}     IN RANGE       1       3
         \    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check LAG Group Member Status      n${num}     ${lag_group}     ${DEVICES.n${num}.ports.p1.port}        ${oper_state1}      static
    ...
    :For     ${num}     IN RANGE       1       3
          \     Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Lag Interface Traffic Utilization Without Traffic Static      n${num}     ${lag_group}     ${DEVICES.n${num}.ports.p1.port}
    ...
    sleep   10
     ...
     Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Run Keyword And Continue On Failure     Verify Lag Interface Traffic TX RX Utilization Status    n1    n2     ${lag_group}     ${DEVICES.n1.ports.p1.port}      ${DEVICES.n2.ports.p1.port}     static      ${value1}
...
Basic_Test_Teardown
    ...
    [Arguments]     
    #Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Verify Lag Interface Traffic TX RX Utilization Status    n1    n2     ${lag_group}     ${DEVICES.n1.ports.p1.port}      ${DEVICES.n2.ports.p1.port}     static      ${value1}
    ...
    :For     ${num}     IN RANGE       1       3
         \    Unconfigure LAG From Device Interface     n${num}     ${DEVICES.n${num}.ports.p1.port}      ${lag_group}    ${slot}

    :For     ${num}     IN RANGE       1       3
         \    Unconfigure LAG From Device Interface     n${num}     ${DEVICES.n${num}.ports.p3.port}      ${lag_group}    ${slot}
