...
Veopy E5 device running configuration data *****
    ...
    ...    13 Configure LAG Hash method as SRC_DST_MAC for the LAG group.
    ...    14 Verify LAG Hash method as SRC_DST_MAC for the LAG group
    ...
    ...    19 Verify LAG Hash method as SRC_DST_MAC using traffic.\
    ...    20 Starting traffic.
    ...    21 Check Traffic utilization.
    ...    22 Clear Lag interface Counters
    ...    23 Check Lag Interface Tx and Rx Counters Without Traffic.
    ...    24 Check Traffic utilization.
    ...    25 Verify LAG Hash method as SRC_DST_MAC using traffic.
    ...    26 Starting traffic
    ...    27 Check Traffic utilization.
    ...    28 Unconfigure hash-method.
    ...    29 Remove transport service profile from interface.
    ...    30 Remove transport service profile from lag.
    ...    31 Remove interface from lag.
    ...    32 Remove transport service profile.

    [Tags]       @tcid=AXOS_E72_PARENT-TC-1030    @globalid=2316516    @smoke   @priority=P1
...
#***** 13. Configure LAG Hash method as SRC_DST_MAC for the LAG group *****
    ...
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Configure LAG Group Hash Method      n1     ${lag_group}     src-dst-mac
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Configure LAG Group Hash Method      n2     ${lag_group}     src-dst-mac

#***** 14. Verify LAG Hash method as SRC_DST_MAC for the LAG group*****

    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Hash Method      n1     ${lag_group}     src-dst-mac
    Wait Until Keyword Succeeds    60 Seconds    5 Seconds    Check Hash Method      n1     ${lag_group}     src-dst-mac
...
#STEP 1 (19-24)
#Check for upstream traffic where source mac is incremented and destination mac is same
...
#***** 19. Verify LAG Hash method as SRC_DST_MAC using traffic*****
    Tg Create Single Tagged Stream On Port          tg1     up_1        p2          p1        vlan_id=500
     ...  vlan_user_priority=7            mac_src=00:00:01:00:00:01       mac_src_count=100
     ...    mac_src_mode=increment          mac_dst=00:00:02:00:00:01       rate_percent=${rate_percent_p1}
     ...    frame_size=1024         length_mode=fixed
...
#STEP 2 Send traffic from single source mac and incremented destination mac (25-20)
...
#***** 25. Verify LAG Hash method as SRC_DST_MAC using traffic*****
    Tg Create Single Tagged Stream On Port          tg1     up_2        p2          p1          vlan_id=500     vlan_user_priority=7            mac_src=00:00:01:00:00:01       mac_dst=00:00:02:00:00:01       mac_dst_count=100         mac_dst_mode=increment
...
#**** 32 Remove transport service profile
    ...
    run keyword and ignore error    tg delete traffic stream    tg1    up_1
