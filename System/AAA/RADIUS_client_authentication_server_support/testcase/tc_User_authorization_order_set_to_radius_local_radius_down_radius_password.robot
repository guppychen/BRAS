...
*** Test Cases ***
tc_User_authorization_order_set_to_radius_local_radius_down_radius_password
    [Documentation]
      
    ...    1	User-authorization set to radius-local. Radius server is down. Login with bozo/clown (configure in radius only). Use Telnet and SSH. Execute the show session command.	Verify that login fails.		

    
    [Tags]     @author=YUE SUN     @tcid=AXOS_E72_PARENT-TC-4758      @globalid=2534721      @priority=P2      @eut=10GE-12          @user_interface=CLI    
    [Setup]     case_setup
    [Teardown]     case_teardown
      
    log    STEP:1 User-authorization set to radius-local. Radius server is down. Login with bozo/clown (configure in radius only). Use Telnet and SSH. Execute the show session command. Verify that login fails. 
    prov_radius_server    euta_radius    ${invalid_server}    secret=${invalid_secret}    port=${invalid_port}
