...
*** Test Cases ***
tc_User_authorization_order_set_to_radius_if_up_else_local_radius_down_e7support_password
    [Documentation]
      
    ...    1	User-authorization set to radius-if-up-else-local. Radius server is down. Login with root/root (local only). Use Telnet and SSH. Execute the show session command.	Verify that it is a local login.		

    
    [Tags]     @author=YUE SUN     @tcid=AXOS_E72_PARENT-TC-4772      @globalid=2534735      @priority=P2      @eut=10GE-12          @user_interface=CLI    
    [Setup]     case_setup
    [Teardown]     case_teardown
      
    log    STEP:1 User-authorization set to radius-if-up-else-local. Radius server is down. Login with root/root (local only). Use Telnet and SSH. Execute the show session command. Verify that it is a local login. 
