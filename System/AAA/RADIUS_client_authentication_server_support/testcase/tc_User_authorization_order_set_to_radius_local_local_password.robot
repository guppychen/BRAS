...
*** Test Cases ***
tc_User_authorization_order_set_to_radius_local_local_password
    [Documentation]
      
    ...    1	User-authorization set to radius-local. Radius server is up. Login with blank/blanker (configured only in E7). Use Telnet and SSH. Execute the show session command.	Verify that it is a local login.		

    
    [Tags]     @author=YUE SUN     @tcid=AXOS_E72_PARENT-TC-4752      @globalid=2534715      @priority=P2      @eut=10GE-12          @user_interface=CLI    
    [Setup]     case_setup
    [Teardown]     case_teardown
      
    log    STEP:1 User-authorization set to radius-local. Radius server is up. Login with blank/blanker (configured only in E7). Use Telnet and SSH. Execute the show session command. Verify that it is a local login. 
