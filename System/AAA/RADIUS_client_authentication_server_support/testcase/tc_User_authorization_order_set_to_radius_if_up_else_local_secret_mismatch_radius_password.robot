...
*** Test Cases ***
tc_User_authorization_order_set_to_radius_if_up_else_local_secret_mismatch_radius_password
    [Documentation]
      
    ...    1	User-authorization set to radius-if-up-else-local. radius up. SECRET mismatch between radius server and E7 radius-cfg auth configuration. Login with bozo/clown (configured only in radius). Use Telnet and SSH. Execute the show session command.	Verify that radius login fails.		

    
    [Tags]     @author=YUE SUN     @tcid=AXOS_E72_PARENT-TC-4766      @globalid=2534729      @priority=P2      @eut=10GE-12          @user_interface=CLI    
    [Setup]     case_setup
    [Teardown]     case_teardown
      
    log    STEP:1 User-authorization set to radius-if-up-else-local. radius up. SECRET mismatch between radius server and E7 radius-cfg auth configuration. Login with bozo/clown (configured only in radius). Use Telnet and SSH. Execute the show session command. Verify that radius login fails. 
...
case_setup
    ...
    [Arguments]
    log    case setup
    log    dprov valid radius server
    dprov_radius_server    euta_radius    ${radius_server}   
    log    dprov local user
