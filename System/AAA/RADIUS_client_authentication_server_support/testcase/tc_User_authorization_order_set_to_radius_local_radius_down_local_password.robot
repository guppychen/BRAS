...
*** Test Cases ***
tc_User_authorization_order_set_to_radius_local_radius_down_local_password
    [Documentation]
      
    ...    1	User-authorization set to radius-local. Radius server is down. Login with blank/blanker (configured in E7 only). Use Telnet and SSH. Execute the show session command.	Verify that it is a local login.		

    
    [Tags]     @author=YUE SUN     @tcid=AXOS_E72_PARENT-TC-4759      @globalid=2534722      @priority=P2      @eut=10GE-12          @user_interface=CLI    
    [Setup]     case_setup
    [Teardown]     case_teardown
      
    log    STEP:1 User-authorization set to radius-local. Radius server is down. Login with blank/blanker (configured in E7 only). Use Telnet and SSH. Execute the show session command. Verify that it is a local login. 
