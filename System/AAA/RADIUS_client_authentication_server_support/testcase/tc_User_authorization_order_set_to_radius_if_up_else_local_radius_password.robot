...
*** Test Cases ***
tc_User_authorization_order_set_to_radius_if_up_else_local_radius_password
    [Documentation]
      
    ...    1	User-authorization set to radius-if-up-else-local. Radius server is up. Login with bozo/clown (configured only in radius). Use Telnet and SSH. Execute the show session command.	Verify that it is a radius login.		

    
    [Tags]     @author=YUE SUN     @tcid=AXOS_E72_PARENT-TC-4762      @globalid=2534725      @priority=P2      @eut=10GE-12          @user_interface=CLI    
    [Setup]     case_setup
    [Teardown]     case_teardown
      
    log    STEP:1 User-authorization set to radius-if-up-else-local. Radius server is up. Login with bozo/clown (configured only in radius). Use Telnet and SSH. Execute the show session command. Verify that it is a radius login. 
    ...
    log    Destroy the Telnet session
