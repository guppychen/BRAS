...
*** Variables ***
${manager}        rsyslogd
${shellscript}     rsyslog.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_rsyslog
    ...
    [Tags]       @author=sdas     @tcid=AXOS_E72_PARENT-TC-2080
    [Setup]      AXOS_E72_PARENT-TC-2080 setup
    ...
    # Get all ARC processes before rsyslog restart
    ${arcmgrs_before}    ${rsyslogmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart rsyslog
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after rsyslogd restart
    ${arcmgrs_after}    ${rsyslogmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except rsyslogd if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if rsyslogd process information has changed
    Validate ARC process restart    ${rsyslogmgr_before}     ${rsyslogmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-2080 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2080 setup
