...
*** Variables ***
${manager}    logmgrd
${shellscript}     logmgr.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_log_mgr
    ...
    [Tags]       @author=bswamina     @tcid=AXOS_E72_PARENT-TC-2075
    [Setup]      AXOS_E72_PARENT-TC-2075 setup
    ...
    # Get all ARC processes before logmgr restart
    ${arcmgrs_before}    ${logmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart process
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after restart
    ${arcmgrs_after}    ${logmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except restarted process if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if restarted process information has changed
    Validate ARC process restart    ${logmgr_before}     ${logmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-2075 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2075 setup
