...
*** Test Cases ***
tc_ARC_ARC_manager_monitor_pcmd
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-205744
    [Setup]      AXOS_E72_PARENT-TC-205744 setup
    ...
    # Get all ARC processes before pcmd restart
    ${arcmgrs_before}    ${pcmmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart pcmd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after pcmd restart
    ${arcmgrs_after}    ${pcmmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except pcmd if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if pcmd process information has changed
    Validate ARC process restart    ${pcmmgr_before}     ${pcmmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205744 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205744 setup
