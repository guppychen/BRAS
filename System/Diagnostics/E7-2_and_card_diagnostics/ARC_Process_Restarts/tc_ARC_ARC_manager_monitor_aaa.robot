...
*** Test Cases ***
tc_ARC_ARC_manager_monitor_aaa
    ...
    [Tags]       @author=dzala    @tcid=AXOS_E72_PARENT-TC-2073
    [Setup]      AXOS_E72_PARENT-TC-2073 setup
    ...
    # Get all ARC processes before aaa restart
    ${arcmgrs_before}    ${aaamgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart aaamgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after aaamgrd restart
    ${arcmgrs_after}    ${aaamgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except aaamgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if aaamgrd process information has changed
    Validate ARC process restart    ${aaamgr_before}     ${aaamgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-2073 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2073 setup
