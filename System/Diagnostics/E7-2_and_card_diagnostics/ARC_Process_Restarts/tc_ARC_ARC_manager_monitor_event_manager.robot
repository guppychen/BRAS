...
*** Variables ***
${manager}   evtmgrd
${shellscript}     evtmgr.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_event_manager
    ...
    [Tags]       @author=bswamina    @tcid=AXOS_E72_PARENT-TC-2058      dual_card_not_support
    [Setup]      AXOS_E72_PARENT-TC-2058 setup
    ...
    # Get all ARC processes before evt restart
    ${arcmgrs_before}    ${evtmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart evtmgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after evtmgrd restart
    ${arcmgrs_after}    ${evtmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except evtmgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if evtmgrd process information has changed
    Validate ARC process restart    ${evtmgr_before}     ${evtmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-2058 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2058 setup
