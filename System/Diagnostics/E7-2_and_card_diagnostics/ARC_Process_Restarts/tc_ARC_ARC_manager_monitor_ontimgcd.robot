...
*** Variables ***
${manager}     ontimgcd
${shellscript}     ontimgcache.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_ontimgcd
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-205747
    [Setup]      AXOS_E72_PARENT-TC-205747 setup
    ...
    # Get all ARC processes before ontimgcd restart
    ${arcmgrs_before}    ${ontimgcmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart ontimgcd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after ontimgcd restart
    ${arcmgrs_after}    ${ontimgcmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except ontimgcd if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if ontimgcd process information has changed
    Validate ARC process restart    ${ontimgcmgr_before}     ${ontimgcmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205747 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205747 setup
