...
*** Variables ***
${manager}    ontimgfd
${shellscript}     ontimgfile.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_ontimgfd
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-205748
    [Setup]      AXOS_E72_PARENT-TC-205748 setup
    ...
    # Get all ARC processes before ontimgfd restart
    ${arcmgrs_before}    ${ontimgfmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart ontimgfd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after ontimgfd restart
    ${arcmgrs_after}    ${ontimgfmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except ontimgfd if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if ontimgfd process information has changed
    Validate ARC process restart    ${ontimgfmgr_before}     ${ontimgfmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205748 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205748 setup
