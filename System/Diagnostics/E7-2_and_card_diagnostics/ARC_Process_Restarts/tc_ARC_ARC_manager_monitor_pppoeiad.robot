...
*** Variables ***
${manager}    pppoeiad
${shellscript}     pppoeia.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_pppoeiad
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-205737
    [Setup]      AXOS_E72_PARENT-TC-205737 setup
    ...
    # Get all ARC processes before pppoeiad restart
    ${arcmgrs_before}    ${pppoeiad_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart pppoeiad
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after pppoeiad restart
    ${arcmgrs_after}    ${pppoeiad_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except pppoeiad if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if pppoeiad process information has changed
    Validate ARC process restart    ${pppoeiad_before}     ${pppoeiad_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205737 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205737 setup
