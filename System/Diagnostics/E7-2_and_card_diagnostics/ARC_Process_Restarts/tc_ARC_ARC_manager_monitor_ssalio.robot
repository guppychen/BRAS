...
*** Variables ***
${manager}        ssaliod
${shellscript}     ssalio.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_ssalio
    ...
    [Tags]       @author=kshettar     @tcid=AXOS_E72_PARENT-TC-2068
    [Setup]      AXOS_E72_PARENT-TC-2068 setup
    ...
    # Get all ARC processes before ssalio restart
    ${arcmgrs_before}    ${ssaliomgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart ssaliomgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after ssaliomgrd restart
    ${arcmgrs_after}    ${ssaliomgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except ssaliomgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if ssaliomgrd process information has changed
    Validate ARC process restart    ${ssaliomgr_before}     ${ssaliomgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-2068 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2068 setup
