...
*** Variables ***
${manager}   upgmgrd
${shellscript}     upgmgr.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_upgrade_mgr
    ...
    [Tags]       @author=upandiri     @tcid=AXOS_E72_PARENT-TC-2072
    [Setup]      AXOS_E72_PARENT-TC-2072 setup
    ...
    # Get all ARC processes before upgmgr restart
    ${arcmgrs_before}    ${upgmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart upgmgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after upgmgrd restart
    ${arcmgrs_after}    ${upgmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except upgmgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if upgmgrd process information has changed
    Validate ARC process restart    ${upgmgr_before}     ${upgmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-2072 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2072 setup
