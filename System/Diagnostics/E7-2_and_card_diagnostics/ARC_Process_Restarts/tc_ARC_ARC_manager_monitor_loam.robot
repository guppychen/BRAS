...
*** Test Cases ***
tc_ARC_ARC_manager_monitor_loam
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-20577    @jira=EXA-30819   dual_card_not_support
    [Setup]      AXOS_E72_PARENT-TC-20577 setup
    ...
    # Get all ARC processes before loam restart
    ${arcmgrs_before}    ${loammgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart loamd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after loamd restart
    ${arcmgrs_after}    ${loammgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except loamd if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if loamd process information has changed
    Validate ARC process restart    ${loammgr_before}     ${loammgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-20577 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-20577 setup
