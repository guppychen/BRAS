...
*** Variables ***
${manager}    ontupgmd
${shellscript}    ontupgmgr.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_ontupgmd
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-205745
    [Setup]      AXOS_E72_PARENT-TC-205745 setup
    ...
    # Get all ARC processes before ontupgmd restart
    ${arcmgrs_before}    ${ontupgmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart ontupgmd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after ontupgmd restart
    ${arcmgrs_after}    ${ontupgmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except ontupgmgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if ontupgmd process information has changed
    Validate ARC process restart    ${ontupgmgr_before}     ${ontupgmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205745 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205745 setup
