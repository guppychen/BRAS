...
*** Test Cases ***
tc_ARC_ARC_manager_monitor_igmp
    ...
    [Tags]       @author=pmunisam     @tcid=AXOS_E72_PARENT-TC-20572
    [Setup]      AXOS_E72_PARENT-TC-20572 setup
    ...
    # Get all ARC processes before igmp restart
    ${arcmgrs_before}    ${igmpmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart igmpmgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after igmpmgrd restart
    ${arcmgrs_after}    ${igmpmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except igmpmgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if igmpmgrd process information has changed
    Validate ARC process restart    ${igmpmgr_before}     ${igmpmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-20572 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-20572 setup
