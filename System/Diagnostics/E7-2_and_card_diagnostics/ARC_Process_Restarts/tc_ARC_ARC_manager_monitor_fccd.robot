...
*** Test Cases ***
tc_ARC_ARC_manager_monitor_fccd
    ...
    [Tags]       @author=clakshma    @tcid=AXOS_E72_PARENT-TC-205738
    [Setup]      AXOS_E72_PARENT-TC-205738 setup
    ...
    # Get all ARC processes before fccd restart
    ${arcmgrs_before}    ${fccmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart fccd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after fccd restart
    ${arcmgrs_after}    ${fccmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except fcc if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if fccd process information has changed
    Validate ARC process restart    ${fccmgr_before}     ${fccmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205738 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205738 setup
