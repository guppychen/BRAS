...
*** Test Cases ***
tc_ARC_ARC_manager_monitor_vcamgr
    ...
    [Tags]    @author=kshettar    @tcid=AXOS_E72_PARENT-TC-2078
    [Setup]    AXOS_E72_PARENT-TC-2078 setup
    ...
    # Get all ARC processes before vcamgr restart
    ${arcmgrs_before}    ${vcamgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart vcamgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after vcamgrd restart
    ${arcmgrs_after}    ${vcamgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except vcamgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if vcamgrd process information has changed
    Validate ARC process restart    ${vcamgr_before}    ${vcamgr_after}

    # Validate if there is a restart event in event manager
    Validate ARC Restart event    n1_session1    ${manager}

*** Keywords ***
AXOS_E72_PARENT-TC-2078 setup
    log    Enter AXOS_E72_PARENT-TC-2078 setup
