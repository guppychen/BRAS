...
*** Variables ***
${manager}    ipmgrd
${shellscript}    ipmgr.sh


*** Test Cases ***
tc_ARC_ARC_manager_monitor_ipmgrd
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-205742     dual_card_not_support
    [Setup]      AXOS_E72_PARENT-TC-205742 setup
    ...
    # Get all ARC processes before ipmgrd restart
    ${arcmgrs_before}    ${ipmgrd_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart ipmgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after ipmgrd restart
    ${arcmgrs_after}    ${ipmgrd_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except ipmgrd if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if ipmgrd process information has changed
    Validate ARC process restart    ${ipmgrd_before}     ${ipmgrd_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205742 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205742 setup
