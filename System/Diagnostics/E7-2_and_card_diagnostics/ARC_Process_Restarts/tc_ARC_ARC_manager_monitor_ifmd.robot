...
*** Test Cases ***
tc_ARC_ARC_manager_monitor_ifmd
    ...
    [Tags]       @author=clakshma     @tcid=AXOS_E72_PARENT-TC-205740
    [Setup]      AXOS_E72_PARENT-TC-205740 setup
    ...
    # Get all ARC processes before ifmd restart
    ${arcmgrs_before}    ${ifmd_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart ifmd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after ifmd restart
    ${arcmgrs_after}    ${ifmd_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except ifmd if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if ifmd process information has changed
    Validate ARC process restart    ${ifmd_before}     ${ifmd_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-205740 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-205740 setup
