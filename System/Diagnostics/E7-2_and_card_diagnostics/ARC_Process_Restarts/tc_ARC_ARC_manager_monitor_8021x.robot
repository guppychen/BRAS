...
*** Variables ***
${manager}        dot1xd
${shellscript}    dot1x.sh

*** Test Cases ***
tc_ARC_ARC_manager_monitor_8021x
    ...
    [Tags]    @author=dzala    @tcid=AXOS_E72_PARENT-TC-2082
    [Setup]    AXOS_E72_PARENT-TC-2082 setup
    ...
    # Get all ARC processes before 8021x restart
    ${arcmgrs_before}    ${dot1xmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart 8021xmgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after 8021xmgrd restart
    ${arcmgrs_after}    ${dot1xmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except 8021xmgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if 8021xmgrd process information has changed
    Validate ARC process restart    ${dot1xmgr_before}    ${dot1xmgr_after}

    # Validate if there is a restart event in event manager
    Validate ARC Restart event    n1_session1    ${manager}

*** Keywords ***
AXOS_E72_PARENT-TC-2082 setup
    log    Enter AXOS_E72_PARENT-TC-2082 setup
