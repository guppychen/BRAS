...
...     
...    
...    Following processes should be restartable on E3-16F device:
...     
...    
...    8021x
...    bin-fpga-sat-nat
...
*** Variables ***
${manager}    diagmgrd
${shellscript}     diagmgr.sh

*** Test Cases ***
tc_ARC_ARC_manager_monitor_diag_mgr
    ...
    [Tags]       @author=clakshma    @tcid=AXOS_E72_PARENT-TC-2057
    [Setup]      AXOS_E72_PARENT-TC-2057 setup
    ...
    # Get all ARC processes before diagmgrd restart
    ${arcmgrs_before}    ${diagmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart diagmgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after diagmgrd restart
    ${arcmgrs_after}    ${diagmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except diagmgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if diagmgrd process information has changed
    Validate ARC process restart    ${diagmgr_before}     ${diagmgr_after}
...
*** Keywords ***
AXOS_E72_PARENT-TC-2057 setup
    [Documentation]
    [Arguments]
    log    Enter AXOS_E72_PARENT-TC-2057 setup
