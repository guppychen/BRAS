...
*** Variables ***
${manager}        vlanmgrd
${shellscript}    vlanmgr.sh

*** Test Cases ***
tc_ARC_ARC_manager_monitor_vlanmgrd
    ...
    [Tags]    @author=pmunisam    @tcid=AXOS_E72_PARENT-TC-2079
    [Setup]    AXOS_E72_PARENT-TC-2079 setup
    ...
    # Get all ARC processes before vlanmgrd restart
    ${arcmgrs_before}    ${vlanmgr_before}    Get ARC process    n1_session2    Manager    ${manager}

    # Restart vlanmgrd
    Restart ARC process    n1_session2    ${manager}    ${shellscript}

    # Get all ARC processes after vlanmgrd restart
    ${arcmgrs_after}    ${vlanmgr_after}    Get ARC process    n1_session2    Manager    ${manager}

    # Validate all ARC processes except vlanmgr if they are the same
    Validate ARC process    n1_session2    ${arcmgrs_before}    ${arcmgrs_after}    ${manager}

    # Validate if vlanmgrd process information has changed
    Validate ARC process restart    ${vlanmgr_before}    ${vlanmgr_after}

    # Validate if there is a restart event in event manager
    Validate ARC Restart event    n1_session1    ${manager}

*** Keywords ***
AXOS_E72_PARENT-TC-2079 setup
    log    Enter AXOS_E72_PARENT-TC-2079 setup
