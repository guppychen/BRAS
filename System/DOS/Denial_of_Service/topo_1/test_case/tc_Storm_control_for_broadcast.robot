...
*** Test Cases ***
tc_Storm_control_for_broadcast
    [Documentation]    1 set strorm control broadcast rate 1000pps for eth-port1
    ...    2 send broadcast traffic 2000pps into eth-port1
    [Tags]    @author=Lincoln Yu    @globalid=2384305    @tcid=AXOS_E72_PARENT-TC-2917    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    Tg Create single Tagged Stream On Port    tg1    raw_upstream1    p2    p1    vlan_id=${service_vlan}    vlan_user_priority=0
    ...    frame_size=512    length_mode=fixed    mac_src=${mac1}    mac_dst=${broadcast_mac}    l3_protocol=ipv4    ip_src_addr=${ip1}
    ...
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p2    eth.src==${mac1} and eth.dst==${broadcast_mac} and vlan.id==${service_vlan} and vlan.priority ==0    ${limit_rate}
    [Teardown]    teardown
...
setup
    ...
    prov_interface_ethernet    eutA    ${service_model.service_point1.member.interface1}    storm-control broadcast=${storm_control_rate}
...
teardown
    ...
    dprov_interface_ethernet    eutA    ${service_model.service_point1.member.interface1}    storm-control broadcast
