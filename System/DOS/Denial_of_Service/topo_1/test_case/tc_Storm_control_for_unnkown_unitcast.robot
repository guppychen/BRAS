...
*** Test Cases ***
tc_Storm_control_for_unnkown_unitcast
    [Documentation]    1	set strorm control unnkown unitcast rate 1000pps for eth-port1
    ...    2	send unnkown unitcast traffic 2000pps into eth-port1
    [Tags]    @author=Lincoln Yu    @globalid=2384306    @tcid=AXOS_E72_PARENT-TC-2918    @eut=NGPON2-4    @priority=P1
    [Setup]    setup
    ...
    verify_traffic_loss_within_with_filter    tg1    raw_upstream1    p2    eth.src==${mac1} and eth.dst==${mac2} and vlan.id==${service_vlan} and vlan.priority ==0    ${limit_rate}
    [Teardown]    teardown
...
setup
    ...
    prov_interface_ethernet    eutA    ${service_model.service_point1.member.interface1}    storm-control unknown-unicast=${storm_control_rate}
...
teardown
    ...
    dprov_interface_ethernet    eutA    ${service_model.service_point1.member.interface1}    storm-control unknown-unicast
