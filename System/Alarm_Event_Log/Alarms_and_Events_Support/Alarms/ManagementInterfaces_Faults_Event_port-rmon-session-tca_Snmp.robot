...
*** Test Cases ***
ManagementInterfaces_Faults_Event_port-rmon-session-tca_Snmp
    [Documentation]    Testcase to verify that alarm is generated when the pon port rmon-session threshold exceeds.
    [Tags]  dual_card_not_support   @feature=Alarm_Event_Log    @subFeature=Alarms and Events Support    @author=Doris He   @author=Shesha Chandra    @tcid=AXOS_E72_PARENT-TC-332   @user=root   @user=root   @user=root    @globalid=2226253    @priority=P1    @user_interface=Snmp
    Cli    n1_session2    cli
    ...
    Should contain    ${snmp_trap}    pon-rmon-session-tca
    [Teardown]    Teardown ManagementInterfaces_Faults_Event_port-rmon-session-tca_Snmp    n1_session2

*** Keywords ***
Teardown ManagementInterfaces_Faults_Event_port-rmon-session-tca_Snmp
    [Arguments]    ${DUT}
