...
Clearing RMON MINOR alarm
    ...
    ${port}    Run Keyword If    '${user_interface}' == 'cli'   Set Variable If    ${result.__contains__('E3-2')}==True     ${port_id}      ${rmonport}
    ...
    ${port}    Run Keyword If    '${user_interface}' == 'netconf'    Set Variable If    ${str.__contains__('E3-2')}==True    ${net_port_id}     ${rmonport}
...
Configuring OSPF on Cisco
    ...
    Telnet.Login        ${user}      ${password}    login_prompt=Password:   password_prompt=ARF-ROLT-2960>     login_timeout=10 seconds
    Telnet.Write        enable
...
#    Run Keyword If     '${user_interface}' == 'cli' and '${port_status}' == 'port_down'      Should Contain      ${result1}      ${port_id}']
    ...
    Wait Until Keyword Succeeds      2 min     10 sec    Send Command And Confirm Expect     ${device}   show alarm active subscope id 1201 name loss-of-signal
...
Trigerring NTP prov alarm 
    ...
    #sleep for 5s
    sleep    5s
    cli    ${device}      ntp server 2 255.255.255.253     timeout=90      prompt=\\#     retry=4
    ...
    #sleep for 5s
    sleep    5s
    cli    ${device}       no ntp server 2 255.255.255.253     timeout=50      prompt=\\#     retry=4
...
Clearing NTP prov alarm
    ...
    cli    ${device}      ntp server 2 invalid1         timeout=50    prompt=\\#     retry=4
    cli    ${device}      end      timeout=90     prompt=\\#     retry=4
    ${result}      cli    ${device}       show alarm active subscope id 1919     timeout=50         retry=4
    Should Not Contain      ${result}      name ntp-prov 
...
Clearing NTP prov alarm netconf
    ...
    Netconf Raw    ${device}    xml=<?xml version="1.0" encoding="utf-8"?><rpc xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id=""><edit-config><target><running/></target><config><config xmlns="http://www.calix.com/ns/exa/base"><system><ntp><server><id>2</id><address>invalid1</address></server></ntp></system></config></config></edit-config></rpc>]]>]]>
    # Sleep for 3s to clear the alarm
    BuiltIn.Sleep    3s
...
#    Wait Until Keyword Succeeds      2 min     10 sec      caferobot.command.adapter.CliAdapter.Login      ${device}
    ...
    #${result}    cli    n1    show inventory    prompt=\\#        timeout=50
    #${dut}    Run Keyword If    ${result.__contains__('E3-2')}==True    Set Variable    E3-2
    #...      ELSE IF      ${result.__contains__('E9-2')}==True    Set Variable    E9-2
...
#    \    Exit For Loop If    '${match_pri}' == 'True'
    ...
    cli    ${device}        ex -sc '%s@${syslog_dir}/${syslog_log}@${syslog_dir}/${syslog_log}@g|x' /etc/rsyslog.conf      timeout_exception=0
#    cli    ${device}        ex -sc '%s@/var/log/E3-2/${syslog_log}@${syslog_dir}/${syslog_log}@g|x' /etc/rsyslog.conf      timeout_exception=0
#    cli    ${device}        ex -sc '%s@/var/log/E9-2/${syslog_log}@${syslog_dir}/${syslog_log}@g|x' /etc/rsyslog.conf      timeout_exception=0
    #${match_dut}     OperatingSystem.Get File    /etc/rsyslog.conf
    #Should Contain     ${match_dut}     ${dut}

    Log    *** Deleting Syslog file and restarting syslog server ***
    cli    ${device}         echo ''>${syslog_dir}/${syslog_log}      timeout=120
#    ${result}      cli    ${device}    service rsyslog restart    timeout_exception=0     prompt=Starting system logger:    timeout=120
#    #sleep for 10s
#    Sleep    10s
#    cli    ${device}        sudo chmod 777 ${syslog_dir}/${syslog_log}      timeout=120       timeout_exception=0
...
Configuring rsyslog file on local pc
    ...
    # Adding $template RemoteLogs,"${syslog_dir}/${syslog_log}" *
    cli    ${device}    sudo cp /etc/rsyslog.conf /tmp/rsyslog.conf    timeout=90     
    ...
    cli    ${device}    ex -sc '%s@cygwin2@$template RemoteLogs,"${syslog_dir}/${syslog_log}" *@g|x' /etc/rsyslog.conf     timeout=90     timeout_exception=0
  
    # Adding *.*  ?RemoteLogs
    cli    ${device}    sudo cp /etc/rsyslog.conf /tmp/rsyslog.conf    timeout=90     
    cli    ${device}    awk '/${syslog_log}/{print;print "*.*\ \ \?RemoteLogs";next}1' /tmp/rsyslog.conf > /etc/rsyslog.conf     timeout=90
...
Clearing history log captured in syslog server
    [Arguments]    ${device}   ${syslog_server_ip}     ${user_password}     ${device1}=n1
    [Documentation]        Clearing history log captured in syslog server
    [Tags]        @author=ssekar

    #${result}    cli    ${device1}    show inventory    prompt=\\#        timeout=50
    #${dut}    Run Keyword If    ${result.__contains__('E3-2')}==True    Set Variable    E3-2
    #...      ELSE IF      ${result.__contains__('E9-2')}==True    Set Variable    E9-2

#    Wait Until Keyword Succeeds      2 min     10 sec         Disconnect     ${device}
#    cli    ${device}     cd /var/log       timeout=120        timeout_exception=0
    ...
    ${get_file}   cli   ${device}    cat ${syslog_dir}/${syslog_log}   prompt=${DEVICES.${device}.user}
    Log     ${get_file}
#    Wait Until Keyword Succeeds      2 min     10 sec            Disconnect    ${device}
...
Alarm_messages_in_syslog_server
    ...
    #${result}   Run Keyword If    '${user_interface}' == 'netconf'    Netconf Raw    ${DUT}    xml=<?xml version="1.0" encoding="utf-8"?><rpc xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id="9"><get><filter xmlns:t="http://www.calix.com/ns/exa/base" type="xpath" select="/status/system/inventory"/></get></rpc>]]>]]>
    #${result}   Run Keyword If    '${user_interface}' == 'netconf'    Convert to string    ${result}
    #...   ELSE IF      '${user_interface}' == 'cli'     cli    ${DUT}    show inventory    prompt=\\#        timeout=50

    #${dut1}    Run Keyword If    ${result.__contains__('E3-2')}==True    Set Variable    E3-2
    #...      ELSE IF      ${result.__contains__('E9-2')}==True    Set Variable    E9-2
...
#    Wait Until Keyword Succeeds      2 min     10 sec      Disconnect    ${device}
    #sleep for 10s
    Sleep     10s
#    cli    ${device}     cd ${syslog_dir}      timeout=120     retry=4
    #cli    ${device}     sudo cat ${syslog_dir}/${syslog_log}       timeout_exception=0        retry=4        prompt=password     timeout=120
...
#    Wait Until Keyword Succeeds      2 min     10 sec      Disconnect    ${device}
#    Wait Until Keyword Succeeds      2 min     10 sec      caferobot.command.adapter.CliAdapter.Login      ${device}
#    cli    ${device}     cd ${syslog_dir}      timeout=120     timeout_exception=0
    #cli    ${device}     sudo cat ${syslog_dir}/${syslog_log}       timeout_exception=0        retry=4        prompt=password     timeout=120
...
#    Wait Until Keyword Succeeds      2 min     10 sec      Disconnect    ${device}
#    Wait Until Keyword Succeeds      2 min     10 sec      caferobot.command.adapter.CliAdapter.Login      ${device}
#    cli    ${device}     cd ${syslog_dir}      timeout=120
#    cli     ${device}         ll
...
Verify Alarm definitions does not get paginated
    ...
    ${result}    cli    ${device}    show alarm definitions    timeout=120           prompt=${hostname}
...
Generic Alarms
    ...
    ${result}    cli    ${device}    ${command}      timeout=150     #prompt=E3-2
...
Suppressing Active alarms
    ...
    ${suppress}    Set Variable If    ${result.__contains__('E3-2')}==True    alarm-suppression ENABLED
...
Suppressing Active alarms using netconf
    ...
    ${suppress}    Set Variable If    ${str.__contains__('E3-2')}==True   <alarm-suppression>ENABLED</alarm-suppression>    <suppress>TRUE</suppress>
...
Unsuppressing Active alarms using netconf
    ...
    ${get_interface_by_inventory_check}   Netconf Raw    ${device}    xml=<rpc xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id="1"><get><filter xmlns="http://www.calix.com/ns/exa/base"><status><system><inventory/></system></status></filter></get></rpc>]]>]]>
    ${str}      Convert to string     ${get_interface_by_inventory_check}
    ${suppress}    Set Variable If    ${str.__contains__('E3-2')}==True   <alarm-suppression>DISABLED</alarm-suppression>    <suppress>FALSE</suppress>
    Netconf Raw    ${device}    xml=<?xml version="1.0" encoding="utf-8"?><rpc xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id=""><edit-config><target><running/></target><config><config xmlns="http://www.calix.com/ns/exa/base">${suppress}</config></config></edit-config></rpc>]]>]]>
    ${result}     Netconf Raw    ${device}    xml=<?xml version="1.0" encoding="utf-8"?><rpc xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id="9"><get><filter xmlns:t="http://www.calix.com/ns/exa/base" type="xpath" select="/status/system/alarm/suppressed"/></get></rpc>]]>]]>
    ${str}=    Convert to string    ${result}
    ${total_count}   Get Regexp Matches     ${str}    <total-count>([0-9]+)</total-count>    1
...
Unsuppressing Active alarms
    ...
    [Tags]        @author=ssekar
    ${result}    cli    ${device}    show inventory    prompt=\\#       timeout=10
    ${unsuppress}    Set Variable If    ${result.__contains__('E3-2')}==True    alarm-suppression DISABLED
...
#    cli    ${device}    y    prompt=#    timeout=30
...
#    Wait Until Keyword Succeeds      5 min     10 sec      caferobot.command.adapter.CliAdapter.Login      ${device}
...
Verifying_Event_subscription_max
    ...
    \    Log      ${output}
    \    Should Contain Any      ${output}        <new-value>Ero1</new-value>   <category>DBCHANGE</category>
...
Triggering System restart event
    ...
    Wait Until Keyword Succeeds      2 min     10 sec       Reload System      ${device}
...
Triggering_Alarms_netconf
    ...
    Wait Until Keyword Succeeds     30 sec    10 sec     Triggering Loss of Signal MAJOR alarm    device=${device1}      linux=${device1_linux_mode}     user_interface=${user_interface}
...
Triggering alarm for dhcp server detected
    ...
    cli    ${device}     configure        timeout=120
    cli    ${device}     interface craft ${craft}      timeout=120
...
Triggering_Alarms
    ...
    Wait Until Keyword Succeeds      30 sec     10 sec     Triggering Loss of Signal MAJOR alarm    ${device1}      ${device1_linux_mode}      ${user_interface}
