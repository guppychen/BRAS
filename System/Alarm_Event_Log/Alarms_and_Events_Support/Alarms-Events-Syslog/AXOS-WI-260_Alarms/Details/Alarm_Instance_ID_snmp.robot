...
Alarm_Instance_ID
    ...
    ...                1.Verify the field is correct in the alarm definition. show alarm definition subscope instance-id X (Not Applicable)
    ...                2.Verify the field is correct in the active alarm. show alarm active subscope instance-id X (Not Applicable)
    ...                3.Verify the field is correct in the alarm history. show alarm history subscope instance-id X (Not Applicable)
    ...
    ...                6.Verify the field is correct in the archive alarm. show alarm archive subscope instance-id X (Not Applicable)
    ...                7.Verify the field is correct in the acknowledged alarm. show alarm acknowledged subscope instance-id X (Not Applicable)
    [Tags]        @feature=Alarm_Event_Log    @subFeature=Alarms and Events Support    @author=Doris He    @author=ssekar   @tcid=AXOS_E72_PARENT-TC-2888   @user=root    @functional    @priority=P2      @user_interface=snmp      @runtime=long
    ...
    #Run Keyword And Continue On Failure     Verify Suppressing Alarms        n1       instance-id     suppressed_alarm_loss_of_signal
    #Run Keyword And Continue On Failure      Verify Suppressing Alarms        n1       instance-id      suppressed_alarm_rmon_session
    ...
    \    Log   *** Starting SNMP trap, shelving, and un-shelving NTP-prov alarm, and verifying Trap is received and the Alarm instance-id after stopping it ***
    \    Wait Until Keyword Succeeds    30 seconds    5 seconds     SNMP_start_trap    n1_snmp_v2     port=${DEVICES.n1_snmp_v2.redirect}
    \    Wait Until Keyword Succeeds    30 seconds    5 seconds    Shelving Active alarms    n1       parameter=instance-id     command_execution=shelved_ntp_prov
    ...
    : FOR    ${INDEX}    IN RANGE    0    5
    \    Log   *** Starting SNMP trap, shelving, and un-shelving application suspended alarm, and verifying Trap is received and the Alarm instance-id after stopping it ***
    \    Wait Until Keyword Succeeds    30 seconds    5 seconds     SNMP_start_trap    n1_snmp_v2     port=${DEVICES.n1_snmp_v2.redirect}
    \    Wait Until Keyword Succeeds    30 seconds    5 seconds    Shelving Active alarms    n1       parameter=instance-id     command_execution=shelved_app_sus
    \    @{list}     Wait Until Keyword Succeeds    30 seconds    5 seconds    SNMP_stop_trap     n1_snmp_v2
    \    ${result}     Get From List     ${list}    0
    \    ${count}      Get From List     ${list}    1
    \    Exit For Loop If    ${count} >= 2
