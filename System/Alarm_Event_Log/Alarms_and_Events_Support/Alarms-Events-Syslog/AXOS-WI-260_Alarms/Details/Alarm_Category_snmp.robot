...
Alarm_Category
    ...
    ...                1.Verify the field is correct in the alarm definition. show alarm definition subscope category X (Not Applicable)
    ...                2.Verify the field is correct in the active alarm. show alarm active subscope category X (Not Applicable)
    ...                3.Verify the field is correct in the alarm history. show alarm history subscope category X (Not Applicable)
    ...
    ...                6.Verify the field is correct in the archive alarm. show alarm archive subscope category X (Not Applicable)
    ...                7.Verify the field is correct in the acknowledged alarm. show alarm acknowledged subscope category X (Not Applicable)
    [Tags]        @feature=Alarm_Event_Log    @subFeature=Alarms and Events Support    @author=Doris He    @author=ssekar   @tcid=AXOS_E72_PARENT-TC-2905   @user=root      @functional    @priority=P2      @user_interface=snmp      @runtime=short
    ...
    \     Log   *** Starting SNMP trap, shelving, and un-shelving NTP-prov alarm, and verifying Trap is received and the Alarm category after stopping it ***
    \     Wait Until Keyword Succeeds    30 seconds    5 seconds     SNMP_start_trap    n1_snmp_v2     port=${DEVICES.n1_snmp_v2.redirect}
    \     Wait Until Keyword Succeeds    30 seconds    5 seconds    Shelving Active alarms    n1       parameter=category     command_execution=shelved_ntp_prov
    ...
    Wait Until Keyword Succeeds      2 min     10 sec         SNMP_trap_verification_for_NTP_alarm     n1_snmp_v2     ${result}     instance-id=${ntp_instance_id}      parameter=category

    Log   *** Starting SNMP trap, shelving, and un-shelving application suspended alarm, and verifying Trap is received and the Alarm category after stopping it ***
    : FOR    ${INDEX}    IN RANGE    0    5
    \    Wait Until Keyword Succeeds    30 seconds    5 seconds     SNMP_start_trap    n1_snmp_v2     port=${DEVICES.n1_snmp_v2.redirect}
    \    Wait Until Keyword Succeeds    30 seconds    5 seconds    Shelving Active alarms    n1       parameter=category     command_execution=shelved_app_sus
    \    @{list}      Wait Until Keyword Succeeds    30 seconds    5 seconds    SNMP_stop_trap     n1_snmp_v2
    \    ${result}     Get From List     ${list}    0
    \    ${count}      Get From List     ${list}    1
    \    Exit For Loop If    ${count} >= 2
    Log   *** verifying trap is received in local PC when alarm is shelved and un-shelved ***
    Wait Until Keyword Succeeds      2 min     10 sec         SNMP_trap_verification_for_application_suspended_alarm      n1_snmp_v2     ${result}      instance-id=${app_instance_id}       parameter=category
