...
Alarm_Address
    ...
    [Tags]        @feature=Alarm_Event_Log    @subFeature=Alarms and Events Support    @author=Doris He    @author=ssekar   @tcid=AXOS_E72_PARENT-TC-2838   @user=root      @functional    @priority=P2      @user_interface=snmp

    #Log    *** Verifying Alarm address is displayed correctly while suppressing (will be included in 3.1.0) ***
    ...
    \    Wait Until Keyword Succeeds    30 seconds    5 seconds    Shelving Active alarms    n1    parameter=address
    ...
    Run Keyword And Continue On Failure    SNMP_trap_verification_for_application_suspended_alarm    n1_snmp_v2    ${result}    instance-id=${app_instance_id}    parameter=address
