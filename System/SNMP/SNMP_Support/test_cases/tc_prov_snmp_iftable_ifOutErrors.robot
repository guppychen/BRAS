...
*** Test Cases ***
tc_prov_snmp_v2_iftable_ifOutErrors
    [Documentation]    prov snmp v2 iftable_ifOutErrors
    [Tags]    @author=Sean Wang    @globalid=2358312   @tcid=AXOS_E72_PARENT-TC-2424    @feature=SNMP    @subfeature=SNMP Support    @priority=P1
    [Setup]    case setup
    ...
    ...    l4_protocol=udp    udp_dst_port=${udp_port1}    udp_src_port=${udp_port2}    rate_bps=${rate_bps}    fcs_error=1
    ...
    ${re_m}    ${re_outerrors}    should match regexp    ${admin}    tx-errors\\s+(\\d+)
    ${result}=    snmp get    eutA_snmp_v2    ifOutErrors.${port_1_oid}
    ${re}    Convert To Integer    ${result}
    Should be true    ${re}-${re_outerrors}>=0
