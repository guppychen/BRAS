...
tc_prov_snmp_v2_iftable_ifUcastPkts
    ...
    [Tags]    @author=Sean Wang    @globalid=2358216   @tcid=AXOS_E72_PARENT-TC-2413    @feature=SNMP    @subfeature=SNMP Support    @priority=P1
    [Setup]    case setup
    ...
    ${re_m}    ${re_inoctets}    should match regexp    ${admin}    rx-unicast-pkts\\s+(\\d+)
    ${result}=    snmp get    eutA_snmp_v2    ifInUcastPkts.${port_1_oid}
    ${re}    convert to string    ${result}
    Should Contain    ${re}    ${re_inoctets}
