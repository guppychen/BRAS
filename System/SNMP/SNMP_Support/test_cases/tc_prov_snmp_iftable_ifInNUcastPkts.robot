...
*** Test Cases ***
tc_prov_snmp_v2_iftable_ifUcastPkts
    [Documentation]    prov snmp v2 iftable_ifUcastPkts
    [Tags]    @author=Sean Wang    @globalid=2358302   @tcid=AXOS_E72_PARENT-TC-2414    @feature=SNMP    @subfeature=SNMP Support    @priority=P1
    [Setup]    case setup
    ...
    ${re_m}    ${re_inmulticast}    should match regexp    ${admin}    rx-multicast-pkts\\s+(\\d+)
    ${re_m}    ${re_inbroadcast}    should match regexp    ${admin}    rx-broadcast-pkts\\s+(\\d+)
    ${re_inNUcast}    evaluate    (${re_inmulticast}+${re_inbroadcast})
    ${result}=    snmp get    eutA_snmp_v2    ifInNUcastPkts.${port_1_oid}
    ${re}    Convert To Integer    ${result}
    Should be true    ${re}-${re_inNUcast}>=0
