...
*** Test Cases ***
tc_prov_snmp_v2_iftable_ifoutdiscard
    [Documentation]    prov snmp v2 iftable_ifoutdiscard
    [Tags]    @author=Sean Wang    @globalid=2358310   @tcid=AXOS_E72_PARENT-TC-2422    @feature=SNMP    @subfeature=SNMP Support    @priority=P1
    [Setup]    case setup
    ...
    ${re_m}    ${re_outdiscard}    should match regexp    ${admin}    tx-discards\\s+(\\d+)
    ${result}=    snmp get    eutA_snmp_v2    ifOutDiscards.${port_1_oid}
    ${re}    Convert To Integer    ${result}
    Should be true    ${re}-${re_outdiscard}>=0
