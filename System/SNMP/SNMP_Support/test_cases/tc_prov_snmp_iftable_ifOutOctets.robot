...
*** Test Cases ***
tc_prov_snmp_v2_iftable_ifOutOctets
    [Documentation]    prov snmp v2 iftable_ifOutOctets
    [Tags]    @author=Sean Wang    @globalid=2358307   @tcid=AXOS_E72_PARENT-TC-2419    @feature=SNMP    @subfeature=SNMP Support    @priority=P1
    [Setup]    case setup
    ...
    ${re_m}    ${re_ifOutOctets}    should match regexp    ${admin}    tx-octets\\s+(\\d+)
    ${result}=    snmp get    eutA_snmp_v2    ifOutOctets.${port_1_oid}
    ${re}    convert to string    ${result}
    Should be true    ${re}>=${re_ifOutOctets}
