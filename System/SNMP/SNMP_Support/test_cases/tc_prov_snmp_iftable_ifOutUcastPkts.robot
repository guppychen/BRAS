...
*** Test Cases ***
tc_prov_snmp_v2_iftable_ifOutUcastPkts
    [Documentation]    prov snmp v2 iftable_ifOutUcastPkts
    [Tags]    @author=Sean Wang    @globalid=2358308   @tcid=AXOS_E72_PARENT-TC-2420    @feature=SNMP    @subfeature=SNMP Support    @priority=P1
    [Setup]    case setup
    ...
    ${re_m}    ${re_ifOutUcastPkts}    should match regexp    ${admin}    tx-unicast-pkts\\s+(\\d+)
    ${result}=    snmp get    eutA_snmp_v2    ifOutUcastPkts.${port_1_oid}
    ${re}    convert to string    ${result}
    Should Contain    ${re}    ${re_ifOutUcastPkts}
