...
*** Test Cases ***
tc_prov_snmp_v2_iftable_ifdiscard
    [Documentation]    prov snmp v2 iftable_ifdiscard
    [Tags]    @author=Sean Wang    @globalid=2358303   @tcid=AXOS_E72_PARENT-TC-2415    @feature=SNMP    @subfeature=SNMP Support    @priority=P1
    [Setup]    case setup
    ...
    ${re_m}    ${re_discard}    should match regexp    ${admin}    rx-discards\\s+(\\d+)
    ${result}=    snmp get    eutA_snmp_v2    ifInDiscards.${port_1_oid}
    ${re}    Convert To Integer    ${result}
    Should be true    ${re}-${re_discard}>=0
