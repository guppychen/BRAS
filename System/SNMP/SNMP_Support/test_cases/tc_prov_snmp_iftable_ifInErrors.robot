...
*** Test Cases ***
tc_prov_snmp_v2_iftable_ifInErrors
    [Documentation]    prov snmp v2 iftable_ifInErrors
    ...
    ${re_m}    ${re_errors}    should match regexp    ${admin}    rx-errors\\s+(\\d+)
    ${result}=    snmp get    eutA_snmp_v2    ifInErrors.${port_1_oid}
    ${re}    Convert To Integer    ${result}
    Should be true    ${re}-${re_errors}>=0
