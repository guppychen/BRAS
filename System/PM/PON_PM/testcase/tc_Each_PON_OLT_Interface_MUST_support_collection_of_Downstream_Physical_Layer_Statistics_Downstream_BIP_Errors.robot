...
*** Test Cases ***
tc_Each_PON_OLT_Interface_MUST_support_collection_of_Downstream_Physical_Layer_Statistics_Downstream_BIP_Errors
    [Documentation]    Each PON OLT Interface MUST support collection of Downstream Physical Layer Statistics on a Per ONU basis by processing the PLOAM 'REI Messages' received from the ONUs/SFP/GAMMA PON MAC devices:Downstream BIP Errors
    [Tags]       @author=JerryWu     @TCID=AXOS_E72_PARENT-TC-830     @globalid=2307690
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:Each PON OLT Interface MUST support collection of Downstream Physical Layer Statistics on a Per ONU basis by processing the PLOAM 'REI Messages' received from the ONUs/SFP/GAMMA PON MAC devices:Downstream BIP Errors

    ${output}    show_ont_pm    eutA    ${service_model.subscriber_point1.attribute.ont_id}    ${rmon_session_15_min}    ${rmon_type}    ${num_back}    ${num_show}
    Should Match Regexp    ${output}    downstream-bip-errors \\d+
