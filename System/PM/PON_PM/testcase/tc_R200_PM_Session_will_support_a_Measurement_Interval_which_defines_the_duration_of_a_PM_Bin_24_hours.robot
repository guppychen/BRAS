...
*** Test Cases ***
tc_R200_PM_Session_will_support_a_Measurement_Interval_which_defines_the_duration_of_a_PM_Bin_24_hours
    [Documentation]    A Measurement Interval defines a Measurement Bin Duration: 24 hours.
    [Tags]       @author=JerryWu     @TCID=AXOS_E72_PARENT-TC-755    @globalid=2307615
    [Setup]      setup
    [Teardown]   teardown
    
    clear_pon_pm    eutA    ${pon_port}    ${rmon_type}    ${rmon_session_1_day}     ${all_or_current}
    log    PM current bin has been cleared.
    verify_pon_pm_counters_all_cleared    eutA    ${pon_port}    ${rmon_session_1_day}    ${rmon_type}    ${num_back}    ${num_show}    ${pon_pm_counter_name}
    ...
    @{pon_pm_counter_value}    get_pon_pm_counter    eutA    ${pon_port}    ${rmon_session_1_day}    ${rmon_type}    ${num_back}    ${num_show}    rx-unicast-pkts
    Should Not Be Equal    @{pon_pm_counter_value}[1]    0
    log    rx-unicast-pkts are being counted
    @{pon_pm_counter_value}    get_pon_pm_counter    eutA    ${pon_port}    ${rmon_session_1_day}    ${rmon_type}    ${num_back}    ${num_show}    tx-unicast-pkts
    Should Not Be Equal    @{pon_pm_counter_value}[1]    0
...
setup
     ...
     prov_pon_pm    eutA     ${pon_port}    ${rmon_session_1_day}    ${bin_count}
...
teardown
    ...
    dprov_pon_pm    eutA    ${pon_port}    ${rmon_session_1_day}    ${bin_count}