...
*** Test Cases ***
tc_Each_PON_OLT_Interface_MUST_support_collection_of_a_Per_ONU_basis_Upstream_HEC_Errors
    [Documentation]    Each PON OLT Interface MUST support collection of Upstream Physical Layer Statistics on a Per ONU basis: Upstream HEC Errors
    [Tags]       @author=JerryWu     @TCID=AXOS_E72_PARENT-TC-796    @globalid=2307656
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:Each PON OLT Interface MUST support collection of Upstream Physical Layer Statistics on a Per ONU basis: Upstream HEC Errors

    ${output}    show_ont_pm    eutA    ${service_model.subscriber_point1.attribute.ont_id}    ${rmon_session_15_min}    ${rmon_type}    ${num_back}    ${num_show}
    Should Match Regexp    ${output}    upstream-gem-hec-errors \\d+
