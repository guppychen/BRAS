...
*** Test Cases ***
tc_OLT_EXA_device_must_support_the_ability_to_clear_the_PM_Session_History_Bins
    [Documentation]    A user with the appropriate permissions must be able to clear the history PM Session - this operation MUST be recorded. We need topersistentlyrecord who did the enable/disable operation - this needs to be reportable on the mgmt plane (EWI, CLI, NetConf) in association with the PM Session.
    ...
    ...    Step 5: Clear the history PM counter of unicast.
    ...
    [Setup]    setup
    [Teardown]    teardown
    [Tags]    @author=JerryWu    @tcid=AXOS_E72_PARENT-TC-763   @globalid=2307623    @eut=NGPON2-4    @priority=P1
    ...
    @{pon_pm_counter_value}    get_pon_pm_counter    eutA    ${pon_port}    ${rmon_session_15_min}    ${rmon_type}    ${num_back}    ${num_show}    rx-unicast-pkts
    Should Not Be Equal    @{pon_pm_counter_value}[1]    0
    log    rx-unicast-pkts are still being counted
    @{pon_pm_counter_value}    get_pon_pm_counter    eutA    ${pon_port}    ${rmon_session_15_min}    ${rmon_type}    ${num_back}    ${num_show}    tx-unicast-pkts
    Should Not Be Equal    @{pon_pm_counter_value}[1]    0
    log    tx-unicast-pkts are still being counted.
    ...
    clear_pon_pm    eutA    ${pon_port}    ${rmon_type}    ${rmon_session_15_min}     ${all_or_current}
    verify_pon_pm_counters_all_cleared    eutA    ${pon_port}    ${rmon_session_15_min}    ${rmon_type}    1    1    ${pon_pm_counter_name}
    log    History PM bin has been cleared. 
