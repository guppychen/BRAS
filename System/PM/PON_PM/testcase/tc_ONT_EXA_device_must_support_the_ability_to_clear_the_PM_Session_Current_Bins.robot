...
*** Test Cases ***
tc_ONT_EXA_device_must_support_the_ability_to_clear_the_PM_Session_Current_Bins
    [Documentation]    A user with the appropriate permissions must be able toclear the PM Session Current and History Bins. The clearing ofthe PM Session Binsneeds to berecorded. We need topersistentlyrecord who did the clear and when it was done. This needs to be reportable on the mgmt plane (EWI, CLI, NetConf) in association with the PM Session Bins.
    ...    The clear also needs to be recorded in syslog.
    [Tags]       @author=JerryWu     @TCID=AXOS_E72_PARENT-TC-766    @globalid=2307626
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP1:A user with the appropriate permissions must be able toclear the PM Session Current and History Bins. The clearing ofthe PM Session Binsneeds to berecorded. We need topersistentlyrecord who did the clear and when it was done. This needs to be reportable on the mgmt plane (EWI, CLI, NetConf) in association with the PM Session Bins.
    ...
    ${str_ont_id}     Convert To String    ${service_model.subscriber_point1.attribute.ont_id} 
    Should Be Equal    ${str_ont_id}    @{res}[1]
