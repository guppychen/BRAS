*** Settings ***
Documentation     tc_Ethernet_Statistics_CRC_Align_Errors
Resource          ./base.robot
...
*** Test Cases ***
tc_Ethernet_Statistics_CRC_Align_Errors
    [Documentation]    tc_Ethernet_Statistics_CRC_Align_Errors
    ...    Step 1:Provision all ethernet ports on ONT for Data Services. 
    ...           The provisioning should take place.
    ...    Step 2:With Traffic generator provide traffic to the port to exercise the associated counter 'etherStatsCRCAlignErrors' (CRC). 
    ...           Expect the associated counter to increment.
    
    [Setup]    Case_Test_Setup
    [Teardown]    Case_Test_Teardown
    [Tags]    @author=Meiqin_Wang    @tcid=AXOS_E72_PARENT-TC-187    @globalid=2224515    @eut=NGPON2-4    @priority=P3
    ...
    Tg Create Untagged Stream On Port    tg1    upstream1    p1    p2    mac_src=${mac1}    mac_dst=${mac2}    fcs_error=1    l3_protocol=ipv4    ip_src_addr=${ip1}    ip_dst_addr=${ip2}
    ...                                  frame_size=512    length_mode=fixed    rate_pps=1000
    ...
    ...                                                    ${rmon_type}    ${num_back}    ${num_show}    upstream-crc-errored-packets
