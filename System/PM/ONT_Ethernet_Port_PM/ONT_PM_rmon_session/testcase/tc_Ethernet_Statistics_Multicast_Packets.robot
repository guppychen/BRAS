*** Settings ***
Documentation     tc_Ethernet_Statistics_Multicast_Packets
Resource          ./base.robot
...
*** Test Cases ***
tc_Ethernet_Statistics_Multicast_Packets
    [Documentation]    tc_Ethernet_Statistics_Multicast_Packets
    ...    Step 1:Provision all ethernet ports on ONT for Data Services. 
    ...           The provisioning should take place.
    ...    Step 2:With Traffic generator provide traffic to the port to exercise the associated counter 'etherStatsMulticastPkts'.
    ...           Expect the associated counter to increment.
    
    [Setup]    Case_Test_Setup
    [Teardown]    Case_Test_Teardown
    [Tags]    @author=Meiqin_Wang    @tcid=AXOS_E72_PARENT-TC-186    @globalid=2224514    @eut=NGPON2-4    @priority=P3
    ...
    Tg Create Untagged Stream On Port    tg1    upstream1    p1    p2    mac_src=${mac1}    mac_dst=${mac4}    l3_protocol=ipv4    ip_src_addr=${ip1}
    ...                                  ip_dst_addr=${ip4}    frame_size=512    length_mode=fixed    rate_pps=100
    ...
    ...                                       rate_pps=100    mac_src=${mac2}    mac_dst=${mac4}    l3_protocol=ipv4    ip_src_addr=${ip2}    ip_dst_addr=${ip4}
    ...
    ...                                                    ${duration}    ${rmon_type}    ${num_back}    ${num_show}    upstream-multicast-packets
