*** Settings ***
Documentation     tc_Ethernet_Statistics_Octets
Resource          ./base.robot
...
*** Test Cases ***
tc_Ethernet_Statistics_Octets
    [Documentation]    tc_Ethernet_Statistics_Octets
    ...    Step 1:Provision all ethernet ports on ONT for Data Services. 
    ...           The provisioning should take place.
    ...    Step 2:With Traffic generator provide traffic to the port to exercise the associated counter 'etherStatsOctets'.
    ...           Expect the associated counter to increment.
    
    [Setup]    Case_Test_Setup
    [Teardown]    Case_Test_Teardown
    [Tags]    @author=Meiqin_Wang    @tcid=AXOS_E72_PARENT-TC-184    @globalid=2224512    @eut=NGPON2-4    @priority=P3
    ...
    Tg Create Untagged Stream On Port    tg1    upstream1    p1    p2    mac_src=${mac1}    mac_dst=${mac2}    l3_protocol=ipv4    ip_src_addr=${ip1}    ip_dst_addr=${ip2}    frame_size=512    length_mode=fixed    rate_pps=1000
    Tg Create single Tagged Stream On Port    tg1    downstream1    p2    p1    vlan_id=${service_vlan}    vlan_user_priority=0    frame_size=512    length_mode=fixed    rate_pps=1000    mac_src=${mac2}    mac_dst=${mac1}    l3_protocol=ipv4    ip_src_addr=${ip2}    ip_dst_addr=${ip1}
    ...
    ${tx_total_pkt_bytes}    get_traffic_stream_stats    tg1    upstream1    tx    total_pkt_bytes
    
    ${ont_pm_counter}    check_interface_ont_pm    eutA    ${service_model.subscriber_point1.attribute.interface_type}    ${service_model.subscriber_point1.name}    ${duration}    ${rmon_type}    ${num_back}    ${num_show}    upstream-octets
    log    the total frame size is ${ont_pm_counter}
    log      compare send traffics = ont_pm
    check_correct    ${tx_total_pkt_bytes}    ${ont_pm_counter}
