*** Settings ***
Documentation     tc_Ethernet_Statistics_Oversize_Packets
Resource          ./base.robot
...
*** Test Cases ***
tc_Ethernet_Statistics_Oversize_Packets
    [Documentation]    tc_Ethernet_Statistics_Oversize_Packets
    ...    Step 1:Provision all ethernet ports on ONT for Data Services. 
    ...           The provisioning should take place.
    ...    Step 2:With Traffic generator provide traffic to the port to exercise the associated counter 'etherStatsOversizePkts' (frames > 1518).
    ...           Expect the associated counter to increment.
    
    [Setup]    Case_Test_Setup
    [Teardown]    Case_Test_Teardown
    [Tags]    @author=Meiqin_Wang    @tcid=AXOS_E72_PARENT-TC-189    @globalid=2224517    @eut=NGPON2-4    @priority=P3
    ...
    ...                                  ip_dst_addr=${ip2}    frame_size=1519    length_mode=fixed    rate_pps=1000
    Tg Create single Tagged Stream On Port    tg1    downstream1    p2    p1    vlan_id=${service_vlan}    vlan_user_priority=0    frame_size=1519    length_mode=fixed
...
PM checking
    ...
    ...                                                    ${duration}    ${rmon_type}    ${num_back}    ${num_show}    upstream-oversize-packets
