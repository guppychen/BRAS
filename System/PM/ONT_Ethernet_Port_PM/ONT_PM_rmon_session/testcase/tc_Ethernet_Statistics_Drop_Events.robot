*** Settings ***
Documentation     tc_Ethernet_Statistics_Drop_Events
Resource          ./base.robot
...
*** Test Cases ***
tc_Ethernet_Statistics_Drop_Events
    [Documentation]    tc_Ethernet_Statistics_Drop_Events
    ...    Step 1:Provision all ethernet ports on ONT for Data Services. 
    ...           The provisioning should take place.
    ...    Step 2:With Traffic generator provide traffic to the port to exercise the associated counter 'etherStatsDropEvents' (i.e. CRC).
    ...           Expect the associated counter to increment.
    
    [Setup]    Case_Test_Setup
    [Teardown]    Case_Test_Teardown
    [Tags]    @author=Meiqin_Wang    @tcid=AXOS_E72_PARENT-TC-183    @globalid=2224511    @eut=NGPON2-4    @priority=P3
    ...
    ...                                  ip_dst_addr=${ip2}    frame_size=512    length_mode=fixed    rate_pps=1000    fcs_error=1
...
PM checking
    ...
    ...                                                    ${duration}    ${rmon_type}    ${num_back}    ${num_show}    upstream-drop-events
