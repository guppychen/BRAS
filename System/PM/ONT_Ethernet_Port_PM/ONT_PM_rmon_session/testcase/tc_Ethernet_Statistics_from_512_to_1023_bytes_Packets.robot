*** Settings ***
Documentation     tc_Ethernet_Statistics_from_512_to_1023_bytes_Packets
Resource          ./base.robot
...
*** Test Cases ***
tc_Ethernet_Statistics_from_512_to_1023_bytes_Packets
    [Documentation]    tc_Ethernet_Statistics_from_512_to_1023_bytes_Packets
    ...    Step 1:Provision all ethernet ports on ONT for Data Services. 
    ...           The provisioning should take place.
    ...    Step 2:With Traffic generator provide traffic to the port to exercise the associated counter 'etherStatsHighCapacityPkts512to1023Octets' (frames between 512 and 1023 bytes).
    ...           Expect the associated counter to increment.
    
    [Setup]    Case_Test_Setup
    [Teardown]    Case_Test_Teardown
    [Tags]    @author=Meiqin_Wang    @tcid=AXOS_E72_PARENT-TC-193    @globalid=2224521    @eut=NGPON2-4    @priority=P3
    ...
    ...                                  ip_dst_addr=${ip2}    frame_size=512    length_mode=fixed    rate_pps=1000
