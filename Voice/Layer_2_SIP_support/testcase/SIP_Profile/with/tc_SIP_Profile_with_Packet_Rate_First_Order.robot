...
*** Test Cases ***
tc_SIP_Profile_with_Packet_Rate_First_Order
    [Documentation]    1. Enter SIP-Profile packet-rate first-order = 10ms and 20ms and 30ms, packet-rate first-order = selection
    [Tags]       @author=XUAN LI     @TCID=AXOS_E72_PARENT-TC-3324    @globalid=2473081    @eut=GPON-8r2    @priority=P1    @user_interface=CLI
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1. Enter SIP-Profile packet-rate first-order = 10ms and 20ms and 30ms, packet-rate first-order = selection
