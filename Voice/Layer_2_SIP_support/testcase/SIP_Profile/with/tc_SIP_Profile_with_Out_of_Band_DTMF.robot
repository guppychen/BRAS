...
*** Test Cases ***
tc_SIP_Profile_with_Out_of_Band_DTMF
    [Documentation]    1. Enter SIP-Profile out-of-band-dtmf = none and rfc2833 and info, out-of-band-dtmf = selection, Out-of-band DTMF = selection
    ...    2. Enter SIP-Profile with out-of-band-dtmf = abc, command rejected
    [Tags]       @author=XUAN LI     @TCID=AXOS_E72_PARENT-TC-3369    @globalid=2473126    @eut=GPON-8r2    @priority=P1    @user_interface=CLI
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1. Enter SIP-Profile out-of-band-dtmf = none and rfc2833 and info, out-of-band-dtmf = selection, Out-of-band DTMF = selection
    ...
    Wait Until Keyword Succeeds    2min    10sec    check_running_configure    eutA    sip-profile    ${sip_profile}    | detail    out-of-band-dtmf=${out_of_band_dtmf1}
    ...
    Wait Until Keyword Succeeds    2min    10sec    check_running_configure    eutA    sip-profile    ${sip_profile}    | detail    out-of-band-dtmf=${out_of_band_dtmf2}
    Wait Until Keyword Succeeds    5min    10sec    check_ont_sip_profile    ontA    ${out_of_band_dtmf2}    Out-of-band DTMF
    
    log    STEP:2. Enter SIP-Profile with out-of-band-dtmf = abc, command rejected
    cli    eutA    configure
