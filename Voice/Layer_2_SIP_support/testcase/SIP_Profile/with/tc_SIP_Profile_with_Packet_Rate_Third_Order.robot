*** Settings ***
Documentation     SIP Profile with Packet Rate Third Order
Resource          ./base.robot
...
*** Test Cases ***
tc_SIP_Profile_with_Packet_Rate_Third_Order
    [Documentation]    1. Enter SIP-Profile packet-rate third-order = 10ms and 20ms and 30ms, packet-rate third-order = selection, Pkt Rate 3rd order set to selection in ONT
    [Tags]       @author=XUAN LI     @TCID=AXOS_E72_PARENT-TC-3332    @globalid=2473089    @eut=GPON-8r2    @priority=P1    @user_interface=CLI
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1. Enter SIP-Profile packet-rate third-order = 10ms and 20ms and 30ms, packet-rate third-order = selection, Pkt Rate 3rd order set to selection in ONT
