...
*** Test Cases ***
tc_SIP_Profile_modification_of_Packet_Rate_Second_Order
    [Documentation]    1. Edit SIP-Profile packet-rate second-order = 10ms and 20ms and 30ms, packet-rate second-order = selection, Pkt Rate 2nd order set to selection in ONT
    [Tags]       @author=XUAN LI     @TCID=AXOS_E72_PARENT-TC-3329    @globalid=2473086    @eut=GPON-8r2    @priority=P1    @user_interface=CLI
    [Setup]      case setup
    [Teardown]   case teardown
    log    STEP:1. Edit SIP-Profile packet-rate second-order = 10ms and 20ms and 30ms, packet-rate second-order = selection, Pkt Rate 2nd order set to selection in ONT
